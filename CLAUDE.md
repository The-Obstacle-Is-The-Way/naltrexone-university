...
All `/app/*` pages require Clerk auth. For this repo, Playwright itself remains the guaranteed authenticated path. The committed Playwright + CDP bridge documented in `docs/dev/agent-browser.md` is still the best `agent-browser` lead, but you must re-verify it in the current environment instead of assuming it stays authenticated after `agent-browser connect`. Do not assume `agent-browser --state` can reliably reuse a Playwright Clerk login. Keep the app host exact (`localhost` vs `127.0.0.1` matters for Clerk cookies). When using Chrome MCP tools, the user's browser is typically already authenticated — use `tabs_context_mcp` to check existing tabs. **Never skip visual verification because of an auth redirect — authenticate first.**
