...
async function DeferredGetStartedCta() {
...
async function MarketingHeroCopy() {
  'use cache';
  ...
  return (
    <>
      ...
      <p className="mx-auto mt-6 max-w-2xl text-balance text-lg text-muted-foreground leading-relaxed md:text-xl">
        ...
        and Addiction Medicine. Practice with confidence and track your
        progress.
      </p>
    </>
  );
...
async function MarketingImpactStatsSection() {
  'use cache';
...
async function MarketingFeaturesSection() {
  'use cache';
...
async function MarketingPricingSection() {
  'use cache';
    ...
    <section aria-label="Pricing" className="border-t border-border py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          ...
          <p className="mt-3 text-base text-muted-foreground">
            One subscription unlocks the full question bank and all study modes.
          </p>
        </div>
...
async function MarketingFinalCtaSection() {
  'use cache';

  return (
    <section aria-label="Get started" className="border-t border-border py-20">
      <div className="mx-auto max-w-3xl px-4 text-center sm:px-6 lg:px-8">
        <h2 className="font-heading text-3xl font-bold tracking-tight md:text-4xl">
          Ready to start studying?
        </h2>
        <p className="mt-4 text-base text-muted-foreground">
          Join physicians and psychiatrists preparing for addiction boards. Full
...
export async function MarketingHomeShell({
  authNavSlot,
  primaryCtaSlot,
        ...
        <section aria-label="Hero" className="py-20 lg:py-32">
          <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
            ...
            <div className="mt-10 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
              <Suspense fallback={<MarketingPrimaryCtaFallback />}>
                {primaryCtaSlot ?? <DeferredGetStartedCta />}
              </Suspense>
              <Button asChild variant="outline" className={pillSizeClasses}>
...
export async function renderMarketingHome(input?: {
  ...
  getStartedCtaFn?: () => ReactNode | Promise<ReactNode>;
  ...
  const getStartedCtaFn =
    input.getStartedCtaFn ?? (() => <MarketingPrimaryCtaFallback />);

  const [authNavSlot, primaryCtaSlot] = await Promise.all([
    authNavFn(),
    getStartedCtaFn(),
  ]);
