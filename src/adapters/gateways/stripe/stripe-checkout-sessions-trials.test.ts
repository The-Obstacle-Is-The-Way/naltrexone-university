...
describe('createStripeCheckoutSession trial params', () => {
  ...
  it('adds no-card trial params with annual price and variant-scoped idempotency key', async () => {
    const { stripe, sessionsCreate } = createStripeMock();
  ...
  it('reuses an existing annual trial checkout session when checkout variant matches', async () => {
    const { stripe, sessionsCreate, sessionsExpire, sessionsRetrieve } =
      createStripeMock({
        openSessionsData: [
          { id: 'cs_existing', url: 'https://stripe/checkout/annual-trial' },
        ],
    ...
    ).resolves.toEqual({ url: 'https://stripe/checkout/annual-trial' });
