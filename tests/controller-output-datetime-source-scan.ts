// Source scanner behind the controller output datetime contract
...
export type PassThroughTypeScan = {
  action: string;
  filePath: string;
  prefix: string;
  typeName: string;
...
// Zod 4 shapes read by the scanner: every def has a type, and every check
// carries its definition under _zod.def.
type ZodCheck = { _zod: { def: { format?: unknown } } };
...
export const EXPECTED_SCHEMALESS_ACTIONS: readonly PassThroughAction[] = [
  {
...
export const PASS_THROUGH_TYPE_SCANS: readonly PassThroughTypeScan[] = [
  {
...
export function readControllerSources(dir = CONTROLLER_ROOT): SourceFile[] {
...
export function parseSourceText(filePath: string, source: string): SourceFile {
  return {
    filePath,
    source,
...
function isDatetimeStringSchema(schema: ZodSchema): boolean {
  ...
  return (
    def.type === 'string' &&
    (def.checks ?? []).some((check) => check._zod.def.format === 'datetime')
  );
}
...
export function getZodContractIssues(
  schema: ZodSchema,
  schemaName: string,
  ...
  if (
    ...
    const representation =
      def.type === 'number' ? 'date-like z.number()' : `z.${def.type}()`;
    issues.push(
...
export function collectOutputSchemaSourceIssues(
...
export function collectSchemaLessActions(
  sourceFiles: readonly SourceFile[],
...
// TypeScript 5 parses a null type as a literal type, handled below.
...
export function collectPassThroughDatetimeFields(
  scans: readonly PassThroughTypeScan[] = PASS_THROUGH_TYPE_SCANS,
  readSource: (filePath: string) => SourceFile = parseRepoSource,
): {
  fields: string[];
  issues: ContractIssue[];
} {
  ...
  for (const scan of scans) {
    const sourceFile = readSource(scan.filePath);
