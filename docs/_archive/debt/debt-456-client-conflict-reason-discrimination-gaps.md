...
**Resolved:** 2026-07-21 — FW-2 reason-discriminated both recovery arms through the existing shared practice-session helper. The email-ownership regression cases now retain the server message and perform no reload, summary probe, or key rotation, while the live reasonless bootstrap/end compatibility paths and existing typed session recoveries remain green.
...
> **Resolution anchor correction (2026-07-21, FW-2):** the bootstrap discriminator/server-message fallback is now [`use-practice-session-page-model.ts#L447-L467`](<../../../app/(app)/app/practice/[sessionId]/hooks/use-practice-session-page-model.ts#L447>), and the end/finalize discriminator, compatibility recovery, and fail-safe fallback are [`practice-session-page-logic.ts#L258-L299`](<../../../app/(app)/app/practice/[sessionId]/practice-session-page-logic.ts#L258>).
...
- [DEBT-438 (archived, resolved PR #608)](./debt-438-conflict-reason-client-coverage-gaps.md) — established the targeted-arm/fail-safe pattern this item extends to the new reason.
- [BUG-284 (archived, resolved PR #628)](../bugs/bug-284-user-upsert-email-reclaim-cross-identity-takeover.md) — introduced the reason; its fix scope deliberately excluded client UX arms.
