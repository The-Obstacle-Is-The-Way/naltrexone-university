...
**Status:** ✅ **RESOLVED 2026-06-11 — feature shipped + flag removed; archived.** PR-1 (`8e2e1489`), PR-2 (`f7463dec`), PR-3 (`3ba5576a`), PR-4 launch/config, the E2E tail **DEBT-415** ([PR #417](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/417)), and the flag-removal cleanup **DEBT-413** ([PR #418](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/418)) are all done — the trial is live in production and unconditionally on for eligible first-timers. The one remaining tail is **DEBT-414** (legal pages — tabled, owner-gated). Archived together with DEBT-413.
...
**Related:** [Debt Index](../../debt/index.md), [DEBT-332](../../debt/debt-332-security-posture-audit.md) (billing-flow CSP `form-action`), [DEBT-406 archived](./debt-406-stripe-live-endpoint-version-reconciliation.md) (live webhook endpoint pinned to `2026-05-27.dahlia`), [DEBT-412 archived](./debt-412-checkout-success-trial-copy-redirect-gap.md) (checkout-success interstitial shipped), [DEBT-413](./debt-413-remove-free-trial-enabled-flag.md) (remove the rollout flag — ready to implement; merge after standard gate + owner grade), [DEBT-414](../../debt/debt-414-public-legal-pages-privacy-terms.md) (legal pages + Stripe legal links + descriptor verification), [DEBT-415 archived](./debt-415-e2e-suite-flag-on-alignment.md) (flag-on E2E alignment + trial-start spec — resolved PR #417); Stripe primary sources cited inline in §B.2–B.4.
...
| D10 | **Kill-switch: `FREE_TRIAL_ENABLED` env flag** | Billing-touching feature → a short-lived rollout lever, following the existing `NEXT_PUBLIC_SKIP_CLERK` enum-flag precedent. Operational safety, not a product toggle; Vercel env changes require redeploy. |
...
3. **Checkout success (`app/(marketing)/checkout/success/`).** PR-3 made the fallback copy trial-aware, and DEBT-412 shipped the real checkout-success interstitial so that copy is now user-visible after a completed Checkout return. `syncCheckoutSuccess()` still eagerly persists the Stripe customer mapping + subscription row before render (`app/(marketing)/checkout/success/checkout-success-sync.tsx:241-253`); invalid, signed-out, and non-entitled outcomes still redirect (`app/(marketing)/checkout/success/checkout-success-sync.tsx:97-133`, `app/(marketing)/checkout/success/checkout-success-sync.tsx:258-265`); entitled outcomes return the synced status (`app/(marketing)/checkout/success/checkout-success-sync.tsx:268`) so `page.tsx` renders "Your 7-day free trial has started — no charge today" for `inTrial`, paid confirmation for non-trial entitled statuses, an aria-live status line, a visible "Go to your dashboard" `<Button>`, and a controlled 3.5s client redirect (`app/(marketing)/checkout/success/page.tsx:41-75`, `app/(marketing)/checkout/success/checkout-success-redirect.tsx:1-29`).
...
- **Already launched — PR-4 / Config + rollout:** `FREE_TRIAL_ENABLED=true` is live in target environments; Stripe live/test webhooks cover the trial-relevant events; Customer Portal payment-method update and self-serve cancellation are configured; active recurring prices match the app's $29/mo and $199/yr plans; the Stripe account has the descriptor fields present and short enough for the trial-ending suffix. The remaining tails are intentionally split out instead of hidden here: DEBT-414 publishes legal pages + wires Stripe legal links + verifies the descriptor, DEBT-415 aligns E2E with flag-on production and adds the trial-start spec, and DEBT-413 removes the rollout flag (ready to implement; merge after standard gate + owner grade; live smoke + Fable bug-hunt are a follow-up on the flag-free code, not a gate).
...
- [x] A user who has trialed (or previously subscribed) is not offered a second trial; the normal paid flow still works for them (PR-2 + PR-3 code/tests shipped; remaining E2E coverage tracked in DEBT-415).
...
- **E2E:** no-card trial start happy path (`tests/e2e/trial-start.spec.ts`).
