...
**Related:** [src/adapters/shared/zod-schemas.ts](../../../src/adapters/shared/zod-schemas.ts), [db/schema.ts](../../../db/schema.ts), [src/domain/test-helpers/](../../../src/domain/test-helpers/), [src/application/test-helpers/fakes/](../../../src/application/test-helpers/fakes/), [docs/dev/dependency-update-protocol.md](../../dev/dependency-update-protocol.md), [DEBT-397](../../debt/debt-397-datetime-boundary-type-normalization.md), [DEBT-402](../../debt/debt-402-mocked-dto-type-assertion-drift.md), [DEBT-394 (archived)](./debt-394-supply-chain-hardening.md), PR #330

**Status:** Resolved 2026-05-31 — shipped across six PRs (one slice consciously skipped). PR 1 ([#368](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/368)) made shared factories/fakes emit `crypto.randomUUID()` defaults + added the `tests/shared/fixture-uuid-integrity.test.ts` proof harness. PR 2a ([#369](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/369)) migrated controller/shared/job/gateway app-UUID fixtures (surgically, leaving provider IDs). PR 2b ([#370](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/370)) migrated repository row fixtures modeling Drizzle `uuid()` columns. PR 3a ([#371](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/371)) migrated app/component/E2E-helper Tier-1 fixtures. PR 3b ([#372](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/372)) migrated browser-spec mocked-DTO fixtures + shared browser fixtures. **PR 3c was value-re-evaluated and consciously SKIPPED**: application use-case unit tests are fake-backed (no validator at test time), the real boundary is covered by `tests/integration/**`, and UUID-ifying readable cross-reference keys would reduce readability for zero active boundary protection. PR 4 ([#373](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/373)) shipped the durable guardrail: the `.claude/rules/fixture-integrity.md` SSOT rule (boundary definition, FIX-vs-LEAVE incl. the fake-backed-key LEAVE logic, the `vi.hoisted()`-only-inside-a-`vi.mock`-factory discipline, and named-UUID-var linkage mechanics), a `testing.md` pointer, a `CLAUDE.md` Path-Scoped Rules row, and a Schema-Validation-Majors fixture-audit step in `docs/dev/dependency-update-protocol.md`. The related mocked-DTO type-assertion-drift class was filed separately as [DEBT-402](../../debt/debt-402-mocked-dto-type-assertion-drift.md) (P3). Done-state is intentionally NOT "zero grep hits": fake-backed cross-reference keys, Tier-3 fake-repository behavior keys, and provider IDs remain by design, documented as LEAVE.
...
- Non-UUID Zod boundaries: current non-UUID validators are enums, `z.string().email()`, `z.string().url()`, and datetime schemas. The datetime boundary drift is already tracked as [DEBT-397](../../debt/debt-397-datetime-boundary-type-normalization.md). Email/URL/enum invalid values found in tests are negative validation cases, not latent fixture drift.
