...
**Created:** 2026-05-25
**Source:** Follow-up to [DEBT-392](./debt-392-dependency-hygiene-audit.md) Tier 6b, which installed Dependabot. The first weekly run on 2026-05-25 opened three PRs (#336, #337, #338); all three are red on `test`, two need real engineering and one is a hard reject. There is no documented protocol for how we triage these and no `ignore` rule preventing the hard-reject class from recurring weekly.
**Related:** [DEBT-394](../../debt/debt-394-supply-chain-hardening.md) (supply-chain hardening; this doc sets Dependabot `cooldown.default-days: 7`, which DEBT-394 must keep matched to pnpm `minimumReleaseAge: 10080`), [DEBT-392](./debt-392-dependency-hygiene-audit.md) (parent), [DEBT-340 (archived, Clerk v7 + Next.js 16.2.1 upgrade)](./debt-340-clerk-v7-nextjs-upgrade.md)

**Status:** Resolved 2026-05-25 — triaged #338 (`@types/node` 25) as a hard reject and closed it with rationale; shipped pre-Step-C test-isolation fix in PR #342 and Dependabot CI policy fix in PR #343 after #336 surfaced CI brittleness; merged #336 (`actions/upload-artifact` 6 -> 7); shipped Tier 2 Dependabot config hardening in PR #344 (`@types/node` semver-major ignore, `@biomejs/biome` split, `cooldown.default-days: 7` matched to DEBT-394's planned `minimumReleaseAge: 10080`, `versioning-strategy: increase-if-necessary`, explicit `applies-to`, and separate security-update entries); #337 was closed by Dependabot after `@dependabot recreate` as no longer needed; merged the re-emitted 10-package non-Biome group in PR #345; shipped the triage protocol in PR #346. No standalone Biome PR had appeared by archive time; the split is in place and the next Biome-only Dependabot PR should be triaged independently when emitted.
