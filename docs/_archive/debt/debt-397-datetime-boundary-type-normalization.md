...
**Related:** [src/adapters/controllers/practice-schemas.ts](../../../src/adapters/controllers/practice-schemas.ts), [src/adapters/repositories/practice-session-params.ts](../../../src/adapters/repositories/practice-session-params.ts), [docs/practice-engine/interaction-contracts.md](../../practice-engine/interaction-contracts.md), [DEBT-394 (archived)](./debt-394-supply-chain-hardening.md)

**Status:** Resolved 2026-06-01 — shipped across three PRs. PR 1 ([#377](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/377)) documented the canonical contract in `docs/practice-engine/interaction-contracts.md`: every controller action-output datetime is an ISO 8601 string (`z.string().datetime()` for schema-backed fields; `Date.toISOString()` for pass-through outputs); `z.date()` and date-like epoch `z.number()` are not allowed at the output boundary; domain/use-case layers keep `Date` and the controller serializes at the boundary. PR 2 ([#378](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/378)) migrated the two `z.date()` drift fields (`SaveExamDraftAnswerOutputSchema.latestAnsweredAt`/`draftSavedAt`) to `z.string().datetime().nullable()` and added the controller-boundary serialization, with output-facing fixtures updated (typecheck-driven); domain/use-case/persistence `Date` handling unchanged. PR 3 ([#379](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/379)) added the durable regression guardrail `src/adapters/controllers/controller-output-datetime-contract.test.ts` (AST-scans + runtime-introspects controller output schemas, rejecting `z.date()` / date-like `z.number()` / mixed union branches, requiring schema-backed datetimes to be `z.string().datetime()`, and maintaining a type-scanned inventory of the pass-through ISO outputs) — proven red on a reintroduced `z.date()`. The audit confirmed the drift was contained to those two fields; all other controller datetime outputs were already ISO. No separate debt was filed (Clerk/Stripe epochs are ingress inputs, not outputs).
