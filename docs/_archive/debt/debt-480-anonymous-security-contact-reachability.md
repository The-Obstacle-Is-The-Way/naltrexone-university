...
**Status:** Resolved — 2026-09-21; archived after anonymous production verification
**Priority:** P2
**Date:** 2026-09-20
...
The exact anonymous resource is now published without initializing Clerk. Implementation PR #976 (`0b074cee`) and ancestry/receipt PR #979 (`43ec7be7`) shipped through promotion #978 (`ac9d0fba`). Each received formal CodeRabbit approval on its exact head, zero unresolved threads and green CI. The fix changes neither the file's contents nor the protected-route policy for neighboring paths.

Main CI `35640880064` passed at **19:00:18 UTC**, including 46/46 required E2E without retries. Vercel's build was ready at **18:50:51.870 UTC**, was observed **STAGED** with the old production commit still assigned at **18:51:03 UTC**, and assigned production only at **19:00:20.500 UTC** after the required check passed. Main/dev shared tree `fe6851dbdc994a71880a523935138f25e76373b8`.

At **19:01 UTC**, fresh signed-out, non-redirect-following production GETs with both `Accept: */*` and `Accept: text/html` returned **200**, `text/plain; charset=utf-8`, and the exact unchanged contact file. `/` returned 200; `/api/health` returned 200 with app/database healthy. The [inventory ledger](../../debt/assets/inventory-2026-09-21/verification.md#debt-480-production-closeout) records the red-first proxy/built-app checks, negative-boundary mutations, gates and release evidence. All resolution/verification work below is complete; its earlier Open/no-fix wording is retained as dated history, not the current disposition.

## Description

**2026-09-21 implementation, pending release verification:** the exact static path now exits the proxy before Clerk initializes. A public-route entry alone was insufficient: built-app HTML requests still hit Clerk's `dev-browser-missing` handshake. Both anonymous Accept variants now pass with the unchanged contact file, and real-matcher negative tests retain protection for dashboard and neighboring/prefix paths. Red proofs and scope are in the [inventory ledger](../../debt/assets/inventory-2026-09-21/verification.md#debt-480-redgreen-and-scope). Status remains Open until the full gate, review, promotion and signed-out production receipt exist; the dated no-fix statements below describe their original snapshots.

**2026-09-21 re-execution.** The defect still exists on deployed main `76e65e9c`: anonymous, non-redirect-following GET returns **404** with `Accept: */*` and a Clerk `protect-rewrite` reason, and **307** with `Accept: text/html`. The file-content/expiry guard and newly shipped renewal reminder do not establish HTTP publication. No fix is claimed. [Audit receipt](../../debt/assets/active-audit-2026-09-21/verification.md).

**CONFIRMED:** `public/.well-known/security.txt` exists, but its canonical production URL is intercepted by Clerk. Fresh signed-out GETs on 2026-09-20 returned 404 with `Accept: */*` and `x-clerk-auth-reason: protect-rewrite`; `Accept: text/html` returned 307 to the identity host. [Commands and outputs](../../debt/assets/adversarial-2026-09-20/review.md#http-receipts).
...
- [DEBT-475](../../debt/debt-475-toolchain-coherence.md) — existing expiry warning/hard expiry work, unchanged
