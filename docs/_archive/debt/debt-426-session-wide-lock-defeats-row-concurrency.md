...
- The idempotency-cache semantics in [DEBT-435](../../debt/debt-435-practice-session-conflict-and-test-hygiene-follow-ups.md) are unchanged by this design because the active exam draft-save path is not idempotency-wrapped. If a future task changes cached transient-conflict behavior for finalize or mark-for-review actions, do it under DEBT-435 with a separate product decision.
