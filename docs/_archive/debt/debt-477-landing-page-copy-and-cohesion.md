...
**Status:** Resolved — archived 2026-09-18. PR #912 (f7ec2cb0) reached main through promotion #913 (fd45fdf4); main CI 35385213560 passed test and deploy. Current production layout is verified. Authenticated casing remains an explicitly out-of-scope follow-up seed.

**Pre-archive status record:** Open — filed 2026-09-16 from an owner walkthrough of the live signed-out surfaces; evidence captured against production (`main` = `58a99635`, tree-identical to `dev` `a0d4378e` for every file cited here). Implementation merged to `dev` in [PR #904](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/904) on 2026-09-17 (merge commit `6c923683`), then promoted to `main` in [PR #906](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/906) on 2026-09-17 (merge commit `09a45af6`); `main` and `dev` were left tree-identical at `8fc10e54` by that promotion, post-merge run [35259751078](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/actions/runs/35259751078) passed test and deploy, and production `/` and `/pricing` return 200 with a healthy `/api/health`. Owner decisions D1–D4 were recorded later the same day (D1 reversed: plan cards stay left-aligned; D2 held, then decided by the owner on 2026-09-18 as sentence case and implemented in [PR #912](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/912); D3 no; D4 as proposed). The `/pricing` composition and consent-copy proposal that arrived with them is verified and filed separately as [DEBT-478](./debt-478-pricing-plan-consent-dialog.md)
...
**Related:** [DEBT-250](./debt-250-frontend-visual-divergence-compliance-plan.md) (Decision 2 approved the D-15 exception this debt retires), [DEBT-258](./debt-258-marketing-alignment.md) (marketing CTA variants; Decision 1), [DEBT-382](./debt-382-landing-page-content-refresh-question-count-and-author-credibility.md) (hero copy), [DEBT-389](./debt-389-footer-layout-brand-left-links-right.md) (footer brand-left / links-right layout, which this debt keeps), [DEBT-410](./debt-410-free-trial-pathway-and-pricing-access-copy.md) (trial CTA and disclosure copy), [DEBT-414](../../debt/debt-414-public-legal-pages-privacy-terms.md) (owns the consent disclosure strings in `lib/pricing-data.ts`; added the footer legal links in #727), [Pattern Registry](../../frontend/pattern-registry.md), [Frontend Standards](../../frontend/standards.md), [Typography Policy](../../frontend/typography-policy.md), `.claude/rules/frontend.md`, [DEBT-478](./debt-478-pricing-plan-consent-dialog.md) (`/pricing` grid width, disclosure placement, and the `Back to home` link; owns everything on `/pricing` other than label casing and CTA size)
...
Screenshots (before): [hero 1440](../../debt/assets/debt-477/home-1440-hero-before-2026-09-16.png) · [pricing section 1440](../../debt/assets/debt-477/home-1440-pricing-before-2026-09-16.png) · [final CTA 1440](../../debt/assets/debt-477/home-1440-finalcta-before-2026-09-16.png) · [footer 1440](../../debt/assets/debt-477/home-1440-footer-before-2026-09-16.png) · [footer 390](../../debt/assets/debt-477/home-390-footer-before-2026-09-16.png) · [`/pricing` plans 1440](../../debt/assets/debt-477/pricing-1440-plans-before-2026-09-16.png).
...
**Phase B (owner decision D2 — held 2026-09-16; the stated blocker dissolved on 2026-09-17; decided and implemented 2026-09-18, see the notes after this paragraph):** `Subscribe Monthly` / `Subscribe Annual`. At the time of the ruling the `standardDisclosure` strings quoted the label verbatim — `lib/pricing-data.ts:32` "By selecting Subscribe Monthly, you authorize recurring monthly charges." and `:50` for annual — under `disclosureVersion: '2026-08-05'` (`:26,43`). Changing the label without the disclosure produces a visible-label / consent-text mismatch; changing the disclosure is consent evidence and belongs to DEBT-414's owner. Phase B therefore waits for a ruling on whether a casing-only change to a quoted label requires a `disclosureVersion` bump. Until then `Subscribe Monthly` / `Subscribe Annual` stay as they are, and this record says so rather than leaving the mismatch implicit. Phase B also touches `tests/e2e/checkout-redirect.spec.ts:57`, `tests/e2e/stripe-hosted-paid-checkout.spec.ts:45,48`, and the remaining `page.test.tsx` / `pricing-view.test.tsx` assertions.

**Blocker dissolved 2026-09-17 (DEBT-478, PR #905) — Phase B is now a cosmetic owner call, not a legal one.** #905 deleted `standardDisclosure` and `trialDisclosure` outright, replacing them with structured `consent: { rows, sentence, buttonLabel }` and bumping `CHECKOUT_DISCLOSURE_VERSION` to `2026-09-16`. Verified on `main` `09a45af6`: no string in `lib/`, `app/`, `src/`, or `docs/legal/` quotes `Subscribe Monthly` or `Subscribe Annual`, and the cited `lib/pricing-data.ts:32` now reads `trialCta: 'Start 7-day free trial'`. The dialog's tying sentence quotes its own commit button (`Subscribe`), which is what the consent snapshot records; the card trigger label never enters `disclosureSnapshot`. Consequences: recasing the two triggers to `Subscribe monthly` / `Subscribe annual` would need **no** `disclosureVersion` bump and **no** DEBT-414 sign-off. Cost is four string literals (`pricing-view.tsx:188,235`, `plan-consent-dialog.tsx:99,100`) plus two exact-match E2E selectors (`checkout-redirect.spec.ts:59`, `stripe-hosted-paid-checkout.spec.ts:46`) and the matching unit assertions, behind the full gate and a CodeRabbit review because it touches billing UI. Until the owner rules, these two remain the only Title Case labels on the signed-out surfaces, and Pattern Registry Part 5 records them as the deliberate current state rather than an oversight.

**Owner ruling 2026-09-18: sentence case.** The owner chose `Subscribe monthly` / `Subscribe annual`. [PR #912](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/912) recases four string literals across three controls (the two signed-out links in `pricing-view.tsx`, and the consent-dialog trigger in `plan-consent-dialog.tsx`, whose label is a two-branch conditional), the unit and browser assertions, and the two exact-match E2E selectors (`checkout-redirect.spec.ts`, `stripe-hosted-paid-checkout.spec.ts`). The updated assertions failed against the old labels first (8 of 88 unit tests), then 88 unit and 13 browser tests passed after the change. No `disclosureVersion` bump was needed: no disclosure string or consent snapshot quotes these labels. Frontend Standards, the Pattern Registry, QA-002, and the master spec now describe the sentence-case labels. This was the record's last recorded tail; archive it after the change is promoted and verified on production.
...
| D2 | Phase B casing: `Subscribe Monthly` / `Subscribe Annual` (F3) | **Held 2026-09-16** on the ground that dated disclosures quoted the labels. **That ground no longer exists:** #905 deleted those strings on 2026-09-17, so Phase B is now a cosmetic change needing no version bump and no DEBT-414 sign-off. Reopened as an owner call, then **decided 2026-09-18: sentence case** (`Subscribe monthly` / `Subscribe annual`), implemented in [PR #912](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/912); see the F3 notes. |
...
- [ ] Every button and nav-link label on `/`, `/pricing`, `/not-found`, and the marketing header/footer is sentence case except proper nouns, plan names, and page titles. **Met on 2026-09-17** except the D2 pair, which [PR #912](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/912) recased to `Subscribe monthly` / `Subscribe annual` on 2026-09-18.
...
| F2: hero height defect and predicted 48/46 result | CORRECTED | [DOM experiment](../../debt/assets/debt-477/prototype-measurements.json): 36→48 px at all four widths; outline remains 46. The residual 2 px is **4 px line-height difference minus 2 px outline border**, not “border delta only.” |
...
| F5: 3 of 53 centered Card sites; keep plans left | CORRECTED | Independent TypeScript AST [census](../../debt/assets/debt-477/source-census.json) confirms 3/53 and left-aligned empty states. The assertion that every app page has a **centered page title** is false; D1 follows list readability, not that analogy. |
| F6: footer grouping and mobile split | CONFIRMED | [390 crop](../../debt/assets/debt-477/review-before-production-home-footer-390-2026-09-16.png) shows the legal pair split. The new grid prototype keeps the legal pair on one line at all four widths; no horizontal overflow was observed. |
