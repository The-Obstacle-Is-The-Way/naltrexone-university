...
**Related:** [DEBT-393](./debt-393-dependabot-triage-and-config-hardening.md) (Dependabot triage protocol and config), [DEBT-394](./debt-394-supply-chain-hardening.md) (7-day `minimumReleaseAge` gate and override discipline), [`docs/dev/supply-chain-overrides.md`](../../dev/supply-chain-overrides.md) (override playbook; js-yaml worked example), [`docs/dev/dependency-update-protocol.md`](../../dev/dependency-update-protocol.md) (full-gate and E2E rules for repo-owned dependency PRs), [DEBT-474](../../debt/debt-474-ci-secret-scope-and-action-immutability.md) (never run a Dependabot head locally with shared credentials), [DEBT-460](./debt-460-dependency-train-residues.md) (standing dependency-train rules)

---

## Verified closeout (2026-09-22 UTC)

The September 16 remediation and the final dated fast-uri follow-up are shipped. This section supersedes the older pending labels and upstream-release assumptions below; dated evidence is preserved.

- **Source PR [#995](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/995):** exact head `41a9c6a487ecdd95e2b8ca6fb114bb2bd61f384c` received formal CodeRabbit approval `5275377558` at `07:50:11Z`; the mandatory merge guard re-read zero unresolved threads and green checks before merge `e643211b4d7150d13a98bf91efefb18831b7a41b` at `07:57:59Z`. CI `35701294196` passed, including 46/46 E2E without failures or retries.
- **Promotion [#996](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/996):** the executable source-provenance check passed for #994/#995; merge `000326290832226e86e10071f63111f28ee9fa22` reached main at `08:10:46Z`. Promotion CI `35702334421` and main CI [35703405386](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/actions/runs/35703405386) passed. Each ran 5,637 unit, 411 browser, 349 integration plus six intentional provider opt-in skips, and 46/46 required E2E with no failures or retries. The local full gate before the only implementation push also passed, including 46/46 E2E in 3.7 minutes.
- **Production hold:** Vercel was Ready at `08:13:09.435Z` and observed STAGED with the old release serving through `08:18:42.430Z`. Main `test` passed at `08:20:36Z`; Vercel's required check succeeded at `08:20:38.832Z`, then the production alias moved at `08:20:39.030Z`. Home and health returned 200, with app/database healthy. Dev/main tree identity: `85bc4102802d9acb7217abd04ae03cec272baba3`. [Full release receipt](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/996#issuecomment-5773370476).
- **Final dependency/alert evidence:** the promoted override/lockfile resolves 3.1.8; frozen installation and the red/green normalization probe are recorded below. The post-release complete open-alert API count is **0**. The original thirteen remain twelve fixed and one owner-approved dismissal; no new dismissal or audit-ignore entry was added.

### Deferred accepted risk and upstream correction
