...
**Status:** Resolved
**Resolution State:** Fixed in PR #596 (squash `f7528935` to `dev`): seed re-import now blocks in-place answer-key flips over graded attempts or graded practice-session state unless `SEED_ALLOW_KEY_CHANGES_OVER_GRADED_HISTORY=true` is explicitly set, with slug/path/count audit context and regression coverage for block, override, no-history, text-only, and fail-fast batch semantics. Promoted to `main` via PR #597 (merge `603f27b3`, main/dev trees identical); CodeRabbit approved the exact promo head after the rate-limit cooldown, production deployment `56j4BMx4o15fDtWATVzXN4xw7gom` completed 2026-07-08, CI passed on the merge commit, and `https://addictionboards.com/` returned HTTP/2 200.
**Severity:** P3
**Date:** 2026-07-05
...
- [`question-syncer.ts`](../../../scripts/seed/question-syncer.ts#L268-L287) upserts existing choices and pre-fix rewrote `isCorrect` unconditionally.
...
- PR #596 added the code and tests, PR #597 promoted it to `main`, production deployment `56j4BMx4o15fDtWATVzXN4xw7gom` completed successfully for merge commit `603f27b3`, the main-branch CI run passed, and `https://addictionboards.com/` returned HTTP/2 200.
