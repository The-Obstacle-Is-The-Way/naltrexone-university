...
**Status:** Resolved
**Resolution State:** Fixed in PR #581 (squash `cb58133a` to `dev`): unauthenticated pricing CTAs carry the selected plan and return destination through Clerk sign-up via ROUTES helpers; anonymous payment_processing recovery context preserved; e2e expectations made literal in PR #586 (squash `b29148de`); promoted to `main` via PR #587 (merge `4c380f61`, main/dev trees identical); production deployment `5355265999` succeeded 2026-07-08, `https://addictionboards.com/` returned HTTP/2 200.
**Severity:** P3
**Date:** 2026-06-30
