...
**Status:** Resolved
**Resolution State:** Fixed in PR #580 (squash `44424b2a` to `dev`): SessionSummaryView focuses its heading via the focusElementWithoutScroll precedent on genuine transitions only; hardened by PR #585 (squash `dc39679c`): commit completion now moves focus to the answer-feedback region so keyboard/AT users are never stranded on an unmounted/disabled control; promoted to `main` via PR #587 (merge `4c380f61`, main/dev trees identical); production deployment `5355265999` succeeded 2026-07-08, `https://addictionboards.com/` returned HTTP/2 200.
**Severity:** P3
**Date:** 2026-06-30
