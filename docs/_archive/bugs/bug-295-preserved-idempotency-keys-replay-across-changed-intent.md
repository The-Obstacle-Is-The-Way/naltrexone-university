...
Residuals filed from the wave-2 close review: this doc's invariant was implemented for the feedback tokens and the local abandon-success arm only — [BUG-298](./bug-298-preserved-keys-unbound-on-submit-mark-bookmark-surfaces.md) completed the unswept sibling surfaces (submit-answer choice binding, mark-for-review, bookmark direction, and the server guard's missing attempt/session context fields), and [BUG-299](./bug-299-recovery-panel-external-resolution-stale-state-dead-ends.md) closed recovery-panel resolutions that bypass the local abandon-success arm (terminal lifecycle conflicts and second-tab resolution). Both were fixed and production-verified in wave 3.
