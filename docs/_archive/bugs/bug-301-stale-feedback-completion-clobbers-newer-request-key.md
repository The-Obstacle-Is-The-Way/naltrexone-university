...
Fixed in [PR #656](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/656), which received CodeRabbit formal APPROVED review on exact final head `beca12ea` and was squash-merged to dev as `da3cdfc6`. Promoted through [PR #658](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/658), which also received CodeRabbit formal APPROVED review on exact head `da3cdfc6` after its rate-limit cooldown was honored in full; promo merge `889b4178` reached main. Main CI run [29453523008](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/actions/runs/29453523008) passed both test and deploy jobs.

- **Approach:** The neutral `idempotency-request-key.ts` module now owns a reusable generation-fenced request-key slot. Each invocation claims the slot, current-generation retry/success/error transitions remain writable, and a superseded invocation's token writes are ignored. `usePracticeQuestionFeedback` applies that owner claim to both rating and report before launching the unchanged shared helpers, covering all three existing consumers by construction: the practice question flow, practice-session page model, and standalone question page.
- **Tests:** Red-first coverage reproduces both stale-completion failures in Chromium (reattempted rating and edited report after dialog close), pins the slot's compare-and-set contract, and exercises the lost-response interleaving through real `withIdempotency`, `RateQuestionUseCase`, and `FakeQuestionFeedbackRepository`; the newer key replays without a duplicate append-only write. Existing controls continue to pin consumed-success rotation, determinate-error rotation, and the helper's same-generation reused-token retry. No server or schema changes shipped.

Fresh fix-wave-4 close proof on 2026-07-16 found main and dev byte-identical at tree `b40078a8`; the final wave main run [29459937203](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/actions/runs/29459937203) remained successful including deploy, and `https://addictionboards.com/` returned HTTP/2 200. The close review confirmed that dialog presentation has a separate stale-completion owner and filed that lower-severity residue as [BUG-302](../../bugs/bug-302-stale-report-completion-closes-newer-dialog.md); BUG-301's token-slot invariant remains correctly fixed.

## Related

- [BUG-295 (archived)](./bug-295-preserved-idempotency-keys-replay-across-changed-intent.md) — established fingerprint-bound preservation and retirement for feedback tokens.
- [BUG-298 (archived)](./bug-298-preserved-keys-unbound-on-submit-mark-bookmark-surfaces.md) — completed identity binding across sibling surfaces and extracted the neutral key primitive; this finding is the distinct asynchronous ownership gap.
- [BUG-289 (archived)](./bug-289-idempotency-caches-transient-errors-billing-bookmark-feedback.md) — owns feedback's determinacy-aware server policy and request-token persistence mechanism.
