...
**Resolution State:** Implemented on branch `fix/bug-285-286-stripe-failure-domain-lock-order` in [PR #626](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/626), "Fix BUG-285/286: durable Stripe webhook failure state + canonical subscription-writer lock order" (merged and production-verified 2026-07-11 — see the Resolution section below). Phase A moves failure persistence to a fresh transaction, preserves the original processing error, and adds unit plus real-Postgres aborted-transaction regressions.
