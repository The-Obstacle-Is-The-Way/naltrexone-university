...
Tracer-bullet path:
1. `runLoadQuestionFlow` at [question-flow-actions.ts:22-97](../../../app/(app)/app/practice/shared/question-flow-actions.ts#L22) correctly uses `canCommit()` (line 43-47) which checks both `isMounted()` AND `isLatestRequest(requestId)`.
2. `runSubmitAnswerFlow` at [question-flow-actions.ts:133-201](../../../app/(app)/app/practice/shared/question-flow-actions.ts#L133) only checks `isMounted()` (lines 180, 188).
...
5. The same stale path also calls `input.onSuccess?.(res.data)` at line 199, and the caller in [use-practice-session-page-controller.ts:60-71](../../../app/(app)/app/practice/[sessionId]/hooks/use-practice-session-page-controller.ts#L60) uses that result for `maybeAutoAdvanceAfterSubmit`, so stale responses can trigger wrong navigation side effects.
