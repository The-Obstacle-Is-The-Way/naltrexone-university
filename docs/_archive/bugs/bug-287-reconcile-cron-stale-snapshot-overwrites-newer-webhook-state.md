...
Fixed in [PR #635](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/635) (squash `e2e20f6b` to dev), promoted via PR #636 (main `209b331f`, main/dev trees identical); production deploy succeeded and `https://addictionboards.com/` returned HTTP/2 200. Migration 0029 adds a monotonic `stripe_subscriptions.version`; every Stripe-refresh writer (reconcile Phase 4, webhook, checkout-success eager sync) now reads the local version **before** its Stripe retrieve and passes it into `upsert` as `expectedVersion`, the advisory-locked transaction persists only when the stored version still matches (incrementing on success), and a `version_conflict` outcome retries the whole local-read → Stripe-retrieve → write operation a bounded number of times (`persistSubscriptionObservation`). No local request timestamps or Stripe event `created` values are consulted, per Proposed Fix §2. This closes both confirmed interleavings: the cron's Phase-1→Phase-4 stale-snapshot write and reverse-retrieve-order concurrent webhook commits.

Residuals filed from the wave-2 close review: [BUG-297](../../bugs/bug-297-checkout-success-version-cas-exhaustion-uncaught-conflict.md) — the checkout-success call site surfaces the CAS loop's bounded-exhaustion `CONFLICT` uncaught to a just-paid user whose row the webhook burst has already written correctly; and [DEBT-457](../../debt/debt-457-wave2-determinacy-and-test-hygiene-residues.md) Item 2 — the webhook integration's per-attempt live Stripe retrieve doubles the steady-state API cost per subscription event.
...
- Merged and production-verified 2026-07-12 — see the Resolution section above.
...
4. Accept + document as a retry-backed but unbounded anomaly, given the narrow window and daily convergence opportunity, recording the ruling in the register (precedent: [DEBT-408](../debt/debt-408-clerk-ui-solana-react-native-subtree.md) and the DEBT-437 ACCEPT rulings).

## Related

- [BUG-242](./bug-242-stale-subscription-webhook-overwrites-active-row.md) — fixed the cross-subscription identity gap in the same guard; explicitly scoped its same-subscription ruling to sequential out-of-order arrivals under the re-fetch design, so this finding is not a regression of that fix.
- [DEBT-437](../debt/debt-437-tutor-submit-vs-end-write-skew-and-debt-426-residual-wording.md) — ACCEPT-ruling precedent cited by remediation option 4; verified as a different seam, not reducible to that ruling.
