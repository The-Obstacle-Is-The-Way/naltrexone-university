...
[BUG-295](./bug-295-preserved-idempotency-keys-replay-across-changed-intent.md)'s start-key retirement is composed exclusively through the **local abandon-success** arm: [`use-practice-session-controls.ts#L50-L57`](<../../../app/(app)/app/practice/hooks/use-practice-session-controls.ts#L50-L57>) retires the preserved start key only when `abandonIncompleteSession` resolves `true`, which happens only on a consumed `ok` result ([`practice-page-incomplete-session.ts#L193-L200`](<../../../app/(app)/app/practice/practice-page-incomplete-session.ts#L193-L200>)). Every other way the recovery session can be resolved leaves stale client state behind:
...
Fixed in [PR #652](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/652) (CodeRabbit formal APPROVED review on exact final head `f067e779`; squash `175f35d8` to dev), promoted via [PR #653](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/653) (main `76de5ba3`).
...
Final wave proof: main CI run [29383288782](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/actions/runs/29383288782) succeeded on `76de5ba3`, including deploy; main and dev were byte-identical at tree `c4fc04e0`; and a fresh close-time request to `https://addictionboards.com/` returned HTTP/2 200. The close review found that the new loaded-null retirement is too broad specifically for a still-running `ConcurrentRequestInProgress` claim and filed the narrow residue as [BUG-300](../../bugs/bug-300-concurrent-start-refresh-retires-pending-key.md); BUG-299's externally-ended tutor and second-tab resolution behaviors remain correctly fixed.
