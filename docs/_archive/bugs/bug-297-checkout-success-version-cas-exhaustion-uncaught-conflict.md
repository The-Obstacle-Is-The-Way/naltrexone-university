...
Fixed in [PR #651](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/651) (CodeRabbit formal APPROVED review on exact final head `4668c8c9`; squash `6c01187b` to dev), promoted via [PR #649](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/649) (main `b5c2767c`). The shared observation helper now throws `SubscriptionObservationAttemptsExhaustedError` only when all bounded version-CAS attempts are exhausted. The checkout-success display path discriminates that exact typed outcome, reads the current subscription row, and renders entitlement from that row; a missing fallback row rethrows the typed error. Both the recovered and fail-closed arms emit structured telemetry with the local user id, typed reason, and attempt count. Generic `CONFLICT` errors, including the user-changed-during-refresh outcome, continue to propagate unchanged. The Stripe webhook controller and its per-attempt retrieval path are untouched.
