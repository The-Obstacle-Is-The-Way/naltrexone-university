...
**Status:** Resolved
**Resolution State:** Fix implemented and merged to `dev` in PR #582 (squash `a3855c81`); post-merge CodeRabbit findings remediated in PR #583 (squash `224514a3`: Submit pending-state gate, required selection origin, consolidated pointer-arm clearing); discriminator hardened in PR #585 (squash `dc39679c`: primary-button-only arming with contextmenu/blur clears — closing a confirmed stale-arm resurrection — Enter-commit restricted to the radios, focus-to-feedback on commit); decision record [BS-064](../../brainstorming/bs-064-radio-choice-modality-split.md); promoted to `main` via PR #587 (merge `4c380f61`, main/dev trees identical); production deployment `5355265999` succeeded 2026-07-08, `https://addictionboards.com/` returned HTTP/2 200.
**Severity:** P1
**Date:** 2026-06-30
