...
**Status:** Resolved
**Resolution State:** Fixed in PR #580 (squash `44424b2a` to `dev`): milestones announce on crossing (previous > threshold >= current), lowest-crossed-only; completed by PR #584 (squash `8d2b3df2`): hidden-tab ticks neither announce nor advance the baseline (the original backgrounded-tab scenario), refocus double-updates no longer wipe the live region, and a deadline change discards the old baseline; PR #588 (squash `ee83316f`) extracted the baseline/merge rules as pure functions with their truth tables unit-tested; promoted to `main` via PR #587 (merge `4c380f61`, main/dev trees identical); production deployment `5355265999` succeeded 2026-07-08, `https://addictionboards.com/` returned HTTP/2 200.
**Severity:** P4
**Date:** 2026-06-30
