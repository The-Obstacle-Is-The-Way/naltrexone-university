...
**Status:** ✅ Resolved
**Resolution State:** Implemented in PR #420 (squash `fac21601`) and activated to live mode (`dryRun=false`) in PR #422 (squash `6679cfe2`), both merged 2026-06-12. Production deploy `dpl_GFqkgVoarFVqXbWbsq17Kh6MwtxK` is READY with the daily `0 8 * * *` cron registered. `CRON_SECRET` was normalized header-safe across all Vercel scopes (Production had been empty, Preview carried trailing whitespace) and a `scripts/validate-header-safe-secret.ts` CI guard was added. The flip to `dryRun=false` was done immediately rather than after a dry-run observation cycle because the connected Stripe account has zero subscriptions (pre-revenue), so the destructive operations have nothing unsafe to act on. Owner-graded; full gate green + CodeRabbit approved on the exact head. Archived to `docs/_archive/bugs/`.
...
**Related:** [BUG-242](./bug-242-stale-subscription-webhook-overwrites-active-row.md) & [BUG-243](./bug-243-checkout-success-replay-overwrites-active-subscription.md) (the row corruptions reconciliation would heal), [BUG-245](../../bugs/bug-245-concurrent-two-tab-checkout-creates-duplicate-subscriptions.md) (the duplicate subscriptions reconciliation would cancel), [BUG-241](../../bugs/bug-241-deploy-pipeline-has-no-migration-step.md) (same class: a documented-but-unenforced ops step), [BUG-205](./bug-205-reconciliation-prefers-stale-local-subscription-over-canonical-stripe-state.md) / [BUG-120](./bug-120-reconciliation-missing-authoritative-conflict-strategy.md) / [DEBT-155](../debt/debt-155-stripe-legacy-duplicate-subscriptions-reconciliation.md) (fixes that all assume this job actually runs)
...
1. `vercel.json` registers a daily UTC cron at `/api/cron/reconcile-stripe-subscriptions`; it shipped in PR #420 pointing at `?dryRun=true` for owner observation and was flipped to `?dryRun=false` in PR #422 (see Resolution State — safe because the Stripe account is empty pre-revenue).
...
- [x] Activated to `dryRun=false` directly in PR #422 (cron path flipped) after verifying the connected Stripe account has zero subscriptions of any status — pre-revenue, so phase-5 duplicate-cancel and the BUG-246 drain have nothing unsafe to act on.
- [x] `CRON_SECRET` present and header-safe across Production / Preview / Development (normalized this session; the prior Production value was empty, Preview had trailing whitespace), guarded by `scripts/validate-header-safe-secret.ts`.
- [ ] Owner follow-up (not CI-provable): confirm a scheduled 08:00 UTC invocation appears in Vercel production runtime logs with a 200 and a non-error `{ scanned, updated, failed, pendingStripeCancellations }` report.
