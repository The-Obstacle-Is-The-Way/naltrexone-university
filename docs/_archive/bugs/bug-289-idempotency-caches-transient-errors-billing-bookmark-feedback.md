...
Fixed in [PR #640](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/640) (squash `666097aa` to dev), promoted via PR #641 (main `7d94359b`, main/dev trees identical); production deploy succeeded and `https://addictionboards.com/` returned HTTP/2 200. All five surfaces now pass explicit `shouldCacheError` policies (Proposed Fix §1): raw/unmapped, `STRIPE_ERROR`, and `INTERNAL_ERROR` execution failures abort the fenced claim for same-key re-execution, checkout does not cache `ALREADY_SUBSCRIBED` and portal does not cache `NOT_FOUND` (both derive from mutable state under mount-fixed keys). The feedback prerequisite (§2) shipped as migration 0030 — `question_feedback.idempotency_key` with a `(user_id, kind, idempotency_key)` unique index — plus a typed `feedback_request_token_reused` CONFLICT on request-identity mismatch, and clients rotate keys only on determinate cached outcomes (§3). The identity guard's reach was completed by [BUG-295](./bug-295-preserved-idempotency-keys-replay-across-changed-intent.md) (PR #642): fingerprint-bound client tokens enforce request identity on the wrapper's committed-and-cached replay path, with the repository guard retained as defense in depth for the fenced-claim arm.
...
- 2026-07-14 (pre-merge adversarial review, same PR): checkout no longer caches `ALREADY_SUBSCRIBED` and portal no longer caches `NOT_FOUND` — both derive from mutable state (`currentPeriodEnd > now`; a customer a later checkout can create) and the billing surfaces use mount-fixed keys with no rotation, so nothing user-mutable is cacheable there. The feedback token now enforces request identity: a replayed row must match the request's question and payload or the repository surfaces a typed `feedback_request_token_reused` CONFLICT (both adapters and the fake), and the client helpers mint a fresh key and retry once on that reason — a changed vote or edited report can no longer be silently replaced by the original committed row. The report use case gained the same kind guard as rating.
- 2026-07-14 (post-merge review correction): the "can no longer be silently replaced" claim above was too broad. The repository's reused-token conflict runs inside `execute()`, and `withIdempotency` replays a completed cached outcome **before** `execute()` — so the guard reaches only the fenced-claim arm (feedback row committed, wrapper outcome missing). On the normal committed-and-cached path, a key preserved across a lost response and reused with a changed payload still silently replayed the original committed row. Filed and fixed as [BUG-295](./bug-295-preserved-idempotency-keys-replay-across-changed-intent.md): the client binds each stored key to a request fingerprint and mints fresh on intent change; the repository guard stays as defense in depth.
...
Severity rationale (P3, not the candidate's P2): unlike [BUG-278](./bug-278-end-discard-idempotency-caches-transient-errors-under-fixed-key.md)'s never-rotating `sessionId`-derived key (P2 — reload did not help and the poisoned session blocked all new practice starts for 24h), these keys rotate on hard reload/remount, so recovery exists but is undiscoverable; the trigger also requires a transient fault to land exactly during `execute()`. Comparable precedent [BUG-259](./bug-259-in-place-rate-limit-errors-cache-idempotency-retries.md) (cached `RATE_LIMITED` replay wedge, recoverable) was P4; this is worse (money-path surfaces, infra faults are not self-inflicted, recovery non-obvious) but not BUG-278-grade.
...
- [DEBT-435 (archived)](../debt/debt-435-practice-session-conflict-and-test-hygiene-follow-ups.md) — added `shouldCacheError` policies to the practice state writes; this bug extends that pattern to the remaining surfaces.
