...
**Fixed and verified in production 2026-06-22.** `FinalizeExamAnswersUseCase` now computes a server-authoritative effective end time via a new pure helper `computeFinalExamEndedAt({ now, deadline, latestAnsweredAt })` = `deadline === null ? now : min(now, max(deadline, latestAnsweredAt))`. A single `finalizationNow` is captured per finalize; finalization-created attempts receive a deadline-capped `answeredAt`, except that an accepted BUG-254 grace-window flush keeps its in-grace `answeredAt` so the persisted `endedAt` never precedes a recorded attempt. The effective `endedAt` is threaded through new optional parameters — `PracticeSessionRepository.end(id, userId, endedAt?)` and `AttemptInsertInput.answeredAt?` (Drizzle adapters + fakes) — each defaulting to the repository clock, so the tutor `EndPracticeSessionUseCase` path is unchanged. Summary and history `durationSeconds` project from the one persisted value.

TDD coverage: four use-case branches (late-submit cap, early-submit unchanged, untimed `deadline === null`, and BUG-254 grace-flush monotonicity) plus adapter-forwarding and fake-clock tests; the one pre-existing late-finalization test was recomputed for the capped output.

- **Fix:** PR #490 (squash `2a8d1a64` on `dev`); CodeRabbit `APPROVED` on the exact head `0fb4243f`, 0 unresolved threads; full gate green (unit 2911 tests, typecheck, lint, build, integration).
- **Promotion:** PR #492 (merge `ce8373ef` to `main`); required `test` check green; CodeRabbit `APPROVED` on the exact head `2a8d1a64`, 0 unresolved threads.
- **Production:** deploy `dpl_EokBHBs9hG1MgrhWHoCKvviLAgSg` verified `READY` 2026-06-22 (`addictionboards.com` HTTP 200); `main` and `dev` trees identical.
...
**Verdict: FIX (shipped 2026-06-22).** Accepting this as won't-fix would have left persisted exam timestamps/durations visibly impossible for a normal workflow. The harm was low, but the correct fix was small and belonged at the server-authoritative finalization boundary rather than relying on a client timer. The committed path below shipped as described; see **Resolution (Shipped)** above for PR/commit/deploy references.
