...
**Related:** [BS-020 (archived)](./bs-020-card-contrast-and-hover-consistency.md) — deferred residual hover standardization; [BS-031 (archived)](./bs-031-card-row-affordance-consistency.md); [Frontend Standards](../../frontend/standards.md); [DEBT-250](../debt/debt-250-frontend-visual-divergence-compliance-plan.md)
...
| 2026-02-28 | Promoted active divergences to DEBT-250 implementation plan | Converted D-1..D-16 plus low-severity UX seams into an explicit compliance debt spec with acceptance criteria and tracer-bullet verification |
| 2026-02-28 | Integrated Chrome Agent Chunk 4 findings (Tutor Session full flow audit) | 10 findings reported; 6 false flags debunked (rounded-2xl is canonical, border-border implicit via global base border token, native button semantics do not require explicit `cursor-pointer`, primary/90 is CVA variant, label wraps radio, fieldset+legend is standard), 2 already documented (D-3 bg-muted/80, D-3 border sub-detail), 1 new minor (A11Y-2: radio missing value attr), 1 clarification added (border hover opacity note in Pattern Registry). Score: 0 TRUE, 2 ALREADY KNOWN, 6 FALSE, 1 NEW MINOR, 1 CLARIFICATION |
| 2026-02-28 | Integrated Chrome Agent Chunk 5 findings (Mobile 375px responsive audit) | 8 findings reported; 6 verified as genuinely new (hamburger 40px, theme toggle 36px, systemic h-9=36px buttons, navigator 36px, submit not full-width, zero nav link gap), 1 external dependency (Clerk UserButton ~28-33px), 1 audit gap (pricing not auditable for subscribed user). All pass WCAG AA (24px min) but fail AAA (44px min). Created Decision 11 + TOUCH-1 + TOUCH-2 in DEBT-250. Score: 6 VERIFIED NEW, 1 EXTERNAL, 1 AUDIT GAP, 0 FALSE FLAGS |
| 2026-02-28 | Integrated Chrome Agent Chunk 6 findings (Light Mode full pass audit) | 8 findings reported; 5 genuinely new: systemic opacity scale failure on white (L-2 through L-5 → LIGHT-1), success/destructive text contrast fails WCAG AA at text-xs (L-6 → LIGHT-2), text-success-foreground asymmetry bug in `choice-button.tsx` (L-7 → LIGHT-3; Pattern Registry copy fixed the same day). 2 already documented (L-1: bg-card=bg-background acknowledged in Pattern Registry 1.1a; L-8: pricing audit gap known). 1 partial overlap (L-4: D-3 covers standardization but not light-mode visibility). Key systemic finding: opacity scale Part 1.2 designed for dark mode only — muted at 96.1% lightness means all opacity < 100% is imperceptible on white. Created Decisions 12+13 + LIGHT-1 + LIGHT-2 + LIGHT-3 in DEBT-250. Fixed Pattern Registry line 256 (text-success-foreground → text-success). Chrome agent accuracy: 100% on color/contrast measurements, 0 false flags. Score: 5 GENUINELY NEW, 2 ALREADY DOCUMENTED, 1 PARTIAL OVERLAP, 0 FALSE FLAGS |
| 2026-03-01 | DEBT-250 all 13 decisions resolved, all 31 items completed | Decisions 1–13 resolved across PRs #149–#153. D-1 through D-17: code fixes. COMP-1, A11Y-1, A11Y-2, STRUCT-1, AFFORD-1, LIGHT-3: code fixes. UX-1 (pricing dead space removed), UX-2 (no bookmark by design — Decision 7), UX-3 (ThemeToggle added — Decision 3), UX-4 (Clerk seam accepted — Decision 8). TOUCH-1/TOUCH-2: targeted fixes (Decision 11 Option C). LIGHT-1: asymmetry accepted (Decision 12 Option C). LIGHT-2: token contrast fixed (Decision 13). D-15: approved marketing exception (Decision 2). FE-055 follow-up (`aria-controls` wiring) resolved in `exam-review-view.tsx`, `practice-view.tsx`, and `practice-session-page-view.tsx` with unit + browser regression coverage. |
