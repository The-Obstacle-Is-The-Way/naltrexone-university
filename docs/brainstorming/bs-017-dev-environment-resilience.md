...
**Fix:** Change `url` to point to the health check. However, since the health check is `POST`-only, Playwright's URL polling (which uses `HEAD` requests) won't work directly. Options:
