...
| 2026-06-20 | Re-verified against live code; kept Active | Owner backlog review. Confirmed no `BookmarkToggle` icon component exists and all four surfaces still render text pills: Bookmarks page `Remove bookmark` + `AlertDialog` (`app/(app)/app/bookmarks/page.tsx`), practice bar `'Remove bookmark' : 'Bookmark'` (`practice-view.tsx:200`), and review bar same (`question-page-client.tsx:450`). Premise valid, not superseded. No icon-toggle decision taken — remains parked-but-living polish. |
