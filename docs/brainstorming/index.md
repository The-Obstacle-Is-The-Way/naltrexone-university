...
| [BS-056](./bs-056-magic-numbers-and-hardcoded-literals-audit.md) | Magic Numbers and Hard-Coded Literals — retry config duplicated 4×, timeout constants duplicated 19×, raw `100` limits in 3 places, HTTP status codes raw 25×, `STACK_TRACE_LIMIT` and `PRUNE_BATCH_LIMIT` duplicated 2× each, day/time primitives duplicated | Active | — |
