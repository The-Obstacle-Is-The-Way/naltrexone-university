...
| [BS-057](./bs-057-agent-browser-clerk-auth-reliability.md) | Agent-Browser Clerk Auth Reliability — native `agent-browser` auth/state reuse is unreliable for Clerk-protected `/app/*` routes; the committed CDP bridge is the best lead but is not yet reproducibly verified on local `agent-browser 0.21.1` | Active | [DEBT-322](../debt/debt-322-exam-action-bar-ux-polish.md) |
