...
**Scope:** `createStripeTrialPaymentMethodSetupSession` (`src/adapters/gateways/stripe/stripe-checkout-sessions.ts:102`) and its call chain — the DEBT-414 Stage 2 trial add-card flow. The subscription checkout ladder is **not** in scope: it wraps every create in a live-retrieval attempt and is governed by DEBT-466's settled design. Today both paths share `CHECKOUT_SESSION_RECOVERY_ATTEMPT_LIMIT = 3`; DEBT-466 Part A splits the setup side onto `TRIAL_SETUP_SESSION_RESPONSE_RECOVERY_ATTEMPT_LIMIT`, and this item then owns what that bound means.
