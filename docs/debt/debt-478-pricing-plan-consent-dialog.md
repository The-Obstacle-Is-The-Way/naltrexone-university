...
| DEBT-414 §5–6 | Existing Stripe required Terms consent, same metadata/webhook/success-sync pipeline and retention | New snapshot/version must be asserted in the real local database after both hosted journeys. Existing hosted tests **do not yet assert the ledger**; add that evidence. |
