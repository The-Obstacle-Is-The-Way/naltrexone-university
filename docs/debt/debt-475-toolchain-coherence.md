...
- `tests/shared/resolve-integration-database-url.ts` (133 LOC with test): redundant rather than orphaned — one consumer, `db-connection-session.integration.test.ts`; `tests/integration/setup.ts:61` already refuses non-local hosts for every integration file.
