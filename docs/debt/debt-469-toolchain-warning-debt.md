# DEBT-469: Toolchain Warning Debt — Restore the Abandoned Test-File-Size Policy and Gate on Zero Warnings
...
3. **Rename the vitest configs to `.mts`** (`vitest.config.mts`, `vitest.browser.config.mts`, `vitest.integration.config.mts`), updating the `--config` references in `package.json` scripts and any doc mentions. This eliminates W2 at the root in all three lanes with zero behavior change. Re-run all three lanes to verify the warning is gone and counts are unchanged.
...
5. **Burn down the suppressions by splitting files, largest and most-audited first.** Preferred seam: split by concern following the repo's own precedent (`stripe-checkout-sessions-*.test.ts`, the seven `practice-controller-*` suites). Sequence with DEBT-468 Part 3: fixture proposals (a), (b), (b2), and (c) mechanically shrink the biggest offenders (e.g., `stripe-webhook-controller.test.ts` carries 28 hand-rolled `webhookResult` literals), so extract-then-split avoids splitting duplication into more files. Each split removes that file's suppression. No completion deadline — the gate in step 4 guarantees the set only shrinks.
6. **Record the non-actionables** (W3, W4) here so future gate summaries can cite this item instead of re-investigating, and keep W5's observation duty until it either reproduces (file it) or six months pass without recurrence (drop the exception note from `AGENTS.md`).
...
- **Setting `"type": "module"` in `package.json`** — also fixes W2 but repackages every `.js`/config file in the repo as ESM; far larger blast radius than renaming three files, for the same warning.
