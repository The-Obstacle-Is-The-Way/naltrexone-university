...
**Reconcile suite burn-down (2026-09-25 UTC, with DEBT-468 Part 3 (a)):** the 1,799-line `src/adapters/jobs/reconcile-stripe-subscriptions.test.ts` moves onto `FakeStripeCheckoutClient` and splits by concern into batch processing, row failures and persistence (the original path, 15 cases), `reconcile-stripe-subscriptions-canonical.test.ts` (14) and `reconcile-stripe-subscriptions-duplicates.test.ts` (8) over a shared harness; all 37 case titles are unchanged, and without its suppression main's suite fails `biome lint --error-on-warnings` at 1,798 lines while the new files pass unsuppressed. Tracked suppressions fall **23 → 22**. [DEBT-472 disposition](./debt-472-test-double-fidelity-and-contract-discipline.md#reconcile-suite-fake-migration-and-concern-split-disposition-2026-09-25-utc).
