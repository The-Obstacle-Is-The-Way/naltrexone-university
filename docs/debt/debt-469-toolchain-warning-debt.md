...
| W6 | historical depth-3 E2E; finite-cap variant characterized 2026-08-17 | `trial-start` failing with `pricing?checkout=error&plan=monthly` after >3 same-day completed checkouts | The pre-Part-A DEBT-466 subscription traversal bound stopped after three recovery creates | **Original form resolved; generic redirect still blocks.** Part A raised and split the bound, but an execution audit later saturated cap 10 with 11 retained current-tuple completions. Local `trial-start` now fails early only when its read-only census emits `[E2E_CHECKOUT_CHAIN_SATURATED]` with a count above the cap; that receipt, and no generic `checkout=error`, scopes the temporary gate exception. Structural tail-jump work is [DEBT-470](./debt-470-checkout-replay-tail-jump.md) |
...
- [DEBT-466](./debt-466-checkout-idempotency-replay-chain-exhaustion.md) — Part A resolved historical depth-3 W6; a generic `pricing?checkout=error&plan=monthly` remains a defect signal, while only the count-bearing saturation preflight scopes the finite-cap local exception
