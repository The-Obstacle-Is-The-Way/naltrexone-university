...
| **Local E2E tests (today)** | the **remote Neon dev branch** from `.env.local` | manual (the source of DEBT-391, and the flakiness here) |
...
1. **Proven: error masking** (`reset-e2e-user-state.ts:211,454,…`, plus query-path catches) — the real DB error is discarded, so every reset write failure looks like the same deterministic catastrophe.
...
## 6. Remediation (proposed; bundle into the next PR or a dedicated test-infra PR)
...
1. **Unmask the real error (HIGH, cheap).** In `reset-e2e-user-state.ts` and `e2e-reset-shared.ts`, capture the caught error and attach it as `cause` (and include a short, non-secret form of its message) on every `E2EUserStateResetError` thrown from the `catch` blocks (`reset-e2e-user-state.ts:175-184`, `:211-216`, `:259-268`, `:314-323`, `:454-459`, `:510-519`; `e2e-reset-shared.ts:159-164`, `:203-208`). Then a future failure says *"duplicate key value violates unique constraint"* vs *"connection terminated unexpectedly"* vs *"column ... does not exist"* — instantly triageable. Add/extend unit tests in `reset-e2e-user-state.test.ts` and `e2e-reset-shared.test.ts` asserting `cause` is propagated without leaking secrets.
2. **Add a stale-baseline-owner preflight or remove fixed global IDs (HIGH).** Either stop using fixed global UUIDs in `DETERMINISTIC_BASELINE`, or explicitly detect rows with those IDs owned by a different user before seeding and fail with an actionable `E2E_RESET:STALE_BASELINE_OWNER` code. This prevents an E2E-user change from turning the reset into a deterministic masked primary-key failure.
3. **Cut reset DB client churn (MEDIUM).** In `runE2EUserStateReset()`, open **one** `postgres()` connection and pass it to service methods instead of 7 separate `postgres(url,{max:1})` opens. The current `.env.local` URL already uses a Neon pooled endpoint; this remediation should not depend on switching URLs.
4. **Consider one local E2E retry only after #1 ships (MEDIUM).** `playwright.config.ts:15` → `retries: process.env.CI ? 2 : 1` can reduce developer pain, but adding retries before error unmasking would hide the next root-cause signal. If added, document that the retry is a local ergonomics buffer, not a substitute for fixing reset errors.
...
## 8. Acceptance criteria (for the remediation PR)

- [ ] Reset errors carry the underlying Postgres error as `cause`; a forced failure surfaces the real message (unit-tested).
- [ ] Fixed-ID stale-owner risk is removed or fails with an explicit non-masked reset code.
- [ ] Reset opens materially fewer DB client lifecycles per invocation, measured.
...
- Config: `playwright.config.ts:13-18,37-42`; `tests/e2e/global.setup.ts:7-12`; `tests/e2e/practice.spec.ts:106-107`; `reset-e2e-user-state.ts:143,199,211,227,279,339,454,470`; `.github/workflows/ci.yml:20-37,106,109,111,193`.
