...
**Status:** Resolved by staged test-infra remediation: PR-A (#410) shipped reset signal integrity (causes, stale-owner guard, one reset DB connection, local retry), and PR-B implements the hermetic local Docker E2E switch plus web-server hygiene/reset-frequency review. Archive after the CodeRabbit-approved PR-B merge.
