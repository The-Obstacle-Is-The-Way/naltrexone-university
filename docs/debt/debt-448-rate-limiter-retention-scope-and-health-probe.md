...
**Part 1:**
- **Option 1 (CHOSEN, minimal form):** rename the retention policy so it is explicitly rate-limit-specific, set it to 24 hours (1,440 current one-minute windows), and pin the cutoff in `drizzle-rate-limiter.test.ts`. This reduces the rolling time horizon by 98.9% while keeping one day of debugging headroom.
