...
After the DEBT-354 god-file audit (2026-04-09) split the worst production-side offenders, the size hot spots are now overwhelmingly **test** files. As of 2026-04-25:

| LOC | File |
|----:|------|
| 2,086 | `app/(app)/app/practice/[sessionId]/components/practice-session-page-view.browser.spec.tsx` |
| 1,874 | `components/question/Feedback.test.tsx` |
...
Production-side max is 572 LOC (`history-questions-tab.tsx`) — about a quarter of the largest test file. `biome.json` does not enforce a `max-lines` style rule today. DEBT-234 was archived as resolved but no enforcement landed.
