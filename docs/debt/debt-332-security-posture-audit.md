...
- [x] Enforcing CSP is enabled **or** the accepted residual risk is explicitly documented — **RESOLVED 2026-06-14 via the accepted-residual branch.** [DEBT-420](../_archive/debt/debt-420-csp-enforcing-mode-flip.md) proved (live Sentry + a production-build experiment + official Next/Clerk docs + open Next bug #89754) that enforcing the Clerk strict **nonce + `'strict-dynamic'`** CSP is **fundamentally incompatible** with this app's Next 16 PPR/Cache Components — prerendered static-shell `<script>` tags cannot carry a per-request nonce, so enforcing would block first-party `_next/static/chunks` and break the app. **Decision: keep strict CSP in report-only as the accepted residual posture** (visibility-only; other controls graded "Pass" above). The only PPR-compatible real-enforcement path (a non-nonce host-allowlist CSP) is documented in DEBT-420 as the optional future upgrade. The earlier `form-action 'self'` billing concern is moot for the chosen posture (report-only blocks nothing).
