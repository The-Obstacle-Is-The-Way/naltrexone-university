...
The Vercel project has Web Analytics enabled at the platform layer, but the application has not integrated the analytics package or component. The current Privacy Policy accurately describes that application-build state while leaving the dashboard setting unresolved. Activation must change the integration, public notice, policy copy, provenance record, and retention disclosure atomically.
...
| The application does not integrate `@vercel/analytics`. | **ACCURATE, MEASURED, with corrected scope.** | At the audit baseline, `package.json` has no dependency and `git grep -n -E '@vercel/analytics|<Analytics' origin/dev -- package.json app components lib src` returns no application hit. Vendored agent-skill examples do contain sample imports, so the former claim that a full-repository grep found none was too broad. |
| Production currently embeds no Web Analytics script. | **ACCURATE, MEASURED at one point in time.** | Signed-out `GET /`, `/pricing`, `/terms`, and `/privacy` each returned 200 on 2026-08-10, and their HTML contained no `_vercel/insights` or `va.vercel-scripts` reference. A direct request to `/_vercel/insights/script.js` returned 200 because enabling the dashboard feature provisions analytics routes; direct asset availability does not prove that a page loads the asset or transmits an event. |
| No Web Analytics dataset has ever existed and zero events have ever been collected. | **UNVERIFIABLE.** | The earlier plan named no reproducible endpoint for its claimed 404, and Vercel documents dashboard access rather than a public Web Analytics data-query REST endpoint. Current source, current HTML, and a current empty or missing reporting view cannot prove every historical deployment. Do not carry this absolute into the register, DEBT-414, the Privacy Policy, or closure evidence. |
...
- [Using Web Analytics](https://vercel.com/docs/analytics/using-web-analytics): the supported operator path is Vercel Dashboard -> project -> Analytics, with timeframe and environment selectors.
...
New:
`- Vercel Web Analytics was re-measured on 2026-08-10. The project toggle was enabled on 2026-02-24, while the measured application baseline and signed-out production HTML had no analytics integration or embedded script. The earlier claim that a query API proved no dataset and zero historical events was not reproducible; historical transmission remains UNVERIFIABLE. DEBT-464 resolves the current integration and notice gap with four-route, query-stripped collection and atomic Privacy Policy updates.`
...
7. Open Vercel Dashboard -> project -> Analytics, select Production and a timeframe containing the test, and record a nonzero public-page view with a screenshot or export. Vercel does not document a public Web Analytics data-query REST endpoint, so do not invent one for closure.
...
9. Update this document with Results, resolved status, exact package version, account plan, current event allowance/reporting window, gate counts, review IDs, merge SHAs, production request evidence, dashboard evidence, and invariant hashes. Update the privacy document's pending status with the new production evidence. Move DEBT-464 to Resolved in the register in a docs-only closeout PR.
...
- The component adds a small client script to the four allowed pages, although the wrapper is mounted globally and drops all other page-view events before transmission. Accepted.
