...
- [W3C Referrer Policy](https://www.w3.org/TR/referrer-policy/): `strict-origin-when-cross-origin` sends a full referrer for same-origin requests, while `origin` sends only the source origin. Payload redaction does not rewrite this browser-controlled request header.
...
4. **Collection is public-route-only and fail-closed.** The only collected paths are exactly `/`, `/pricing`, `/terms`, and `/privacy`. Every query string and fragment is removed from the event URL before transmission. `Referrer-Policy: origin` removes source paths and queries from browser request headers on same-origin analytics script and intake requests. Auth, checkout, `/app`, `/api`, dynamic, malformed, and future unknown routes return `null` from `beforeSend`.
...
7. In `next.config.ts`, change the global `Referrer-Policy` header from `strict-origin-when-cross-origin` to `origin` in the same implementation commit. The current policy exposes the full path and query on same-origin requests, independently of `beforeSend`; `origin` preserves origin-level attribution while removing that identifier-bearing detail. Do not activate Analytics while the old value remains.
...
Replace the entire `No analytics` row. The new verdict must say that the old no-integration claim was deliberately superseded by DEBT-464, that collection is restricted to four exact public paths, that event URLs omit query strings and fragments, that request referrers are origin-only, and that all historical pre-activation transmission remains UNVERIFIABLE. In the implementation PR, its evidence cell cites `components/analytics/web-analytics.tsx`, its tests, the root layout, the referrer-policy tests, and the project-toggle REST measurement, then marks production browser-network and path-grouped reporting evidence **PENDING**. The docs-only closeout replaces that pending marker with the exact promotion, browser-network, and official CLI reporting evidence required by Step 6. Do not claim that zero events were ever collected.
...
6. In `next.config.test.ts`, first make the response-header assertion fail on the old value, then require the exact global `Referrer-Policy` value `origin`. Extend `tests/e2e/legal-pages.spec.ts` with a browser-level regression: navigate to `/pricing?session_id=debt-464-referrer-sentinel`, issue a same-origin `/api/health` fetch from that document, capture the request with Playwright, and require its `referer` header to equal only the page origin with a trailing slash. Assert that neither `/pricing` nor the sentinel appears. This stable browser test proves the global header contract for every same-origin fetch without depending on a provider intake path that may be generated per deployment. Item 6 of Step 6 separately verifies the real production Analytics script and intake requests.
...
New:
`- Vercel Web Analytics was re-measured on 2026-08-10. The project toggle was enabled on 2026-02-24, while the measured application baseline and signed-out production HTML had no analytics integration or embedded script. The earlier unnamed 404 probe did not prove that no query API or dataset existed. Vercel now documents Web Analytics API and CLI queries; the official CLI returned no page-view groups for the observable 31-day Hobby window, but could not query back to the toggle date. Historical transmission remains UNVERIFIABLE. DEBT-464 resolves the current integration and notice gap with four-route collection, query-stripped event URLs, origin-only request referrers, and atomic Privacy Policy updates.`
...
6. In a real browser with blockers disabled, load two allowed production pages and capture the analytics script and intake requests. Account for version 2's generated resilient-intake paths rather than requiring only `/_vercel/insights`. For every captured Analytics request, require the `Referer` header to equal only the production origin, with no source path or query. Confirm separately that the event payload contains the allowed path but no query or fragment. A direct 200 from an asset URL is not evidence of page use.
...
- The global `Referrer-Policy` changes from `strict-origin-when-cross-origin` to `origin`. Same-origin and outbound requests retain origin-level attribution but no longer disclose the source path or query through the `Referer` header. Accepted as the transport-level complement to event-payload redaction.
