...
- Stripe webhook processing calls the same repository at [stripe-webhook-controller.ts#L136-L141](../../src/adapters/controllers/stripe-webhook-controller.ts#L136), but a statement-level Postgres failure aborts the shared transaction. The catch then attempts `stripeEvents.markFailed` inside that aborted transaction ([line 148](../../src/adapters/controllers/stripe-webhook-controller.ts#L148)); BUG-285 establishes that this write fails with `25P02` and **no failure row commits**. The original draft's claim that this scenario durably stored a generic `stripe_events.error` was false. The cause-dropping wrapper still weakens the stable error object, while BUG-285 separately owns the missing Stripe failure record/top-level error replacement.
