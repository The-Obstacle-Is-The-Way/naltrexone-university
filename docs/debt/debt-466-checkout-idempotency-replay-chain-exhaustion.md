...
**Scope:** `createStripeCheckoutSession` and `createStripeTrialPaymentMethodSetupSession` in `src/adapters/gateways/stripe/stripe-checkout-sessions.ts` currently share `CHECKOUT_SESSION_RECOVERY_ATTEMPT_LIMIT = 3` (line 25 — no comment or documented rationale). The subscription exhaustion test derives its call count from the constant; the setup exhaustion test hardcodes four calls and recovery attempts 1, 2, and 3. The settled design splits those meanings: subscription replay traversal gets a limit of 10; setup defective-response replacement keeps its own limit of 3. A live setup-mode probe proved that setup replay is a distinct stale-URL defect, not this replay-chain exhaustion mechanism.
...
**Part B — write the missing vendor knowledge down.** Add Stripe's saved-response semantics and at-least-24-hour key retention to `docs/vendor-docs/stripe.md` (currently contains no idempotency content), and a short note in `docs/dev/testing-infrastructure.md` that completed `trial-start` Sessions remain chain rungs while their keys are retained — with the local volume-persistence fact (`pnpm test:e2e` reuses the per-clone database; only `pnpm db:test:reset` drops its volume). ADR-015 §4 is reconciled by its dated 2026-08-14 amendment: deterministic subscription Checkout keys are a narrow, conditioned exception; the setup path is not licensed as precedent while it lacks live post-create retrieval.
...
- [ ] Subscription deep-chain red test (6 retained replays → fresh) and exact limit-relative success/exhaustion boundaries are green after the rename/raise
- [ ] Constants are split: subscription traversal = 10; setup defective-response replacement = 3 with limit-relative setup coverage and no live-retrieval behavior change
- [ ] Traversal cap has the measured WHY comment; low-depth warns and depth ≥ 5 logs at `error`; no code or doc calls the cap a hard request-timeout bound
...
- [ ] `docs/vendor-docs/stripe.md` records saved-response and at-least-24-hour retention semantics; `docs/dev/testing-infrastructure.md` notes the volume-persistence + retained-chain interaction; implementation remains within ADR-015 §4's dated exception
