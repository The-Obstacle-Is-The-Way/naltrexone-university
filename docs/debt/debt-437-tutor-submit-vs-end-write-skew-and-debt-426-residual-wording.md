...
2. **Owner ruling required:** still open. The 2026-07-09 read-path audit above found no persistent score/grade misrendering, so accepting the anomaly is defensible. If the owner wants the stricter timestamp invariant, implement END-WINS or ANSWER-WINS with red-first concurrency coverage rather than changing this as a drive-by cleanup.
...
- If ACCEPT is chosen: archive this doc as accepted with the 2026-07-09 read-path evidence table intact and no production code change.
- If hardening is chosen: a red-first integration test where a tutor submit's transaction spans a concurrent `end()` commit must prove the chosen policy. END-WINS should never persist `attempts.answered_at > practice_sessions.ended_at`; ANSWER-WINS should prove `end()` waits until the in-flight submit is durably before the end timestamp.
