...
1. The "two separate Stripe accounts" framing in the initial draft was wrong — `acct_1Svkj6KAPxQwR68A` "New business sandbox" in CLI config is stale config from before Stripe's test-mode/sandbox consolidation, not a current second environment. There is exactly **one** test environment.
...
```text
ID:          we_1T19r0KItmaHAwgUrGSpxvdZ
Status:      enabled
...
```text
ID:          we_1SxtpVKItmaHAwgU3SXpQPEB
Status:      enabled
...
# → 1 customer: cus_TvoBcSUN9ZpK3A (jj@novamindnyc.com, the user's own; no live subscriptions)
...
|---|---|---|---|---|
| `sub_1SzJcRKItmaHAwgUvAMv6MLb` | **canceled** | `cus_TxDC4CL6IjuC3f` | 2026-02-10 | Canceled 2026-05-11 — 90.004 days after creation = exact match for Stripe's 90-day sandbox auto-retention pattern. Generated the 6 failed deliveries on 5/10–5/11. |
| `sub_1TWexXKItmaHAwgUKd4owOaY` | **active** | `cus_TxDC4CL6IjuC3f` | **2026-05-13 (today)** | Newly created today on the same e2e-test customer. Currently active. Will generate its own webhook failures on future subscribed `customer.subscription.updated/deleted` state changes until either deleted or our webhook path is made defensive. |

Both subscriptions belong to the same customer (`cus_TxDC4CL6IjuC3f`). Critical detail: **the customer itself has correct metadata** —

```bash
stripe customers retrieve cus_TxDC4CL6IjuC3f
# email: e2e-test@addictionboards.com
# metadata: { clerk_user_id: user_39U3XL5GDco6wjfuIfAssDqS4rs,
#             user_id: 3955ed2e-9034-4943-ad15-04cd79daa568 }
```
...
|---|---|---|---|
| `evt_1TVa2eKItmaHAwgUkpN2kfFh` | `customer.subscription.updated` | `sub_1SzJcRKItmaHAwgUvAMv6MLb` | 2026-05-10 |
| `evt_1TVwbkKItmaHAwgUXWtGyHBR` | `customer.subscription.deleted` | `sub_1SzJcRKItmaHAwgUvAMv6MLb` | 2026-05-11 |
...
# Retrieve a specific endpoint (note: signing secret is NOT returned by retrieve)
stripe webhook_endpoints retrieve we_1T19r0KItmaHAwgUrGSpxvdZ

# List recent events
stripe events list --limit 100

# Retrieve a specific event with full payload (used to confirm empty metadata)
stripe events retrieve evt_1TVa2eKItmaHAwgUkpN2kfFh
stripe events retrieve evt_1TVwbkKItmaHAwgUXWtGyHBR

# After fix is shipped: replay the stuck events to verify they now process cleanly
stripe events resend evt_1TVa2eKItmaHAwgUkpN2kfFh
stripe events resend evt_1TVwbkKItmaHAwgUXWtGyHBR
