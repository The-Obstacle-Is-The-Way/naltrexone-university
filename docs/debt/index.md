...
| [DEBT-400](./debt-400-test-fixture-integrity-zod-boundary.md) | Test fixture integrity at the Zod boundary — same bug class as PR #330 (Zod 4 `z.guid()` preservation). PR 1 (#368) fixed factory/fake generated defaults with a `zUuid.safeParse()` proof harness; PR 2a (#369) fixed controller/shared/job/gateway app-UUID fixtures; PR 2b (#370) fixed repository row fixtures. The remaining candidate sweep is 2,040 placeholder-ID assignments across 129 files, with the PR 3 app/browser/application slice split into PR 3a/3b/3c and Tier 3 fake-test files intentionally left for readability. PR 4 creates `.claude/rules/fixture-integrity.md` as the SSOT with a `testing.md` pointer. | P2 | — |
