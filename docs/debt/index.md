...
**Last Updated:** 2026-05-04 (DEBT-378/379 audit-refined)
...
| [DEBT-378](./debt-378-tutor-drop-submit-button-choice-click-commits.md) | Drop Submit button entirely from non-exam learning flows — choice click commits the answer immediately in tutor sessions and Quick Practice by passing the clicked `choiceId` explicitly into the existing submit path. `useQuestionFlowCore` stays mode-agnostic and returns whether selection changed; wrapper hooks own commit branching (`sessionMode === 'exam'` remains select-only). Tutor footer becomes pre-feedback empty/Previous-only, post-feedback `[Previous][Next/End session]` left cluster + `[Bookmark]` `sm:ml-auto` right; header `End session` persists, creating an intentional Q3 duplicate. Significant refactor: ~150-250 production LOC, ~70-120 test assertions across 12-18 files, Pattern Registry I-3 gains mode-dependent semantic note. Doc-first; audit-refined and implementation-ready pending user grade | P2 | — |
