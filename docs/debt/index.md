...
| [DEBT-310](./debt-310-stripe-stale-price-id-in-production-db.md) | Production Neon `main` contains non-production Stripe subscription rows (one manual old-price seed, one E2E test subscription); polluted data crashes `/app/dashboard` with `Unknown Stripe price id` | P1 | — |
