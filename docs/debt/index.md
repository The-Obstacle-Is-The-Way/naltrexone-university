...
| [DEBT-398](./debt-398-design-system-enforcement-gap.md) | Design system enforcement gap (meta-debt) — `docs/frontend/standards.md`, `pattern-registry.md`, `contrast-policy.md` exist but agents do not auto-load them (only `CLAUDE.md` / `AGENTS.md` / `.claude/rules/*` are auto-loaded). `.claude/rules/frontend.md` is 48 lines and does not cross-reference the design docs. No lint rule blocks arbitrary opacity values (`[0.06]`). Three single-concern PRs: expand `.claude/rules/frontend.md` into a Canonical UI Patterns gateway pointing at all design docs; add a "For AI Agents" preamble to `standards.md`; install enforcement (lint rule OR expanded `theme-token-regression.test.tsx`). This is the root cause that DEBT-399 visibly addresses. | P1 | — |
