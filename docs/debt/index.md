...
**Last Updated:** 2026-05-04 (DEBT-378 rescoped to click-to-commit; DEBT-379 opened)
...
| [DEBT-378](./debt-378-tutor-drop-submit-button-choice-click-commits.md) | Drop Submit button entirely from tutor mode — choice click commits the answer immediately, footer collapses to `[Previous][Next/End session]` left cluster + `[Bookmark]` `sm:ml-auto` right (mirroring exam shape), header `End session` persists across all questions creating an intentional same-label duplicate on Q3 (per DEBT-372 precedent), pre-feedback Next removed (question navigator pills carry skip semantic). Significant refactor: ~150-250 production LOC touched, ~50-80 test assertions across 7-9 files, Pattern Registry I-3 gains mode-dependent semantic note, click-to-commit aligns with learning-app UX literature. Doc-first; awaiting audit | P2 | — |
| [DEBT-379](./debt-379-exam-action-bar-promote-primary-cta-to-right-slot.md) | Exam mode action bar promotes primary CTA (Next / Review & Submit) to footer right slot via `sm:ml-auto`, repositioning Mark for review to the exam header rail (parallels tutor's header `End session` slot which is currently empty in exam mode after DEBT-363). Resolves the convention-incorrect "metadata at right edge, primary inside left cluster" inversion. Three options weighed (header rail recommended, three-slot footer as safe-default, icon-on-card rejected for scope). Smaller refactor than DEBT-378: ~80-120 production LOC, ~20-40 test assertions across 4-5 files. Sequenced after DEBT-378 to keep one debt per shipping cycle. Doc-first; awaiting audit | P3 | — |
