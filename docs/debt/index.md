...
**Last Updated:** 2026-05-14 (DEBT-384 T2/T3 code-fixable slice implemented in PR #310; DEBT-385 filed for invoice schema drift)
...
| [DEBT-384](./debt-384-stripe-webhook-error-rate-investigation.md) | Stripe webhook delivery error-rate investigation — H3 confirmed empirically via Stripe CLI + independent adversarial audit. T2/T3 code-fixable slice implemented in [PR #310](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/310). T2 source found and fixed in `tests/e2e/helpers/seed-test-user.ts`: E2E direct Stripe subscription creation now includes `metadata.user_id`, reused active E2E subscriptions missing metadata are repaired, and E2E customer creation includes `{ user_id, clerk_user_id }`. T3 implemented as webhook-controller-only hardening: missing subscription metadata now returns 200 with structured `metadata_missing` warning and no `stripe_events` claim, keyed by typed `ApplicationError.fieldErrors` rather than message text; unrelated Stripe errors still propagate. Reconcile remains fail-closed, verified by test. T1 remains pending user/ops action: delete `sub_E2E_CANCELED_MISSING_METADATA` and `sub_E2E_ACTIVE_MISSING_METADATA` from Stripe Dashboard. Endpoint config gap remains: add `customer.subscription.created` to test/live endpoints. Invoice schema drift moved to DEBT-385. | P2 | — |
