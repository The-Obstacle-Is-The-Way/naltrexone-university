...
**Last Updated:** 2026-05-16 (DEBT-386 shipped via PR #311 — explicit `E2E_STRIPE_OWNER` owner scoping for E2E seeded customers/subscriptions, `STRIPE_WEBHOOK_E2E_OWNER` owner-mismatch skip at the webhook boundary only, reconcile remains fail-closed; doc archived. Post-merge Stripe Dashboard ops `customer.subscription.created` endpoint config and `STRIPE_WEBHOOK_E2E_OWNER=vercel-dev-preview` dev-preview env var still pending operator action.)
