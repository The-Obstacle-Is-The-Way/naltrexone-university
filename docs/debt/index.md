...
**Last Updated:** 2026-05-19 (DEBT-386 post-merge ops complete — `STRIPE_WEBHOOK_E2E_OWNER=vercel-dev-preview` is applied to the branch-scoped Vercel Preview `(dev)` deployment and redeployed; both test-mode and live Stripe webhook endpoints now include `customer.subscription.created`; live mode has zero undelivered events; remaining test-mode undelivered events are historical pre-fix retries. DEBT-385 remains active.)
