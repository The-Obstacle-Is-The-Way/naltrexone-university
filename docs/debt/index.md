...
**Last Updated:** 2026-05-26 (DEBT-394 resolved across three PRs and archived; `pnpm-workspace.yaml` now ships `minimumReleaseAge: 10080` + v11-only strictness, curated `allowBuilds`, `blockExoticSubdeps`, and `trustPolicy: no-downgrade`; `packageManager` pinned to `pnpm@11.3.0`; `docs/dev/supply-chain-overrides.md` documents legitimate-bypass paths. A follow-up cleanup PR is required after 2026-06-02 to remove the ~70 time-bounded `minimumReleaseAgeExclude` bootstrap exceptions. **DEBT-395 through DEBT-400 added** from the deep design-system + test-hygiene excavation: test env isolation hardening, brittle CSS selector remediation, datetime boundary type normalization, design system enforcement gap, component-system bypass cleanup, and test fixture integrity at the Zod boundary.)
...
| [DEBT-398](./debt-398-design-system-enforcement-gap.md) | Design system enforcement gap (meta-debt) — `docs/frontend/standards.md`, `pattern-registry.md`, `contrast-policy.md` exist but agents do not auto-load them (only `CLAUDE.md` / `AGENTS.md` / `.claude/rules/*` are auto-loaded). `.claude/rules/frontend.md` is 58 lines and does not cross-reference the design docs. No automated gate distinguishes documented foreground-ramp opacity values from undocumented one-offs. Three single-concern PRs: expand `.claude/rules/frontend.md` into a Canonical UI Patterns gateway pointing at all design docs; add a "For AI Agents" preamble to `standards.md`; install allowlist-driven enforcement (expanded `theme-token-regression.test.tsx`, with optional lint/script gate if verified). This is the root cause that DEBT-399 visibly addresses. | P1 | — |
| [DEBT-399](./debt-399-component-system-bypass-cleanup.md) | Component-system bypass cleanup — concrete violations of `standards.md` § 2 ("all interactive click targets MUST use `<Button>`") and § 3 (one canonical focus ring). Two production raw `<button>` bypasses (`session-breakdown-list.tsx:39-49`, `exam-review-view.tsx`) plus one history disclosure toggle that must migrate or receive a dedicated registry exception; 26 exact inline copies of the canonical focus-ring string across 19 files with no shared extraction (33 broader related instances). Four single-concern PRs: extract focus-ring as Tailwind utility or shared constant, migrate the 26 exact inline sites, resolve the raw button/disclosure sites, and align two dark-mode opacity divergences. | P2 | — |
