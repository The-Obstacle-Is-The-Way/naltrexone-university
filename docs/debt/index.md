...
| [DEBT-309](./debt-309-filter-chip-hover-border-affordance.md) | FilterChip Hover Affordance — add `hover:border-foreground/70` (both modes) and bump fill hover from `[0.10]` to `[0.12]` (+5pp delta); current hover is only a 3pp fill bump with no border change, barely perceptible | P3 | — |
