...
| [DEBT-397](./debt-397-datetime-boundary-type-normalization.md) | Datetime boundary type normalization — `src/adapters/controllers/practice-schemas.ts` uses ISO strings for some controller action-output datetimes (`endedAt`, `startedAt`) and Date objects for adjacent action outputs (`latestAnsweredAt`, `draftSavedAt`). This cross-output boundary drift forces consumers to remember per-action shapes. Three single-concern PRs: ADR choosing canonical representation (recommend all-ISO-string), migrate `SaveExamDraftAnswerOutputSchema` and boundary conversion, add a regression test enforcing the canonical type. | P2 | — |
