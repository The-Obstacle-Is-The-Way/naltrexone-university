...
**Last Updated:** 2026-05-25 (DEBT-393 resolved and archived after Dependabot triage, config hardening, the dependency-update protocol, and the clean 10-package npm group merge; DEBT-394 remains active for pnpm maturity gates, `strictDepBuilds` / `allowBuilds`, `blockExoticSubdeps`, `trustPolicy`, and the follow-on pnpm 11 migration.)
...
| [DEBT-394](./debt-394-supply-chain-hardening.md) | Supply-chain hardening via pnpm maturity gates and strict build policy — current `pnpm@10.33.4` already supports `minimumReleaseAge`, `blockExoticSubdeps`, `trustPolicy: no-downgrade`, `strictDepBuilds`, and `allowBuilds`, but none are configured. Three single-concern PRs: add pnpm-10-compatible maturity/trust policy, enumerate native-build `allowBuilds` and enable `strictDepBuilds`, then migrate to current pnpm 11.x and add v11-only strict missing-time settings while keeping Dependabot `cooldown.default-days: 7` matched to `minimumReleaseAge: 10080`. | P2 | — |
