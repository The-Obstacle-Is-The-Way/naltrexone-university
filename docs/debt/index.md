...
| [DEBT-400](./debt-400-test-fixture-integrity-zod-boundary.md) | Test fixture integrity at the Zod boundary — same bug class as PR #330 (Zod 4 `z.guid()` preservation). A broad grep finds 604 placeholder-ID assignments across 64 files (`q_1`, `session_1`, `attempt_1`, `user_1`); not every hit crosses a controller schema, but many fields either cross `zUuid = z.guid()` boundaries or model database UUID columns. Four single-concern PRs: classify/sweep fixture files and replace validated-boundary IDs with `crypto.randomUUID()`, audit factories/fakes for UUID rigor, document the rule in `.claude/rules/testing.md`, and add a dedicated `.claude/rules/fixture-integrity.md` rule file. | P2 | — |

**Next Debt ID:** DEBT-402
