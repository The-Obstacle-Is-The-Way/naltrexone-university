...
**Last Updated:** 2026-05-13 (DEBT-383 corrected after live billing diagnostics; DEBT-382 paused)
...
| [DEBT-382](./debt-382-landing-page-content-refresh-question-count-and-author-credibility.md) | Landing page content refresh — replace stale `500+ Board-Style Questions` stat (`components/marketing/marketing-home.tsx:19`) with `900+` (backed by 948 `.mdx` source files in `content/questions/imported/`), update `2 Study Modes` to `3 Study Modes` to count Tutor, Exam, and Quick Practice, and add a single restrained unnamed author-credibility line directly under the hero badge: _"Authored by a practicing psychiatrist — double board-certified in Addiction Psychiatry and General Psychiatry. Grounded in primary literature with citations."_ Author name deliberately not surfaced in this iteration (deferred pending user's potential legal first-name change). Three text edits, no layout changes; the literature/citations claim is backed by the imported source pathways and existing detailed-rationales/references copy. Visual redesign, A/B testing, SEO meta updates, named-variant upgrade, and optional AJA publication copy are out of scope. **Paused pending DEBT-383 resolution.** | P3 | — |
| [DEBT-383](./debt-383-canceled-subscription-recovery-trap.md) | Canceled subscription recovery trap — live diagnostics disproved the original state-drift theory. The affected test-mode account's DB row and Stripe subscription both say `canceled` (`sub_1SxwUEKItmaHAwgUESbx3reB`, canceled 2026-05-07T21:50:59Z); Stripe Dashboard marks the event `Source=Automatic`, the cancellation landed 90.02 days after creation, and Stripe documents this as the test/sandbox subscription retention policy (`customer.subscription.deleted`, older API versions report `cancellation_details.reason='cancellation_requested'`). The product bug is recovery UX plus checkout policy drift: `canceled` is correctly non-entitled, but it falls into `manage_billing`, `/pricing` hides normal plan CTAs behind a "Subscription needs attention" loop, and `CreateCheckoutSessionUseCase` still blocks terminal canceled rows with future `currentPeriodEnd` even though the Stripe adapter allows checkout when Stripe subscriptions are only ended/canceled. Recommended fix: split canceled recovery from payment-management recovery, show cancellation-specific copy plus the normal plan grid, and align the local checkout guard with Stripe's blocking-status list so terminal rows can re-subscribe without DB edits. Dogfood unblock: create a real active Stripe test subscription and exclude it from auto-cancellation. **Paused ahead of DEBT-382 until the billing trap is fixed or explicitly deferred.** | P1 | — |
