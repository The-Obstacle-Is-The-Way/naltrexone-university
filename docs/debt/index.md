...
**Last Updated:** 2026-05-18 (DEBT-386 post-merge ops verified — `STRIPE_WEBHOOK_E2E_OWNER=vercel-dev-preview` is applied to the branch-scoped Vercel Preview `(dev)` deployment and redeployed; the test-mode Stripe webhook endpoint now includes `customer.subscription.created`; the live webhook endpoint still needs `customer.subscription.created` added by an operator with live webhook-endpoint write permission. DEBT-385 remains active.)
