...
| [DEBT-402](./debt-402-mocked-dto-type-assertion-drift.md) | Mocked DTO type assertion drift — three current app tests use `as unknown as ...Output` to force impossible mocked controller/use-case DTO shapes through TypeScript (`GetPracticeSessionReviewOutput` sentinels and an invalid `GetTagsOutput` tag kind). Replace with valid minimal fixture builders or move invalid-shape coverage to the real boundary layer. | P3 | — |
