...
| [DEBT-396](./debt-396-brittle-css-selector-remediation.md) | Brittle CSS selector remediation — same bug class as PR #328 (jsdom 26→29 selector tightening). Two unfixed sites (`app/(app)/app/dashboard/page.test.tsx:355`, `app/(app)/app/questions/[slug]/question-page-client.test.tsx:220`) use `querySelector('a[href="...?tab=..."]')` for query-string hrefs and should be migrated before the next selector-engine/jsdom churn. Two single-concern PRs: extract `findAnchorByHref()` to a shared `tests/shared/dom-helpers.ts` and migrate every query-string/special-character attribute-selector site; document the rule in `.claude/rules/testing-react19.md` with WHY (selector engine fragility) plus citation of PR #328. | P2 | — |
