...
| [DEBT-395](./debt-395-test-environment-isolation-hardening.md) | Test environment isolation hardening — same bug class as PR #342. High-confidence misses are `lib/container.test.ts:49-57` module-scope env defaults, `proxy.test.ts` `vi.stubEnv()` calls (`119/585/633/663/686/710`) without suite-level `vi.unstubAllEnvs()`, and `tests/shared/load-dotenv-file.test.ts` delete-only cleanup for `TEST_ENV_LOADED`; a repo-wide env-mutation sweep must classify already-clean sites instead of blindly churning them. Helper `snapshotProcessEnv`/`restoreProcessEnv` exists in `tests/shared/process-env.ts` but is undocumented. Three single-concern PRs: fix the leak sites, document the pattern in `.claude/rules/testing.md`, and add a dedicated `.claude/rules/test-isolation.md` rule file. | P2 | — |
...
| [DEBT-401](./debt-401-auth-nav-test-module-cache-order-dependency.md) | AuthNav test module-cache order dependency — `components/auth-nav.test.tsx` passes in declaration order but fails under `pnpm test --run components/auth-nav.test.tsx --sequence.shuffle --sequence.seed=1779972928761` because a module-scope `MarketingLayout` import indirectly imports `AuthNav` before per-test `vi.doMock('./auth-user-button')`; the suite currently relies on an earlier test's `afterEach` `vi.resetModules()` to make later mocks effective. Fix the test import timing/cleanup so shuffled unit runs can be used as a clean DEBT-395 verification signal. | P2 | — |
