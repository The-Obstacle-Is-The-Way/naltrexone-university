...
| [DEBT-395](./debt-395-test-environment-isolation-hardening.md) | Test environment isolation hardening — same bug class as PR #342. High-confidence misses are `lib/container.test.ts:49-57` module-scope env defaults and `proxy.test.ts` `vi.stubEnv()` calls (`119/585/633/663/686/710`) without suite-level `vi.unstubAllEnvs()`; a repo-wide env-mutation sweep must classify already-clean sites instead of blindly churning them. Helper `snapshotProcessEnv`/`restoreProcessEnv` exists in `tests/shared/process-env.ts` but is undocumented. Three single-concern PRs: fix the leak sites, document the pattern in `.claude/rules/testing.md`, and add a dedicated `.claude/rules/test-isolation.md` rule file. | P2 | — |
