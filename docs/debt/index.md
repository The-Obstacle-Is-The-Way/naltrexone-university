...
| [DEBT-395](./debt-395-test-environment-isolation-hardening.md) | Test environment isolation hardening — same bug class as PR #342. Eight+ test sites (`lib/container.test.ts:49-57`, `proxy.test.ts:119/585/633/686/710`) mutate `process.env` or call `vi.stubEnv()` without `vi.unstubAllEnvs()` / snapshot-restore cleanup, causing order-dependent failures. Helper `snapshotProcessEnv`/`restoreProcessEnv` exists in `tests/shared/process-env.ts` but is undocumented. Three single-concern PRs: fix the leak sites, document the pattern in `.claude/rules/testing.md`, and add a dedicated `.claude/rules/test-isolation.md` rule file. | P2 | — |
| [DEBT-396](./debt-396-brittle-css-selector-remediation.md) | Brittle CSS selector remediation — same bug class as PR #328 (jsdom 26→29 selector tightening). Two unfixed sites (`app/(app)/app/dashboard/page.test.tsx:355`, `app/(app)/app/questions/[slug]/question-page-client.test.tsx:220`) use `querySelector('a[href="...?tab=..."]')` and will break on the next jsdom major. Two single-concern PRs: extract `findAnchorByHref()` to a shared `tests/shared/dom-helpers.ts` and migrate every site; document the rule in `.claude/rules/testing-react19.md` with WHY (selector engine fragility) plus citation of PR #328. | P2 | — |
| [DEBT-397](./debt-397-datetime-boundary-type-normalization.md) | Datetime boundary type normalization — `src/adapters/controllers/practice-schemas.ts` mixes representations inside a single output schema: `endedAt`/`startedAt` are `z.string().datetime()` while `latestAnsweredAt`/`draftSavedAt` are `z.date().nullable()`. Silent data-corruption risk for any consumer that calls Date methods on the now-ISO-string fields. Three single-concern PRs: ADR choosing canonical representation (recommend all-ISO-string), migrate the mixed schemas, add a regression test enforcing the canonical type. | P2 | — |
...
| [DEBT-400](./debt-400-test-fixture-integrity-zod-boundary.md) | Test fixture integrity at the Zod boundary — same bug class as PR #350 (Zod 4 fixture cleanup). 15+ test sites use stringly-typed placeholder IDs (`q_1`, `session_1`, `attempt_1`, `user_1`) that would fail production `z.uuid()`/`z.guid()` validation but pass today only because unit tests mock the boundary layer. Four single-concern PRs: sweep fixture files and replace with `crypto.randomUUID()`, audit factories/fakes for UUID rigor, document the rule in `.claude/rules/testing.md`, and add a dedicated `.claude/rules/fixture-integrity.md` rule file. | P2 | — |
