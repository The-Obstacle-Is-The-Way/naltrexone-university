...
| [DEBT-399](./debt-399-component-system-bypass-cleanup.md) | Component-system bypass cleanup — concrete violations of `standards.md` § 2 ("all interactive click targets MUST use `<Button>`") and § 3 (one canonical focus ring). Three production raw `<button>` sites (`session-breakdown-list.tsx:39-49`, `history-sessions-tab.tsx` disclosure, `exam-review-view.tsx`) plus 42 inline copies of the canonical focus-ring string with no shared extraction. Four single-concern PRs: extract focus-ring as Tailwind utility or shared constant, migrate the 42 inline sites, refactor the 3 raw buttons to use `<Button>`, and align two dark-mode opacity divergences. | P2 | — |
