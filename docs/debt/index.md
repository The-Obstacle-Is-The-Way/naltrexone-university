...
| [DEBT-403](./debt-403-biome-245-lint-conformance.md) | Biome 2.4.15 lint-rule conformance — the standalone Biome Dependabot bump (#385) goes red because `recommended: true` is not version-pinned. Accept the rule shift (DEBT-393 §C): deterministic export-sort autofixes (10 files) + one manual `noArrayIndexKey` fix + `$schema` bump, landed as a source-only PR so #385 stays a single-purpose lockfile bump. | P3 | — |
