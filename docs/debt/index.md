...
**Last Updated:** 2026-05-21 (DEBT-389 resolved and archived — PR #315 replaced the footer's unbalanced 3-column grid with a brand-left / links-right flex layout: the four links consolidate into a single right-anchored inline row and the unearned `Product`/`Account` labels are dropped, with copy, hrefs, and the brand `footer p` treatment preserved. main and dev synced at `a4df8991`. DEBT-388 (hero credibility line reposition) remains the last open landing-page polish item.)
