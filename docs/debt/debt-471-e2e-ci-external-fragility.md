...
- F1: required PR E2E performs no selector-based action or assertion against Stripe-owned DOM after navigation to a Stripe-owned origin; an origin-only URL assertion may remain to prove the application redirected to Stripe. An explicit application-owned selector allowlist covers any remaining `getByRole`/`getByLabel` calls; the Stripe-owned strings in both `paid-checkout.spec.ts` and `completeNoCardTrialCheckout()` are absent from the required lane. Repository-owned contracts still prove both Session variants' creation/redirect, success sync, persistence, cardless-trial state, and entitlement, including `expectE2EUserHasPaidAnnualSubscription` and `expectE2EUserHasTrialWithoutPaymentMethod`. The named coverage and testing docs record the reduced hosted-UI claim. Any retained hosted smoke is outside required PR checks.
...
- F6: every networked Stripe helper is assigned to a documented required or scheduled lane with a bounded failure contract; no test-mode API call is misreported as hermetic.
...
- Whole-debt regression check: contract tests simulate the hosted-selector absence, a hung dependency installer, and a flaky-then-passing reporter run. Repeated unchanged-head greens are incident receipts, not proof that external dependencies are insulated.
