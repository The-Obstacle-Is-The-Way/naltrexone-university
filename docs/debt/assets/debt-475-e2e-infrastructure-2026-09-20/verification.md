...
| A separately detached web server cannot survive interruption | A group-only implementation failed both signals: descendant remained alive | Snapshot active-command descendants before signalling, terminate their owned groups, retain two-second escalation after early parent exit |
