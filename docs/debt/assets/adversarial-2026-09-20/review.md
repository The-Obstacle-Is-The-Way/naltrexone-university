...
The main/dev reconciliation also brought five existing P1 content records into this register. They were not part of the original twelve-step ordering and must not be hidden behind optional SEO work:

1. Before the next content import, address [DEBT-487](../../../_archive/debt/debt-487-draft-splitter-silent-omission.md) complete block consumption and [DEBT-482](../../../_archive/debt/debt-482-duplicate-qid-output-collision.md) duplicate identity/destination rejection, each in its own PR. Discover every candidate before reasoning about uniqueness; neither requires a new release interface. Their records contain the reordered-block and duplicate-QID receipts. The measured local corpus has 948 distinct identities; no production omission or collision is established.
2. Resolve [DEBT-486](../../debt-486-import-seed-validation-disagreement.md) import/seed validation disagreement before relying on whole-bundle prevalidation. Its empty-stem/Reference fixtures define the gap. This is independent of optional browser telemetry.
3. Implement the explicitly separate initial safeguards in [DEBT-483](../../debt-483-content-withdrawal-and-release-rollback.md#smallest-fix) (explicit withdrawal, clean staging, whole-bundle prevalidation) and [DEBT-484](../../debt-484-question-rewrite-history-identity.md#smallest-fix) (reject substantive rewrites over graded history). Both records explicitly state that these safeguards are actionable without content SPEC-007. Full atomic activation/rollback and immutable revision binding do depend on its release interface; a manifest alone closes neither debt. Production impact remains unestablished.
