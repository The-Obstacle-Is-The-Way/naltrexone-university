...
**Landing scope:** PR A (#933) contains only DEBT-479/480/481, the debt index and these two evidence files. Prepared SPEC-016/017 corrections, master-spec warnings and the spec index belong to specs-only PR B after A merges. DEBT-450/468/475 and testing-infrastructure/retry-evidence corrections belong to PR C last, rebased onto the then-current `dev`. Findings and historical validation below do not claim those follow-ups have landed.
...
| SPEC-016 says errors only while server samples 0.05 | CONFIRMED | Old SPEC-016 Current State/Scope vs `instrumentation.ts:21`; correction prepared for PR B. |
...
| LOG_LEVEL is supported but absent from .env.example | CONFIRMED | `lib/logger.ts:4-16`; `rg LOG_LEVEL .env.example` has no match. Example addition is explicitly declined as optional; behavior documentation is prepared for PR B. |
...
| 13 non-test .limit calls across 10 files | OVERSTATED | True specifically for **RateLimiter** calls; unqualified `.limit()` also counts SQL pagination. Six practice operations share one call; total named operations is 18. The prepared PR B table lists every policy/caller. |
...
| This project has the asserted rule allowance/available slot | UNPROVEN | No project dashboard entitlement check was performed. The owner requires that check before using the allowance as an operational premise; The prepared SPEC-017 update marks it unverified; the decision below already does so. |
...
| Coverage upload establishes a numeric coverage gate | REFUTED | All three coverage configs lack thresholds/runtime include scope; Codecov upload uses `fail_ci_if_error: false`. `codecov.yml` does exist, but contains only the E2E ignore. PR C will correct DEBT-468's “no codecov.yml”; its enforcement remains ADR-gated. |
...
| --- | --- | --- |
| `spec-016-observability.md` | CONFIRMED | Sampling/request-context/logger-copy drift; correction prepared for PR B, including true scope and optional LOG_LEVEL disposition. |
| `spec-017-rate-limiting.md` | CONFIRMED | 9-policy table, old bookmark key, unsupported warm-DB claim and ambiguous triggers; table and E1/E2/E3 wording corrections prepared for PR B. The [IETF readback](https://datatracker.ietf.org/doc/draft-ietf-httpapi-ratelimit-headers/) is draft-11, still an Internet-Draft: E4's RFC trigger has not fired. No Redis/WAF runtime work. |
...
| `master_spec_part4.md` | CONFIRMED | CI example uses pnpm 10.9/Node 22 and provider secrets at job scope (lines 145–205), unlike actual SHA-pinned/step-scoped Node 24 CI. “Time spent=0/no timing” at 410 contradicts `finalize-exam-answers.ts:215-258`, `submit-answer.ts:193-208`, and even Part 2's validated timing contract. |
| `index.md` | CONFIRMED | Labels master a complete SSOT and links all five current copies; dated June 29. PR B will identify the reconciliation warning and link DEBT-481; no new spec ID required. |
...
2. **Merge these documentation corrections through normal review.** Merge A (filings/evidence), then B (specs only), then C (register corrections, rebased onto current `dev`). DEBT-479 steps 6/7 are prepared for B and are not complete in A. E1 owner decision is now recorded: structural deferral, currently observable triggers, unapplied incident lever; log-only declined and project allowance unverified. Reconcile current master ownership under **DEBT-481** before copying its old workflow/seed examples. The doc mechanism can proceed now without an ADR for runtime behavior.
...
### Execution-order update after A/B/C (2026-09-20)

The list above preserves the initial audit sequence. [A #933](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/933), [B #936](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/936), and [C #938](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/938) have now merged. DEBT-479 steps 6/7 are complete; the [#932 production closeout](../e2e-retries-2026-09-20/verification.md#production-closeout--2026-09-20-1453z) is also complete. Master warnings remain containment, not DEBT-481 closure. Promotion #940 is blocked by the [confirmed partial signal-cleanup gap](../debt-475-e2e-infrastructure-2026-09-20/signal-error-review.md); fix and review that narrow DEBT-475 mechanism before reopening promotion. Then verify main CI, the actual production alias, HTTP status, and dev/main tree equality.

The main/dev reconciliation also brought five existing P1 content records into this register. They were not part of the original twelve-step ordering. Their later implementation status is recorded in the [content-import reconciliation snapshot](../content-integrity-2026-09-20/verification.md#reconciliation-snapshot):

1. **CONFIRMED — resolved:** [DEBT-487](../../../_archive/debt/debt-487-draft-splitter-silent-omission.md) complete block consumption and [DEBT-482](../../../_archive/debt/debt-482-duplicate-qid-output-collision.md) duplicate identity/destination rejection merged separately in #943/#945 and were production-verified through #946. Their archived records retain the red/green and release receipts. The measured local corpus has 948 distinct identities; no production omission or collision is established.
2. **CONFIRMED — implementation merged:** [DEBT-486](../../../_archive/debt/debt-486-import-seed-validation-disagreement.md) shared import/seed body validation landed in #949 before #952's static whole-input prevalidation. Its record retains the empty-stem/Reference regressions and exact-head approval; only the later release readback remains at the linked snapshot. Neither implementation depends on optional browser telemetry.
3. **CONFIRMED — safeguards merged, broader work open:** [DEBT-483](../../debt-483-content-withdrawal-and-release-rollback.md#smallest-fix) now records #952's static prevalidation, #953's explicit withdrawal/stale-seed refusal and #954's clean write staging; [DEBT-484](../../debt-484-question-rewrite-history-identity.md#smallest-fix) records #951's graded-history rewrite guard. Managed-caller staging remains actionable without content SPEC-007 in its separately assigned runtime file. Full atomic activation/rollback and immutable revision binding still depend on the release interface; archived-question review and active-session limitations remain open. A manifest or preserved database rows alone close neither debt. Production impact remains unestablished.

The helper-recovery correction under DEBT-475 remains the next test-honesty slice after promotion. DEBT-414 remains the parallel P1 operational/compliance stream. The rest of the original dependency order still applies: DEBT-480 before DEBT-479 crawl resources; current server measurements and retained external-event receipts now; owner-approved CLI/expiry mechanisms independently; DEBT-472 fixtures before DEBT-468/469 deletions and DEBT-465 pilots; DEBT-476 only after its release-age date. No new debt IDs or implementation authority are created by this ordering update.
...
Historical validation of the owner-decision follow-up on the original combined tree: docs-only diff; `pnpm typecheck` and `pnpm lint` passed (1,218 files, fidelity issues=0), 532 relative file-link targets resolved, and `git diff --check` passed. No runtime test/policy or provider configuration changed.

## PR A review adjudication — 2026-09-20

- **CONFIRMED:** CodeRabbit review `5260830357`, comment `4057186525`, identified completion claims for SPEC-016/017 although `gh pr diff 933 --name-only` lists only the six filing/evidence files. Corrected all equivalent spec/master-warning/register completion claims to their pending B/C scopes, including the two named locations. Original audit validation is explicitly historical and does not replace each split PR's full pre-push gate.
- **CONFIRMED:** LanguageTool's missing spaces in `pnpm 10.9`, `Node 22`, `Node 24` and `June 29` were readability defects and are corrected. **REFUTED:** capitalizing `.github/workflows` would corrupt an exact repository path; `git ls-files .github/workflows` returns that lowercase directory. **REFUTED:** the claim that “new record(s)” is redundant here; it distinguishes a new filing from an existing debt, as the adjoining DEBT-445/465 receipts require.
- **REFUTED:** OpenGrep's three possible-card-number reports at `sentry-readback.json:7,50,65`. Each value is the Sentry project query parameter. An authenticated `GET https://sentry.io/api/0/projects/novamindnyc/addiction-boards-web/` returned HTTP 200 and `{"id":"4510829539164160","slug":"addiction-boards-web","name":"addiction-boards-web"}` on 2026-09-20. This is a provider project identifier, not payment data. No scanner suppression or receipt redaction is warranted.
