...
1. Before the next content import, address [DEBT-487](../../debt-487-draft-splitter-silent-omission.md) complete block consumption and [DEBT-482](../../debt-482-duplicate-qid-output-collision.md) duplicate identity/destination rejection, each in its own PR. Discover every candidate before reasoning about uniqueness; neither requires a new release interface. Their records contain the reordered-block and duplicate-QID receipts. The measured local corpus has 948 distinct identities; no production omission or collision is established.
