...
**Created:** 2026-06-06
**Audit corrected:** 2026-06-07 (post-PR-1 docs-only audit against current repo citations + Stripe primary sources); 2026-06-09 (post-PR-2 audit against current repo citations and PR-3 seams)
**Status:** **DECIDED specification — PR-1 shipped (`8e2e1489`); PR-2 shipped (`f7463dec`); PR-3→PR-4 remaining.** PR-3 is the UI/UX implementation; PR-4 is config + rollout only.
...
2. **The low-friction path is wired but still not user-facing.** PR-2 shipped the no-card Checkout wiring behind default-off `FREE_TRIAL_ENABLED`, but the visible pricing, checkout-success, app-shell countdown, expired-state copy, and rollout config still do not present a usable trial path. For a pre-launch product whose owner's #1 goal is first users + feedback, "pay first, evaluate later" remains the user-facing adoption blocker until PR-3/PR-4 land. The domain model was already built to support a trial (`inTrial` is a first-class entitled status — §B.1), so the remaining gap is unfinished MVP scope, i.e. debt.
...
3. **Checkout success (`app/(marketing)/checkout/success/`).** Make the copy trial-aware: "Your 7-day free trial has started — no charge today" vs the existing paid confirmation, driven off subscription status. Copy seam: `app/(marketing)/checkout/success/page.tsx:39-53`. Status-fetch/sync seam: `app/(marketing)/checkout/success/checkout-success-sync.tsx:178-258`, backed by types in `app/(marketing)/checkout/success/checkout-success-types.ts:13-25` and assertions in `app/(marketing)/checkout/success/checkout-success-assertions.ts:36-96`.
4. **In-app trial countdown.** A persistent app-shell indicator "N days left in trial" + an "Add a card to keep access" affordance routing to the billing portal (§B.6 path 1). Days remaining = `trialEndsAt − now`, where `trialEndsAt` is sourced from persisted `currentPeriodEnd` while `status === inTrial` (no Stripe round-trip). Register the pattern first in `docs/frontend/pattern-registry.md`, then extend the data chain: `CheckEntitlementOutput` currently exposes `isEntitled`, `reason`, `subscriptionStatus`, and `hasActiveSubscriptionPeriod` but **not** `trialEndsAt`/`currentPeriodEnd` (`src/application/use-cases/check-entitlement.ts:12-17`, `src/application/use-cases/check-entitlement.ts:36-51`); `RequestAuthState` preserves that output (`lib/auth-request-cache.ts:15-23`, `lib/auth-request-cache.ts:48-52`); `enforceEntitledAppUser` currently returns only `subscriptionStatus` (`app/(app)/app/layout.tsx:25-46`) and `renderAppLayout` only uses that status for the past-due banner (`app/(app)/app/layout.tsx:121-144`). PR-3 must surface `trialEndsAt` through this chain before the layout can render the countdown.
...
- **PR-3 — UI/UX:** trial CTAs + `pricing-data` fields, trial-aware checkout-success, app-shell countdown (register pattern first), the entitlement/auth/layout `trialEndsAt` data seam sourced from persisted `currentPeriodEnd`, portal "add a card" affordance via the protected app billing action, and expired-state copy. Red tests first in `app/pricing/page.test.tsx`, `app/pricing/pricing-view.test.tsx`, `app/(marketing)/checkout/success/page.test.ts`, `app/(marketing)/checkout/success/checkout-success-assertions.test.ts` only if assertion behavior changes, `src/application/use-cases/check-entitlement.test.ts`, `lib/auth-request-cache.test.ts`, `app/(app)/app/layout.test.ts`, and `app/(app)/app/layout-shell.test.tsx`; add `*.browser.spec.tsx` only for client-side interactivity that static markup cannot verify. Visual verify light+dark.
...
- [ ] In-app countdown shows correct days remaining from `trialEndsAt`, sourced from persisted `currentPeriodEnd` during `inTrial`, after PR-3 surfaces that value through `CheckEntitlementOutput` → request auth state → app layout.
