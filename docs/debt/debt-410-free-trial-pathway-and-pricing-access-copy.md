...
**Status:** **DECIDED specification — PR-1 shipped (`8e2e1489`); PR-2 shipped (`f7463dec`); PR-3 shipped (`3ba5576a`); PR-4 launch/config shipped.** The remaining tails are split out to DEBT-414 (legal pages + Stripe legal links + descriptor verification), DEBT-415 (flag-on E2E alignment + trial-start spec), and DEBT-413 (post-GA flag removal).
