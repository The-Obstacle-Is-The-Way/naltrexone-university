...
**Correction (2026-06-30, second review pass):** simply deleting the `FOR UPDATE` call is not a safe drop-in fix. The lock also participates in the read-then-write snapshot consistency `findQuestionStateSnapshot` relies on, and removing it without a compensating design change would reopen parent/child race and error-classification issues that overlap with [BUG-267](../_archive/bugs/bug-267-nested-repeatable-read-silently-drops-isolation.md)'s torn-read class of problem. A real fix needs the updater redesigned around either a single joined statement (session-active check + row CAS in one round trip) or a narrower per-row lock that doesn't serialize across questions — not just lock removal. Treat this as a design task, not a quick delete, and sequence it after (or together with) the BUG-267 fix since they touch the same read/lock surface.
