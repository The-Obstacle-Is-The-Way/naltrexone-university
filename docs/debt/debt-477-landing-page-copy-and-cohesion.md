...
**Owner ruling 2026-09-18: sentence case.** The owner chose `Subscribe monthly` / `Subscribe annual`. [PR #912](https://github.com/The-Obstacle-Is-The-Way/naltrexone-university/pull/912) recases the three literals (the two signed-out links in `pricing-view.tsx` and the consent-dialog trigger in `plan-consent-dialog.tsx`), the unit and browser assertions, and the two exact-match E2E selectors (`checkout-redirect.spec.ts`, `stripe-hosted-paid-checkout.spec.ts`). The updated assertions failed against the old labels first (8 of 88 unit tests), then 88 unit and 13 browser tests passed after the change. No `disclosureVersion` bump was needed: no disclosure string or consent snapshot quotes these labels. Frontend Standards, the Pattern Registry, QA-002, and the master spec now describe the sentence-case labels. This was the record's last recorded tail; archive it after the change is promoted and verified on production.
