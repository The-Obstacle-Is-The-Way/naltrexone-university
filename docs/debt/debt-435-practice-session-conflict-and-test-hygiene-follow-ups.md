...
3. **Idempotency caches transient `CONFLICT`s (active).** Session-review/finalize flows use idempotency keys that clear only on success. A transient practice-session state-write `CONFLICT` can therefore be stored and replayed for the same idempotency key instead of letting an immediate retry execute fresh. That may be acceptable, but it is a product semantics decision: are transient state-write conflicts cacheable terminal outcomes, or should this class clear/avoid the idempotency cache?
