...
2. `hover:border-foreground/60` — light-mode border brightens from 45% → 60% on hover (+15pp)
3. `dark:hover:border-foreground/70` — dark-mode border brightens from 40% → 70% on hover (+30pp)

The dark-mode border change directly mirrors the Button `outline` variant's proven hover behavior (`dark:hover:border-foreground/70` on `button.tsx:19`). The light-mode border uses a softer step (60% instead of 70%) because the rest border in light mode (45%) is already more visible than in dark mode (40%) — jumping to 70% would be disproportionate and has no existing precedent in the codebase. The fill bump brings the hover delta in line with I-2 page-background rows (8% → 12%, also +4pp) — the strongest proven foreground-ramp hover in the codebase.
...
| **Border (light)** | `foreground/45` | `foreground/60` | +15pp (primary) |
| **Border (dark)** | `foreground/40` | `foreground/70` | +30pp (primary) |

The light-mode border uses a smaller step than dark mode intentionally. Light-mode borders are already more visible against light backgrounds, so a +15pp lift is proportionally equivalent to the +30pp lift in dark mode. This avoids a "shouty" border in light mode while still providing a clear hover signal.
...
| Hover border vs chip fill (light) | `foreground/60` vs `bg-foreground/[0.12]` | Well above 3:1 | Yes |
...
| File | Change |
|------|--------|
| `components/ui/filter-chip.tsx:28` | Change `hover:bg-foreground/[0.10]` → `hover:bg-foreground/[0.12]`; add `hover:border-foreground/60 dark:hover:border-foreground/70` |

### Tests

| File | Change |
|------|--------|
| `components/ui/filter-chip.test.tsx` | Update `hover:bg-foreground/[0.10]` → `hover:bg-foreground/[0.12]`; assert `hover:bg-foreground/[0.10]` is absent (negative check); add assertions for `hover:border-foreground/60` and `dark:hover:border-foreground/70` |

### Doc updates

| File | Change |
|------|--------|
| `docs/frontend/pattern-registry.md` (I-4 section, line 330) | Update unselected class string with new hover fill and border tokens |
| `docs/frontend/pattern-registry.md` (I-4 rationale, line 347) | Update design rationale to mention hover border affordance |
| `docs/frontend/pattern-registry.md` (hover summary table, line 1343) | Update I-4 hover column to include border tokens and new fill value |
| `docs/frontend/pages/practice.md` (line 151) | Update unselected FilterChip row with new hover tokens |
| `docs/frontend/pages/practice.md` (line 248) | Update unselected hover row to mention border change |
| `docs/frontend/pages/practice.md` (line 277) | Update FilterChip hover entry in any supplementary token reference |
...
| Step | Requirement |
|------|-------------|
| Manual visual QA | Hover over unselected chips on the Practice page in **both light and dark mode**. Confirm the border brightening and fill lift are clearly perceptible without looking excessive. This is a perceptual change — token assertions alone are not sufficient. |
| Automated tests | `pnpm test --run` passes with updated assertions in `filter-chip.test.tsx` |
| Pre-PR gate | `pnpm typecheck && pnpm lint && pnpm test --run && pnpm test:browser && pnpm build` all green |
...
Chip hover:    bg-foreground/[0.12]      border-foreground/60  dark:border-foreground/70
