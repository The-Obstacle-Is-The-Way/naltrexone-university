...
2. **Add the enforcement scan before any migration** (cf. F7), following both `tests/architecture-boundary-source-scan.ts` and `components/theme-token-regression-source-scan.ts`: factory-form `vi.mock` of an own-code path outside `*.browser.spec.tsx` `{ spy: true }`, parsed multiline-safe so the DEBT-368 grep and the format-bending comment can both be retired; `as unknown as` in test files outside an explicit allowlist; and a check that a test does not hand-roll a shape for which a barrel fake or `FakeStripeCheckoutClient` exists. Fail CI on growth; allow the current census as a ratchet floor.
