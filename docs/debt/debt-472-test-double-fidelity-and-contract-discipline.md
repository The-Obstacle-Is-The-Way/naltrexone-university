...
- F6/F7: the scan exists, runs in `pnpm test --run`, and fails on four synthetic cases — an own-code factory-form `vi.mock`, an `as unknown as` outside the allowlist, a hand-rolled object shape for a port that has a barrel fake or `FakeStripeCheckoutClient`, and any growth of a counted pattern above its recorded ratchet floor; the DEBT-368 format comment is deleted; the scan parses the multiline `{ spy: true }` form.
