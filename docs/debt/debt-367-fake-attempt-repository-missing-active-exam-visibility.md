...
That decision was correct *for the bug fix*, but it leaves the gap load-bearing now that three production fixes depend on the predicate. Three guarantees the policy depends on are NOT covered by unit-tests-against-the-fake:
...
3. Apply the helper in the eight methods listed above. For `listAttemptedQuestionsByUserId` / `countAttemptedQuestionsByUserId`, apply the filter at the *first* step of `getFilteredAttemptedCandidates` (before the `mostRecentByQuestionId` ranking pass) so the fake mirrors BUG-235's rank-then-filter ordering.
...
5. Add a focused unit test (`fake-attempt-repository.test.ts`, create if missing) covering each of the eight methods with a mixed-visibility seed: one active-exam attempt, one ended-exam attempt, one tutor attempt, one standalone attempt. Assert the active-exam row is hidden in every case.
...
This is not a user-visible defect *today* — production code is correct and integration coverage is in place. But it is the single most likely vector for a silent regression of the BUG-235/236/237 trilogy: a future change in the real repo that drops the predicate would pass every unit test and could ship before integration runs catch it. Closing the gap restores symmetry between the unit-test contract and the production contract.

## Verification

- All eight target methods filter active-exam rows when seeded with `sessionMode: 'exam', sessionEndedAt: null`.
- All eight target methods include the row when `sessionEndedAt` is set (ended exam) or `sessionMode: 'tutor' | null` or `practiceSessionId: null`.
