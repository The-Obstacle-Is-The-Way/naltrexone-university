...
| **CAN-SPAM, 15 U.S.C. § 7704; 16 C.F.R. Part 316** | **APPLIES ON NAMED TRIGGER:** each commercial email; mixed messages are classified per message by primary purpose. | The debt's **CONDITIONAL AND OPEN** framing is correct. The committed Resend templates were classified transactional by the 2026-08-10 owner ruling and must remain free of marketing content. The provider-owned Clerk and Stripe templates are not in the repository and remain open for per-message classification. Do not claim a blanket transactional exemption. Inventory every live template and apply 16 C.F.R. § 316.3's subject-line and message-body primary-purpose test. Any commercial message needs the statutory disclosures, opt-out, and valid physical postal address. |
...
- [ ] Inventory the exact live Clerk and Stripe email templates and record each per-message primary-purpose classification. The committed Resend acknowledgment/reminder/notice templates were classified transactional by the 2026-08-10 owner ruling and must remain free of marketing content. Add a valid postal address and opt-out to every commercial template.
...
**Activation gate:** do not configure `RESEND_API_KEY` until the deployed head contains the H1–H8 resolutions above; a local or PR-only green run is not activation evidence. Independently, do not activate the trial add-card flow until the deployed environment has `CONSENT_STATE_SECRET`. Neither secret is added by this repository change. `RESEND_API_KEY` activation also remains blocked on the owner-run Resend account/domain setup. The 2026-08-10 owner ruling clears the committed-template CAN-SPAM classification gate; the provider-owned Clerk and Stripe inventory remains OPEN but does not block transactional Resend sending under issue #772. A real outbound Resend delivery remains an activation-time owner check.
