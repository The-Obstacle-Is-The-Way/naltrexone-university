...
**Sandbox/test-mode precondition — MEASURED 2026-08-05, do not re-derive from docs.** Stripe documents Public details as *excluded* from the sandbox copy and never synchronized, and this account's test-mode `business_profile` is in fact empty (`url` and `support_email` both `null`) while live is populated — so the reasonable inference was that test mode needs its own Terms URL before `consent_collection` works. **That inference is wrong.** A direct probe against this account's test mode (`acct_1SvkizKItmaHAwgU`, `sk_test_` key from `.env.local`, the same key the E2E suite uses) created a **`mode: 'setup'` Session carrying `consent_collection[terms_of_service]=required` successfully** — returned Session `cs_test_…` with `consent.terms_of_service = 'required'` — despite the sandbox's Public details page being empty and offering no edit affordance. Stripe either injects placeholder business data into sandboxes ("Stripe provides placeholder business data to help you start testing most Stripe features") or does not enforce the check outside live mode.

Two consequences for the build:

1. **No sandbox configuration is required** for Stage 2 to be exercised in test mode or preview. If a *fresh blank* sandbox is ever created, re-run the probe before assuming it inherits this behaviour.
2. **`consent_collection` is confirmed compatible with `mode: 'setup'`.** Every Stripe doc example uses `payment` or `subscription` mode, and the setup-mode flow specified below depends on the combination; it is now verified against the real API rather than assumed.
