...
**Branch baseline:** re-audited from `a9c17d90` on 2026-07-29; Stage 1 built, adversarially reviewed, and promoted 2026-08-05 (production merge `874f1599`); record re-audited against `0ec677c8` on 2026-08-06
...
**Stage 1 is published and its gate is SATISFIED (2026-08-05): the production routes return signed-out 200 and the owner has set the Stripe Business Settings Terms URL — the single Checkout precondition for promoting required Terms consent. Stage 2 is therefore unblocked.** DEBT-414 remains active for these five workstreams:
...
4. Draft New York SHIELD Act security and incident-response procedures now exist, but 16 of their 17 live-account `OPEN` checks remain unverified (the support-mail receipt check was closed 2026-08-05) and the owner has not signed either adoption block.
...
| Stripe statement descriptor | **RE-OPENED 2026-08-06.** The owner re-verified a descriptor value of `ADDICTIONBOARDS.COM` on 2026-08-05 — but that is **19 characters**, so it is the *full* descriptor, not the **shortened** descriptor § 11 requires to be 2–10 characters and suffix-compatible. Either a different Stripe field was read, or this row and § 11 have mislabeled the field. **The shortened-descriptor check is unverified; do not treat it as satisfied.** |
| Agent access to Stripe | **The audit did not access or modify *live* Stripe.** It did access **test mode**: on 2026-08-05 a probe authenticated with the `sk_test_` key from `.env.local` and **created test-mode Checkout Sessions** (`cs_test_…`) to measure the `consent_collection` precondition — see Implementation specification § 5. That is authenticated access plus object creation in test mode, not read-only inspection. No live key was ever used, held, or available locally, and no live Stripe state was read or written by any agent. *(Corrected 2026-08-06: this row previously said flatly "This audit did not access or modify Stripe," which the probe made false.)* |
...
- [x] **Verify end-to-end delivery to `support@addictionboards.com`. VERIFIED 2026-08-05, 2:32 PM ET** by owner external send from a personal `@gmail.com` address (not the destination domain). Delivered to the Google Workspace destination inbox **in Inbox, not spam**, within about one minute, with headers `mailed-by: addictionboards.com`, `signed-by: gmail.com`, TLS in transit — that is, the originating sender's DKIM signature survived the ImprovMX forward intact in this test, in contrast to the rewritten self-send described below. One delivered message is not proof of a general deliverability property; it is evidence that the published channel works for ordinary external mail. **Still unverified:** the claimed catch-all (only the published `support@` alias was exercised; no copy represents a catch-all, so nothing depends on it) and outbound sending *as* `support@addictionboards.com` (ImprovMX free tier forwards inbound only, so owner replies currently originate from the Google Workspace address — see the cancellation-procedure gate below). **Unverified and NOT named by the successful test — forwarding of strict-DMARC senders.** The one delivered message came from `gmail.com`, which publishes `p=none`, the most permissive possible policy; its `mailed-by: addictionboards.com` header shows ImprovMX rewrote the envelope sender, which is exactly what breaks SPF alignment. The receiving Workspace evaluates DMARC against the **original sender's** From: domain, so this test says nothing about senders publishing `p=quarantine`/`p=reject` — common on hospital, university, and health-system domains, i.e. this product's actual user population. If such mail is quarantined after the rewrite, consumer cancellation and privacy requests sent to the address published in both legal pages would silently never arrive. **Test this class before relying on the email channel:** send from a `p=reject` domain and confirm Inbox delivery. *(A first attempt sent from the destination mailbox itself never arrived. ImprovMX's own notice attributes this to Google dropping self-addressed mail that reuses a just-emitted Message-ID, plus their rewrite/re-sign workaround tripping spoofing heuristics — **that mechanism is the provider's explanation, not something observed here**, since the message never arrived and no headers or logs existed to inspect. Neither `addictionboards.com` nor `novamindnyc.com` publishes a DMARC record (verified by `dig`), so no DMARC policy of ours was involved either way.)*
...
*Measured, by probe:* against **test mode of the live account** (`acct_1SvkizKItmaHAwgU`, resolved from the `sk_test_` key in `.env.local` — the same key the E2E suite uses), a **`mode: 'setup'` Session carrying `consent_collection[terms_of_service]=required` was created successfully**, returning `consent.terms_of_service = 'required'`; a subscription-mode session with the same field also succeeded; and that account's test-mode `business_profile` came back **empty** (no `url`, no `support_email`, no placeholder data).

*NOT measured, and stated as report or inference only:* the live account's Public details contents (no live key exists locally or is available to any agent — the live side rests entirely on the owner's Dashboard action of 2026-08-05); and the **ToS URL itself is not observable through the Stripe Account API at all** — no `terms`/`tos` field exists on the Account object — so "Public details is empty" was proxied from two adjacent nulls, never read directly.

*Which construct was tested:* **test mode of the live account, NOT a separate Stripe sandbox account.** The Dashboard labels test mode "Sandbox", but a true sandbox is a distinct `acct_`. Stripe's documented exclusion of Public details from the sandbox copy therefore applies to a construct this probe never exercised.

Consequences for the build:

1. **No configuration is required in this account's test mode** — the environment the app's test key and the E2E suite actually target. **A genuinely separate sandbox account is out of scope of this measurement and must be checked on its own** before Stage 2 is pointed at one.
...
3. **⚠️ Green test-mode runs do NOT validate the live precondition.** The empty `business_profile` plus successful creation is best explained by the check simply **not being enforced outside live mode**. So no unit, E2E, or preview run can ever exercise the live ToS-URL requirement: a live URL that is missing, cleared, or edited later produces a hard Checkout Session creation failure that first appears **in production, on the paid-signup path, with zero pre-production signal**. Because the URL is invisible to the Account API, the only evidence it exists is the owner's Dashboard state — **re-confirm it in the Dashboard immediately before promoting Stage 2, and treat any later Public-details edit as a release-blocking change.**
...
- [x] Stripe legal links are set only after deployment. **Done 2026-08-05** — both URLs set in live Public details after production signed-out 200s; support email set to the published `support@addictionboards.com`. This account's test mode requires no equivalent setup (measured — see § 5).
- [ ] **Shortened statement descriptor reverified as 2–10 characters and suffix-compatible.** Re-opened 2026-08-06: the value recorded on 2026-08-05 (`ADDICTIONBOARDS.COM`) is 19 characters, i.e. the full descriptor, so the shortened-descriptor field is still unverified.
