...
**OWNER-SELECTED PROVIDER: Resend.** The provider-independent names are fixed: add `src/application/ports/transactional-email-gateway.ts`, export it from `src/application/ports/index.ts`, and add `src/application/test-helpers/fakes/fake-transactional-email-gateway.ts` plus `.test.ts` and its barrel export. Put the SDK adapter at `src/adapters/gateways/resend-transactional-email-gateway.ts` with a colocated test. Wire it through `lib/container/gateways.ts`, `lib/container/types.ts`, and `lib/container.test.ts` rather than calling an SDK from a use case. **Fail-closed delivery rule:** if `RESEND_API_KEY` is absent or provider delivery fails, do not fail Checkout, webhook validation, or consent persistence; keep the durable acknowledgment row queued and undelivered, return successfully from the consent path, and let the notice job retry delivery. Every gateway send must receive a stable provider idempotency key derived from the delivery-row UUID (for example, `renewal-notice/<delivery-id>`), pass it through the Resend SDK's `idempotencyKey` option, and keep the payload immutable across retries. Before calling Resend, take an atomic eligible-to-processing claim and persist its attempt ID and start time; a worker that disappears after the provider call therefore leaves an auditable in-flight attempt, and stale processing claims become `outcome_unknown` rather than automatically returning to the retry set. [Resend retains idempotency keys for 24 hours](https://resend.com/docs/dashboard/emails/idempotency-keys), so an ambiguous timeout or process-loss outcome must never be blindly resubmitted by the daily job after that window: persist or recover it as `outcome_unknown`, exclude it from automatic selection, and require an operator to confirm in Resend that no send occurred before requeueing the same durable row. Model gateway results explicitly as `sent`, `transient_failure`, `terminal_failure`, or `outcome_unknown`; only a provider response that establishes non-acceptance is transient and gets `nextAttemptAt` with capped exponential backoff, while terminal and unknown outcomes remain excluded until requeued. A missing API key is pre-send and remains queued rather than terminal. Cover missing-key, atomic-claim concurrency, crash-after-claim, stable-key replay, payload mismatch, known-transient, terminal, ambiguous-outcome, backoff-selection, and requeue behavior through the gateway, fake, adapter, container, acknowledgment-use-case, and route/job tests in PR-C.
