...
In the finalize path, translate the `23505` loser into the truth **after the losing transaction aborts**: catch the unique-violation-derived CONFLICT around the `writeTransaction` boundary (not by re-reading through the same stale REPEATABLE READ transaction), re-read the session through a fresh repository snapshot, and if `endedAt` is set throw `practiceSessionAlreadyEndedError()` — which carries `details.reason = AlreadyEnded`, is monotone (correct to cache), and drives the existing client recovery. A lower-level repository seam can also work only if it has explicit finalize context and performs the ended-session re-read outside the stale transaction. Alternatively (deeper fix): make the loser detectable *before* attempt insertion by having finalize take the parent-row write first (`end()` before attempt inserts), forcing the loser onto the retryable `40001` path — but that reorders Track A's deliberate child-first lock order and must be weighed against [BUG-278](bug-278-end-discard-idempotency-caches-transient-errors-under-fixed-key.md)'s deadlock analysis, not done casually.
