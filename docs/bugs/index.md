...
**Latest 2026-08-28 update:** **The BUG-304/305/306 tail is re-audited, BUG-307 is filed, and its workflow fix is implemented pending one promoted-artifact scan.** BUG-304 stays P3/Open: its original cause is unproven and the current application still has a reachable superseded-handler no-op, while the shipped render barrier protects only Playwright. BUG-305 is Resolved and archived after the promoted console redaction, a 70/70 clean post-redaction CI-log rescan, 4,585 successful TEST-session revocations with zero active sessions afterward, and deletion of its three named runs. BUG-306 is Resolved and archived as “cause unproven; mitigation verified”: its one-suite-session lifecycle, fail-closed auth state, cleanup, auth-loss diagnostic, and owner backlog containment are proven; no cause is invented for the original redirect. BUG-307 (P2/Open) corrects the log census to 455/1,210 scanned CI logs (452/1,106 excluding Dependabot branches), owns the separate public artifact surface, and now splits report/failure uploads, excludes auth state and traces, keeps tracing local, and redacts Stripe TEST identifiers; all 23 known trace-bearing artifacts are deleted, and only the post-promotion non-printing artifact scan remains. Hosted failures `33110618884` and `33175157228` confirmed DEBT-471 F1 drift; repair validation `33213317375` passed 4/4 on promoted `main`. **Next Bug ID is BUG-308.**

**Prior 2026-08-26 update (forward pointer: superseded by the 2026-08-28 Latest stanza):** **PR #835 promoted the BUG-304/305/306 mitigations to `main` as merge commit `47b31234`, without converting unresolved causes or owner containment into false closure.** CodeRabbit approved exact head `92c35965` with zero unresolved threads. PR run `32955114161` and post-merge main run `32960005070` each passed required E2E 43/43 on the first attempt; non-printing raw-log censuses found zero flaky markers and zero assignments for the three redacted Clerk query fields. Production deployment `6102157792` succeeded, `addictionboards.com` returned HTTP 200, `/api/health` reported the app and database healthy, and `origin/dev` / `origin/main` share tree `6d7fad66`. BUG-304 remains P3/Open because the original no-navigation event's cause is unproven. BUG-305 remains P2/Open pending exposed-session expiry/revocation and historical-log containment. BUG-306 remains P3/Open because the original auth-loss cause is unproven and the historical TEST-session backlog still needs an owner disposition. **Next Bug ID is BUG-307.**

**BUG-305 hosted correction (pre-promotion history from the same update):** First-mitigation run `32916441885` passed required E2E 42/42 but still logged two unmasked values after the second bookmarks case. That run falsified the target-level idempotence guard. A new red case proves that a restored console method must be rewrapped; method-level wrapper tracking passes 4/4 in the redaction suite and awaits a new exact-head hosted receipt. The run is the third known affected public CI run, not evidence that the leak closed. Closure requires every exposed development session to be revoked or verified expired and requires separate historical public-log deletion or equivalent access containment.

*(Forward pointer: the 2026-08-28 Latest stanza records completed owner containment, archives BUG-305, and files the distinct artifact surface as BUG-307.)*

**BUG-306 filing (pre-promotion history from the same update):** BUG-304's follow-up gate lost a fresh Clerk session on protected navigation: the first attempt of the session-review case redirected to hosted sign-in after successful setup and Start click, an unbounded missing-locator diagnostic consumed the 180-second case budget, and the retry passed. Eleven retry-free repetitions did not reproduce the auth loss, so its cause remains explicitly unproven. A read-only TEST-instance census independently established the lifecycle defect: 47 sign-in calls across 14 spec files, no sign-out, and 70,403 sessions for the one shared test user (4,684 active / 65,719 expired). Red-first changes now create one suite session, explicitly load its ignored `storageState` only in the 14 authenticated specs, fail closed if that state is absent, sign out and remove the state in project teardown, report sign-in redirects directly, and bound the fallback locator probe. The focused result is 31/31 and the exact real-browser setup + failed case + cleanup seam passed 3/3 in 32.1 seconds. BUG-306 stays Open pending exact-head CI, review, promotion, and post-merge proof. **Next Bug ID is BUG-307.**

*(Forward pointer: those delivery receipts completed on 2026-08-26; the 2026-08-28 Latest stanza closes BUG-306 as cause unproven / mitigation verified after owner backlog containment.)*
...
| [BUG-307](./bug-307-public-playwright-artifacts-expose-test-session-credentials.md) | Public Playwright artifacts expose TEST session credentials | P2 | Open | Workflow, trace, exclusion, and Stripe-redaction fixes are implemented and focused contracts pass 73/73; closure awaits the first promoted artifact's non-printing credential-shape scan. |
...
| [BUG-305](../_archive/bugs/bug-305-clerk-dev-browser-credential-in-ci-logs.md) | Runtime-generated Clerk development-browser values no longer enter CI console logs; exposed sessions and named runs were contained, and BUG-307 owns the distinct artifact surface. | P2 | 2026-08-28 |
