...
| [BUG-299](./bug-299-recovery-panel-external-resolution-stale-state-dead-ends.md) | Practice / session start / recovery | P3 | Start-key retirement fires only on the local abandon-SUCCESS arm: a terminal `already_ended`/`exam_time_expired` abandon conflict rotates the abandon key but never refreshes the stale panel or retires the start key (retry dead-end), and a recovery session resolved externally (second tab) leaves a poisoned cached start error replaying against an empty panel until reload. Fix: treat terminal conflicts and no-session-found refreshes as consumed resolutions — refresh state and retire the key. |
