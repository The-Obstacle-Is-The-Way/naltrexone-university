...
**Last Updated:** 2026-07-14 — **Wave-2 fix close: BUG-287, BUG-288 (P2), BUG-289, BUG-290, BUG-291, BUG-294, and BUG-295 are resolved, production-verified, and archived.** Fixes shipped via PR #635 (squash `e2e20f6b`; migration 0029 observation-version CAS + whole-operation retries), PR #634 (squash `21fee665`; deletion-owned Stripe customer lifecycle + conforming deletion lock order), PR #640 (squash `666097aa`; migration 0030, determinacy-aware idempotency policies + client key lifecycles), and PR #642 (squash `980a9bd4`; fingerprint-bound request tokens, session-scoped abandon tokens, start-key retirement), promoted via PRs #636/#638/#641/#643 (main `209b331f`/`438b84d8`/`7d94359b`/`cde6ccd8`, main/dev trees identical at each promo), each with a full local gate and production proof (main CI success, `addictionboards.com` HTTP/2 200). CodeRabbit approval was exact-final-head on #635/#640/#642; **#634 was merged while the formal review decision remained CHANGES_REQUESTED** (conversational acceptance + zero unresolved threads + green checks, no exact-head APPROVED object) — recorded in BUG-288/294's Resolution sections as a merge-gate letter violation. An 8-lens adversarial cross-batch regression review of the combined wave diff (152 files over `318549f5...cde6ccd8`; 25 raw → 14 deduped → 12 confirmed by 3-verifier panels, 2 refuted, none above P3, no revert-grade regression) filed the residue: **BUG-296** (BUG-288's deferred FK-23503 path — post-deletion subscription webhooks 500 through Stripe's retry window), **BUG-297** (checkout-success CAS exhaustion surfaces an uncaught CONFLICT to a just-paid user), **BUG-298** (BUG-295's identity binding not swept to submit/mark/bookmark keys + the feedback guard's missing context fields), **BUG-299** (recovery-panel resolutions outside local abandon-success neither refresh the panel nor retire the start key), and **DEBT-457** (P4 residues: end-session thrown-arm rotation, version-fence double retrieve cost, vacuous fake snapshot/restore test). A once-observed `practice.spec.ts` E2E timeout that passed on Playwright retry was flagged but not filed (file on recurrence). Active bugs: BUG-296..BUG-299 — four P3. **Next Bug ID is BUG-300.** Previous update (2026-07-14) — **BUG-295 filed** (post-merge adversarial review of the wave-2 idempotency PR #640: keys preserved across indeterminate outcomes are neither bound to the request identity they were minted for nor retired when consumed — changed feedback payloads replay the committed original, and the recovery panel's abandon/start keys recreate BUG-291's dead-end; fix shipped with the filing PR #642, production-verified same day). Previous update (2026-07-11) — **Wave-1 fix close: BUG-283, BUG-284 (P2), BUG-285, BUG-286, BUG-292, and BUG-293 are resolved, production-verified, and archived.** Fixes shipped via PR #628 (squash `45bf6232`; fail-closed identity contract + savepoint-safe upsert recovery), PR #626 (squash `8e011843`; fresh-transaction webhook failure persistence + canonical subscription-writer lock order), and PR #627 (squash `0c1221af`; runner-owned discard retries + single-snapshot `end()` pre-reads), promoted via PRs #630/#631/#629 (main `6b9fab48`/`c12d608b`/`cca0470d`, main/dev trees identical at each promo), each with CodeRabbit exact-head approval, a full local gate, and production proof (`test` + `deploy` success on main, `addictionboards.com` HTTP/2 200). A 6-lens adversarial cross-batch regression review of the combined wave diff (refute-biased verification; 14 raw → 6 confirmed findings, none above P3) filed the residue: **BUG-294** (user-deletion cascade lock-order inversion — reproduced on real Postgres), DEBT-455 (FakeUserRepository fidelity divergences), and DEBT-456 (client conflict-reason discrimination gaps), plus post-wave accuracy annotations to DEBT-443/449/452/454. Active bugs: BUG-287..BUG-291, BUG-294, and BUG-295 — one P2 (BUG-288), six P3. **Next Bug ID is BUG-296.**
...
| Bug | Family | Priority | Summary |
|-----|--------|----------|---------|
| [BUG-296](./bug-296-post-deletion-subscription-webhooks-fail-users-fk.md) | Stripe webhook / event ledger / account deletion | P3 | After a committed account deletion, subscription webhooks carrying the deleted local UUID — deterministically including the `customer.subscription.deleted` events our own `customers.del` emits — fail the users FK (`23503`), which the repository wraps as generic `INTERNAL_ERROR`: a durable failed `stripe_events` row plus incident-shaped 500s through Stripe's ≤3-day retry window per event, with correct terminal state everywhere. Recommended fix is BUG-288's deferred Option 3: classify the missing-user FK at the repository boundary and acknowledge post-deletion writes at the controller. |
| [BUG-297](./bug-297-checkout-success-version-cas-exhaustion-uncaught-conflict.md) | Billing / checkout success / observation-version fence | P3 | The BUG-287 CAS loop throws an uncaught `CONFLICT` after 3 version conflicts on `/checkout/success`, and the normal post-checkout webhook burst is exactly the concurrent writer racing each attempt's live-Stripe-retrieve window — a just-paid user can land in the route error boundary while their subscription row is fully correct (pre-#635 code always resolved via the write guard). Fix: catch the exhaustion at this display-oriented call site and fall back to reading the current row. |
| [BUG-298](./bug-298-preserved-keys-unbound-on-submit-mark-bookmark-surfaces.md) | Idempotency client key lifecycle | P3 | BUG-295's identity-binding invariant was never swept across the sibling preserve-across-indeterminate surfaces #640 created: submit-answer keys aren't bound to `selectedChoiceId` (a changed-choice retry replays the old choice's grade as the new one's), mark-for-review uses one session-page key bound to neither question nor direction, bookmark keys ignore the desired direction, and the server-side feedback replay guard omits `attemptId`/`practiceSessionId`. Fix: extend the fingerprint-token pattern via the shared helpers + widen the repository guard. |
| [BUG-299](./bug-299-recovery-panel-external-resolution-stale-state-dead-ends.md) | Practice / session start / recovery | P3 | Start-key retirement fires only on local abandon success: an externally-ended tutor session makes abandon return a bare no-reason `CONFLICT`, after which the client neither refreshes the stale recovery panel nor retires the poisoned start key; resolution in another tab likewise leaves a cached start error replaying until reload. Exam discard already self-heals and is a control. Fix: refetch after every failed abandon and retire the key only when a successful typed refresh proves no incomplete session remains. |

Companion filing from the same review: [DEBT-457](../debt/debt-457-wave2-determinacy-and-test-hygiene-residues.md) (P4 — `endSession`'s indeterminate thrown-arm key rotation, the version fence's per-attempt double Stripe retrieve cost, and a vacuous `FakeSubscriptionRepository` snapshot/restore test). The two refuted candidates (a claimed `user.updated` fallback lock-order regression and a claimed rollout-window version-fence blind spot) died with quoted-code refutations in the panel record.
