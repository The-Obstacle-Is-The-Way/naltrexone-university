...
**Last Updated:** 2026-06-25 (**resolved + archived BUG-262** (P4) — the daily billing cron ran reconcile and the deleted-account Stripe-cancellation drain in one shared `try`, so a true page-0 reconcile throw (e.g. the `listLocalSubscriptions` query failing — not a caught row-level error) skipped the drain (BUG-246's safety net), leaving a deleted user's Stripe sub billable an extra ~24h. Fix decouples the drain into its own `try`/`catch` with `drainFailed = throw || drain.failed > 0`. Shipped via PR #520 (squash `5aac1d5d`) → promo #521 (merge `f40b1904`); production deploy `dpl_BGQQppuN4vaiozxrocgbuXjrDi7t` verified READY 2026-06-25 (`addictionboards.com` HTTP 200); doc moved to `docs/_archive/bugs/`, the Active Bugs register is empty again. BUG-262 was the only filable survivor of a broad ~14-surface adversarial P0–P4 sweep — see the sweep note below. Prior 2026-06-25 — BUG-260 RESOLVED + archived: question rating/report actions persisted client-supplied `attemptId`/`practiceSessionId` with only UUID-shape validation and no ownership/question/session-membership check, so an entitled user could attach feedback to an unrelated existing attempt/session and corrupt the SPEC-041 export-correlation metadata. Fixed with a shared `validateFeedbackContext` helper that proves ownership + question-match + session-membership before persisting (`NOT_FOUND` for not-owned, `VALIDATION_ERROR` for mismatch, null context still allowed), with `AttemptRepository`/`PracticeSessionRepository` injected into both use cases; controller/schema/repository/export unchanged. Shipped via PR #516 (squash `542eedbc`; CodeRabbit clean on the exact fix head `3cb21ae2`), promoted to `main` via PR #517 (merge `602997b9`; promo CodeRabbit rate-limited but byte-identical to the CR-clean fix head, owner-authorized override); production deploy `dpl_EFeg1cHqhTvdTU2W5bWq5iuyJDDe` verified READY 2026-06-25 (`addictionboards.com` HTTP 200); `main` and `dev` trees identical. Prior — BUG-258 RESOLVED + archived: in an active timed exam, a manual draft-saving control invoked after the server deadline while the browser clock lagged decided expiry from `Date.now()` instead of honoring the server's `CONFLICT`, so the captured on-screen selection could be neither persisted nor flushed and a reload before the local timer self-healed finalized the question as omitted (permanent wrong score). Fixed by threading a typed `ExamDraftSaveResult` so the server's CONFLICT (not the browser clock) drives expiry recovery — Review & Submit and every draft-save-before-navigation caller flush the captured draft to `finalizeExamAnswers`; app-layer hooks only, server use-cases unchanged. Fixed via PR #498 (squash `bf653f5a` on `dev`; CodeRabbit clean on fix head `f966aebf`), promoted to `main` via PR #499 (merge `cf7b88a7`; promo CodeRabbit rate-limited but byte-identical to the CR-clean fix head, owner-authorized override); production deploy `5172902620` verified READY 2026-06-23 (`addictionboards.com` HTTP 200); `main` and `dev` trees identical. Prior — BUG-257 RESOLVED + archived: active-exam write paths (draft save, finalize grading, BUG-254 expiry flush) now refetch drafted questions through session-scoped, publication-agnostic repository reads (`findByIdForSession`/`findByIdsForSession`) gated on proven session membership, so a question unpublished mid-exam no longer strands save/submit; public browsing and candidate selection stay published-only. Fixed via PR #494 (squash `de29bf32` on `dev`; CodeRabbit `APPROVED` on `08f947dd`), promoted to `main` via PR #495 (merge `8acc2964`); production deploy `6kHUtCD6ZT3JZ3n6Fi4j36n81WTv` verified READY 2026-06-23 (`addictionboards.com` HTTP 200); `main` and `dev` trees identical. BUG-258 is now resolved + archived. Prior 2026-06-22 — BUG-255 RESOLVED + archived: the Review & Submit screen disabled the active exam timer, so a late manual submit recorded an uncapped wall-clock `endedAt`/`durationSeconds` (inflating summary/history duration + completed-session ordering; answers were already locked at the deadline, so no score/integrity impact). `FinalizeExamAnswersUseCase` now caps the persisted exam end at `min(now, max(deadline, latestAnsweredAt))` — the grace-flush clamp keeps `endedAt` ≥ any recorded attempt — threaded through optional `PracticeSessionRepository.end(…, endedAt?)` / `AttemptInsertInput.answeredAt?` params that default to the repo clock (tutor path unchanged); summary + history project from the one persisted value. Fixed via PR #490 (squash `2a8d1a64` on `dev`), promoted to `main` via PR #492 (merge `ce8373ef`); production deploy `dpl_EokBHBs9hG1MgrhWHoCKvviLAgSg` verified READY 2026-06-22 (`addictionboards.com` HTTP 200); `main` and `dev` trees identical. Prior 2026-06-22 — BUG-256 RESOLVED + archived: resuming an expired exam now recovers to the server-authoritative summary instead of "No more questions found" — an app-layer null-question recovery hook re-reads the summary on the bootstrap-fallback `ok(null)` before the empty state commits (read-only, idempotent, request-id/unmount-race guarded; normal-navigation nulls unchanged). Fixed via PR #479 (squash `b848a778` on `dev`), promoted to `main` via PR #487 (merge `5f71d9cc`); production deploy `dpl_35zefrDS2AX7vi6MTTySDcaZdMTc` verified READY 2026-06-22 (`addictionboards.com` HTTP 200); `main` and `dev` trees identical. Prior 2026-06-21 — BUG-254 RESOLVED + archived: the bounded single-question server-side finalization flush shipped — exam-timer expiry now grades the on-screen selection instead of recording it as omitted. Fixed via PR #476 (squash `87650d55` on `dev`), promoted to `main` via PR #477 (merge `0bc288fb`); production deploy verified READY 2026-06-21; `main` and `dev` trees identical. Manual practice/quiz engine deep sweep filed BUG-254..BUG-257. BUG-255 (P4) Review & Submit disables the active exam timer, so a late manual submit inflates recorded duration only (answers are locked at the deadline; no score/integrity impact). BUG-256 (P3) resuming an expired exam can briefly show "No more questions found" instead of results after server-side auto-finalization (recoverable; self-heals on reload). BUG-257 (P4) active exam draft/finalization paths depend on current published-question state — latent hardening gap with no in-app trigger. BUG-253 remains withdrawn/unfiled per owner guidance. Prior: BUG-252 resolved + archived — exam draft saves now accept nullable `selectedChoiceId`, so unanswered time-only drafts persist `draftCumulativeMs`; `FinalizeExamAnswersUseCase` unchanged and BUG-238 cumulative bounds intact. Fixed via PR #472 (squash `2916f416` on `dev`), promoted to `main` via PR #473 (merge `d2f96d61`), production deploy verified READY; `main` and `dev` trees identical.)
...
_No active bugs — all filed bugs through BUG-262 are resolved + archived to_ `docs/_archive/bugs/`.
