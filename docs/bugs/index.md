...
**Last Updated:** 2026-07-16 — **Fix-wave-4 close: BUG-300 and BUG-301 are resolved, production-verified, and archived.** BUG-301 shipped through PR #656 (squash `da3cdfc6`) → promo #658 (`889b4178`); BUG-300 shipped through PR #657 (squash `d393f11a`) → promo #659 (`53ef2e2f`). Fix and promo PRs all carried CodeRabbit formal APPROVED reviews on their exact heads after full rate-limit cooldowns. Fresh close proof found main/dev byte-identical at tree `b40078a8`, main CI run 29459937203 successful including deploy, and `addictionboards.com` HTTP/2 200. The six-lens combined review of `ade71553...53ef2e2f` produced 18 raw candidates → 9 deduplicated → 3 confirmed by 3-verifier panels: **BUG-302** (P4 stale report completion owns a newer dialog), **BUG-303** (P4 abandon retirement loses a still-running Start key), and **DEBT-459** (P4 test-probe Button-rule ambiguity); six candidates were refuted. Active bugs: BUG-302 and BUG-303. **Next Bug ID is BUG-304.** Prior detailed wave records remain below.
...
|-----|--------|----------|---------|
| [BUG-302](./bug-302-stale-report-completion-closes-newer-dialog.md) | Question feedback / dialog ownership | P4 | BUG-301 fences the request-token slot, but the report dialog still lets an obsolete submit completion close and reset a newer reopened form while its request remains in flight. Token safety is preserved; the impact is form loss and misleading UI state. |
| [BUG-303](./bug-303-concurrent-start-abandon-retires-still-running-key.md) | Practice / session start / recovery ownership | P4 | BUG-300's concurrent uncertainty remains local to one Start call. Abandoning the refreshed per-user session can rotate the same key while the original request is still able to create a different session after the abandon. |

Companion filing: [DEBT-459](../debt/debt-459-test-probe-button-rule-scope-ambiguity.md) (P4 — the live frontend rule reads as universal under `app/**`/`components/**`, while the deliberate production scanner and established test practice exclude browser specs and probe helpers). The close ruled that native semantic buttons are allowed only in test-only hook/state-machine probes; production UI remains fully subject to the shared Button mandate.
