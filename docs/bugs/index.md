...
**Last Updated:** 2026-07-10 — **Cycle B4 completed the independent BUG-283..293 documentation audit by re-deriving BUG-292/293 from current source, installed driver code, and PostgreSQL isolation semantics.** BUG-292 remains P3: the blocked RR DELETE produces a Drizzle query error with a `PostgresError` `40001` cause, not a bare driver error; a bounded fresh-transaction retry is safe only for known abort codes, while indeterminate connection/commit outcomes must remain uncached. BUG-293 remains P4 but is narrower than filed: public end-vs-discard is excluded by tutor/exam mode guards, transaction-bound finalize inherits one RR snapshot, and only the concurrent Clerk account-deletion cascade remains reachable; the controller returns a non-throwing `INTERNAL_ERROR` action result rather than an HTTP 500. The sweep total remains two P2, eight P3, and one P4. Cycles B1-B3 corrected BUG-283..291. The original DDIA-lens sweep filed BUG-283..293 plus DEBT-443..454 after 12 finder lenses, per-candidate verification, and archive-wide dedup. Families remain: BUG-147 email-reclaim residuals (BUG-283/284), Stripe billing transaction hazards (BUG-285..288), idempotency/key-rotation failures (BUG-289..291), and practice-session transaction/read-snapshot gaps (BUG-292/293). See the Latest sweep section for the full method and coverage ledger.
