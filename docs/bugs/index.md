...
**Last Updated:** 2026-07-09 — **Cycle B3 independently re-audited BUG-289..291 and corrected all three fix plans.** BUG-289 remains P3: exhausted Stripe 5xx retries surface as raw errors normalized to cached `INTERNAL_ERROR` (not invariably `STRIPE_ERROR`), each form owns an independent key, and safe remediation must preserve indeterminate keys plus add business-level idempotency before broadly retrying append-only feedback writes. BUG-290 remains P3: active Exam draft navigation is out of scope; Quick/filtered Practice, standalone questions, Tutor/session-review submit, and Exam mark are in scope, while rollback-certain `57014` and commit-indeterminate connection failures require different handling. BUG-291 remains P3: the code proves a 15s-client/30s-server completion window but not that commits after timeout are typical; retries under newly rotated keys re-execute rather than replay, and thrown transport errors must preserve the original key. The sweep total remains two P2, eight P3, and one P4. Cycles B1/B2 corrected BUG-283..288. The original DDIA-lens sweep filed BUG-283..293 plus DEBT-443..454 after 12 finder lenses, per-candidate verification, and archive-wide dedup. Families remain: BUG-147 email-reclaim residuals (BUG-283/284), Stripe billing transaction hazards (BUG-285..288), idempotency/key-rotation failures (BUG-289..291), and unowned practice-session races (BUG-292/293). See the Latest sweep section for the full method and coverage ledger.
