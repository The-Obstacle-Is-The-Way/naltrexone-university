...
**Last Updated:** 2026-07-10 — **Cycle D1 began the independent debt-doc audit with DEBT-443/444; bug findings and priorities are unchanged at two P2, eight P3, and one P4.** The debt audit corrected unsafe idempotency replay-recovery guidance (a completed cached outcome must not be deleted/re-executed), added the first/replay public-error divergence omitted from DEBT-443, narrowed prune contention from necessarily identical/deadlocking batches to proven overlapping-`DELETE` waits, and expanded the direct real-Postgres prune coverage gap to both cleanup tables. Cycle B4 previously completed the independent BUG-283..293 audit by re-deriving BUG-292/293 from current source, installed driver code, and PostgreSQL isolation semantics; Cycles B1-B3 corrected BUG-283..291. The original DDIA-lens sweep filed BUG-283..293 plus DEBT-443..454 after 12 finder lenses, per-candidate verification, and archive-wide dedup. Families remain: BUG-147 email-reclaim residuals (BUG-283/284), Stripe billing transaction hazards (BUG-285..288), idempotency/key-rotation failures (BUG-289..291), and practice-session transaction/read-snapshot gaps (BUG-292/293). See the Latest sweep section for the full method and coverage ledger.
