...
**Last Updated:** 2026-07-10 — **Cycle D2 audited DEBT-445/446; bug findings and priorities remain two P2, eight P3, and one P4, while debt remains eight P3 and four P4.** The audit separated DEBT-445's latent out-of-order migration high-water scenario from the actual early-0027 amended-after-apply incident, identified the Production merge-order variant, corrected current Neon restore semantics, and replaced a feature-preview migration skip that could serve code against an old schema. It also corrected DEBT-446's claim that all seed paths hide their target: bare migrate/seed do, but `db:seed:all` already prints a credential-free Production target and instead lacks enforced exact consent plus a per-question change plan. Cycles B1-B4 previously completed BUG-283..293; Cycle D1 audited DEBT-443/444. The original sweep inventory and bug-family summaries below are unchanged.
