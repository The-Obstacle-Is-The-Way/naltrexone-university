...
- **Zero candidates survived at P1/P2** — skeptic second votes were reserved for P1/P2 claims and every confirmed finding settled at P3/P4. The verifiers made material corrections along the way (refuted the finalize leg of the state-write caching claim, corrected the discard-race trigger analysis, narrowed the deadlock collision window) which are recorded in the filed docs.
...
|-----|--------|----------|---------|
| [BUG-283](./bug-283-clerk-webhook-email-reclaim-dead-in-transaction.md) | Clerk webhook / user provisioning | P3 | The BUG-147 email-reclaim fallback runs its recovery UPDATE on the same postgres-js transaction the failed INSERT just aborted, so `user.updated` for a recreated account 500s deterministically (25P02) through Svix retry exhaustion; the row heals only on sign-in, where the auth gateway runs the same method on the raw autocommit db. |
| [BUG-284](./bug-284-user-upsert-email-reclaim-cross-identity-takeover.md) | User provisioning / identity | P3 | The same fallback treats email as identity: with a stale local row (lost/in-flight `user.updated`), a different person re-registering the freed email silently inherits the old row — Stripe linkage, attempts, bookmarks — and when the acting user already has their own row, the fallback UPDATE dies on `users_clerk_user_id_uq`, deterministically failing sign-in and webhook retries until manual DB intervention. |
| [BUG-285](./bug-285-stripe-webhook-markfailed-on-aborted-transaction.md) | Stripe webhook / event ledger | P3 | The controller catch calls `stripeEvents.markFailed` inside the transaction that just aborted (e.g. a raw 23505 from `stripeCustomers.insert` on the outer tx), so it dies with 25P02: the spec-required durable failure state never persists, the original root cause is replaced in logs, and Stripe redelivers identically for ~3 days. The Clerk controller already implements the correct fresh-transaction `persistFailure` pattern. |
...
| Identity / user provisioning | BUG-283/284 filed (both residuals of the BUG-147 fix, whose tests only exercise the autocommit path) |
...
**2026-07-09 — 11 bugs filed from the DDIA-lens adversarial database-seam sweep: BUG-283..BUG-293 (all P3 except BUG-293 at P4).** Full methodology, per-bug table, and coverage ledger are in the "Latest sweep (2026-07-09)" section near the top of this file; each bug doc carries its verified reproduction interleaving, severity rationale, and recommended fix. None is a data-corruption or wrong-money-now defect — the P3s are deterministic webhook 500s with self-healing paths, ≤24h self-correcting entitlement reverts, a narrow post-deletion orphan-billing race with a human-visible Stripe signal, cached-transient-error retry wedges recoverable by reload, and race-window misclassified errors. **Next Bug ID is BUG-294.**
