...
- [BUG-257](./bug-257-active-session-finalization-depends-on-current-published-question.md) (P4) filed: active exam draft save/finalization refetch currently published questions — a latent hardening gap (read tolerates missing rows, write throws `NOT_FOUND`) with no in-app unpublish/delete trigger; requires an out-of-band content change during an active exam.
...
|-----|----------|---------|
| [BUG-257](./bug-257-active-session-finalization-depends-on-current-published-question.md) | P4 | Active exam draft save/finalization depend on current published-question state — latent hardening gap (read tolerates missing rows, write throws `NOT_FOUND`); no in-app unpublish/delete trigger. |
