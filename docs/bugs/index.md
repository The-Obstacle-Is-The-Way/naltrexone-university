...
|-----|----------|-----------|---------|
| [BUG-262](bug-262-reconcile-page0-failure-starves-cancellation-drain.md) | P4 | Cron / Stripe reconciliation / billing | The daily billing cron runs reconcile + the deleted-account cancellation drain in one shared `try`; a page-level first-page reconcile failure skips the drain for that cron cycle (self-healing next run). Fix decouples the drain into its own try/catch. |
