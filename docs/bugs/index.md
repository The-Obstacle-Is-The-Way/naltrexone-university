...
**Last Updated:** 2026-07-09 — **Cycle B1 independently re-audited BUG-283..285 and corrected their register summaries.** All three defects remain active. BUG-284 was raised to P2 because its silent cross-identity entitlement/history transfer is an authorization and data-isolation breach; the sweep total is now one P2, nine P3, and one P4. BUG-283's `user.created` trigger was removed and its fresh-transaction failure persistence documented. BUG-285's reachability was narrowed to ownership drift that survives the subscription write (or another direct-statement abort), and its logging/fix guidance now matches postgres-js behavior. The original DDIA-lens sweep filed BUG-283..293 plus DEBT-443..454 after 12 finder lenses, per-candidate verification, and archive-wide dedup. Families remain: BUG-147 email-reclaim residuals (BUG-283/284), Stripe webhook/cron transaction hazards (BUG-285..288), idempotency/key-rotation failures (BUG-289..291), and unowned practice-session races (BUG-292/293). See the Latest sweep section for the full method and coverage ledger.
...
- The filing pass initially graded every confirmed finding P3/P4. Cycle B1's independent re-audit raised BUG-284 to P2 after rejecting the proposed identity check: a silent cross-account transfer of entitlement and practice history is an authorization/data-isolation breach even with compound reachability. No finding is P1. Other verifier corrections include the refuted finalize leg of the state-write caching claim, the corrected discard-race trigger analysis, and the narrowed deadlock collision window.
...
| [BUG-286](./bug-286-webhook-vs-reconcile-lock-order-deadlock.md) | Stripe webhook / reconcile cron | P3 | The webhook tx acquires advisory-user-lock → `stripe_customers` row lock; reconcile Phase 4 acquires them in the opposite order — an AB-BA cycle, so a webhook for user U landing inside U's Phase-4 window deadlocks (40P01) and one victim aborts (the webhook victim then also hits the BUG-285 25P02 masking). |
| [BUG-287](./bug-287-reconcile-cron-stale-snapshot-overwrites-newer-webhook-state.md) | Billing / reconcile cron | P3 | `shouldPersistSubscriptionWrite` unconditionally persists same-identity writes — no recency fence, and `stripe_subscriptions` stores no source timestamp — so the daily cron's Phase-4 write of its stale Phase-1 snapshot silently reverts a webhook-applied cancellation or renewal that landed in the window: wrong entitlement in either direction for up to 24h, self-healing at the next run. |
| [BUG-288](./bug-288-checkout-completes-after-account-deletion-orphan-billing.md) | Billing / account deletion | P3 | A Checkout session opened before Clerk `user.deleted` stays completable (≤24h) after the deletion flow cancels point-in-time subscriptions: the new subscription's upsert hits the users FK (23503), the webhook 500s until Stripe stops retrying, and no mechanism can ever cancel it (tombstones unconsulted on the Stripe write path, drain row already deleted, cron iterates local rows only) — the deleted user's card is billed indefinitely with no local record. |
