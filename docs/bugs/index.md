...
**Last Updated:** 2026-07-09 — **Cycle B2 independently re-audited BUG-286..288 and corrected all three fix plans.** BUG-286 remains P3; its affected subscription-first writers are the webhook and checkout-success eager sync, its impact includes temporary stale entitlement or a failed eager-sync request, and its fix reuses the subscription repository's lock by reordering Phase 4 rather than introducing raw SQL. BUG-287 remains P3; the false 24-hour repair bound and non-causal timestamp fence were replaced with an unbounded next-successful-run statement and a pre-retrieve optimistic version fence. BUG-288 was raised to P2 because a paid returning-user Checkout can create an untracked recurring financial obligation after account deletion; its infeasible Clerk-event-FK queue proposal was replaced with Stripe Customer deletion as the preferred lifecycle closure. The sweep total is now two P2, eight P3, and one P4. Cycle B1 previously corrected BUG-283..285, including BUG-284's P2 authorization/data-isolation grade. The original DDIA-lens sweep filed BUG-283..293 plus DEBT-443..454 after 12 finder lenses, per-candidate verification, and archive-wide dedup. Families remain: BUG-147 email-reclaim residuals (BUG-283/284), Stripe billing transaction hazards (BUG-285..288), idempotency/key-rotation failures (BUG-289..291), and unowned practice-session races (BUG-292/293). See the Latest sweep section for the full method and coverage ledger.
...
- The filing pass initially graded every confirmed finding P3/P4. Cycle B1's independent re-audit raised BUG-284 to P2 after rejecting the proposed identity check: a silent cross-account transfer of entitlement and practice history is an authorization/data-isolation breach even with compound reachability. Cycle B2 raised BUG-288 to P2 because its rare but directly reachable paid-checkout path creates an untracked recurring financial obligation after account deletion. No finding is P1. Other verifier corrections include the refuted finalize leg of the state-write caching claim, the corrected discard-race trigger analysis, and the narrowed deadlock collision window.
...
| [BUG-289](./bug-289-idempotency-caches-transient-errors-billing-bookmark-feedback.md) | Idempotency / billing / bookmarks / feedback | P3 | The wrapper's default policy durably caches EVERY execute error for 24h, and five actions (checkout, portal, bookmark toggle, rating, report) pass no `shouldCacheError` while their client keys rotate only on success or remount — one transient Stripe/Neon fault poisons the key and every retry deterministically replays the cached error until hard reload or TTL expiry (the BUG-278 pattern on the surfaces it scoped out). |
| [BUG-290](./bug-290-state-write-idempotency-caches-transient-internal-error.md) | Idempotency / practice state writes | P3 | `shouldCachePracticeSessionStateWriteError` caches transient `INTERNAL_ERROR` (anything outside the 40001/40P01 retry set); the submit key rotates only on question load and the mark key clears only on success, so re-clicking Submit replays the stale failure and one cached mark failure bricks mark-toggle for the page lifetime. Finalize is NOT affected — its key rotates on failure. |
| [BUG-291](./bug-291-session-start-key-rotation-on-timeout-conflict-dead-end.md) | Practice / session start | P3 | `startSession` rotates its idempotency key on the client-side 15s timeout — an indeterminate outcome where the server action typically commits and stores the result under the original key — so the retry re-executes into the incomplete-session CONFLICT while the resume/abandon panel (fetched only on mount) is not on screen: a dead-end for a start that actually succeeded. |
| [BUG-292](./bug-292-discard-unowned-serialization-failure-internal-error.md) | Practice / discard | P3 | `discard()` self-opens a REPEATABLE READ transaction with no 40001/40P01 catch, mapping, or retry, and is wired without `runPracticeSessionStateWriteTransaction` — the one RR practice-session write with no serialization-failure owner, so a discard racing the expired-exam auto-finalizer surfaces a raw serialization failure as generic INTERNAL_ERROR where a single fresh-snapshot retry would return the use case's documented idempotent success (the BUG-278 remediation leg that was never implemented). |
| [BUG-293](./bug-293-end-split-prereads-phantom-corruption-error.md) | Practice / end session | P4 | `end()` pre-reads the session row and its question-state rows in two separate autocommit statements (unlike `findByIdAndUserId`'s single REPEATABLE READ snapshot), so a concurrent discard or user-deletion cascade committing between them throws the register's canonical "missing normalized question state" corruption signature — a false corruption alarm and a 500 instead of NOT_FOUND, in a millisecond self-healing window. |
...
**2026-07-09 — 11 bugs filed from the DDIA-lens adversarial database-seam sweep: BUG-283..BUG-293 (BUG-284 and BUG-288 at P2, eight at P3, BUG-293 at P4).** Full methodology, per-bug table, and coverage ledger are in the "Latest sweep (2026-07-09)" section near the top of this file; each bug doc carries its verified reproduction interleaving, severity rationale, and proposed fix. Cycle B1 re-audit corrected the blanket "no data-corruption" claim: BUG-284 can silently transfer one account's paid entitlement and practice history to another identity. Cycle B2 raised BUG-288: a paid Checkout completed after account deletion creates an untracked recurring financial obligation with no application-owned stop. The remaining P3/P4 items are narrow webhook failures, entitlement drift repaired by later webhooks or successful reconcile runs (not time-bounded), cached-transient-error retry wedges, and race-window misclassifications. **Next Bug ID is BUG-294.**
