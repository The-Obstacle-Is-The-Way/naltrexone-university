...
| [BUG-277](bug-277-exam-expiry-finalize-stale-flush-aborts-finalization.md) | P2 | Practice / exam finalize / conflict contract | Exam-expiry finalize carrying a stale draft flush (>15s past deadline — e.g. backgrounded tab) aborts the entire finalization with a reason-less CONFLICT; client shows false "Practice session has not ended", drops the at-expiry answer, and the one-shot timer guards never retry |
| [BUG-278](bug-278-end-discard-idempotency-caches-transient-errors-under-fixed-key.md) | P2 | Practice / session lifecycle / idempotency | `endPracticeSession`/`discardPracticeSession` cache ALL errors (no `shouldCacheError`) and the abandon surface's key is the never-rotating `sessionId` — one transient failure (incl. a real discard-vs-finalize AB-BA deadlock → raw `40P01`) bricks session abandonment for the 24h TTL |
| [BUG-279](bug-279-idempotency-wrapper-caches-error-after-committed-success.md) | P2 | Shared adapters / idempotency wrapper | `executeIdempotent` conflates a failed `storeResult` with a failed execution — a transient outcome-write failure after a committed effect durably caches INTERNAL_ERROR as the outcome of a request that succeeded (failure-after-success replayed for the TTL) |
