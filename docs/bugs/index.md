...
**Latest 2026-08-25 update:** **BUG-305 was filed after BUG-304's verification exposed an unmasked Clerk development-browser credential in public CI logs; BUG-304's diagnostic work separately exposed two distinct navigation-observation races before promotion.** BUG-305 is P2: exact-head run `32914624781` passed E2E 42/42 yet printed two credential-bearing Clerk teardown warnings, run `32908317465` had the same shape, the value was not GitHub-masked, and Clerk documents it as sensitive and linked to the client token across the development session lifetime. The latest Clerk testing package retains the unsafe warning, so red-first test-process redaction was added while historical session/log containment remains an owner action. For BUG-304, at 03:15Z the viewport-reset case timed out in `startSession()` after **Start session** was clicked; the failure snapshot retained the enabled starter form, showed no loading state, and contained an empty alert. The retry and a later clean run passed. The helper's 15-second navigation bound equaled the client action's 15-second mutation timeout, so it could discard the rendered error before it existed. Source tracing also found a concrete silent path: count changes rotate the idempotency owner synchronously, a prior render's handler deliberately no-ops after that rotation, and the helper had no render barrier between filling the count and clicking Start. Five retry-free traced runs passed 10/10 each (84.39-87.98 s), so no failing trace proved the original incident cause. PR #834 merged the red-first 20-second outcome bound, count-render barrier, alert capture, diagnostics, and redaction to `dev`; promotion #835 run `32903291145` was then rejected despite aggregate green because two session starts timed out and recovered on retry (`2 flaky` / `40 passed`). Both failure snapshots were already on complete Tutor Session pages, and both causes were an alert-locator read whose execution context was destroyed by that successful navigation. A twelfth focused case reproduced the contradiction red; PR #836 merged the immediate catch-time URL re-check after it passed 12/12. Promotion run `32912478032` then found the later-commit ordering: one exam-mode case timed out and recovered (`1 flaky` / `41 passed`) because the URL had not committed at the immediate re-check and a second diagnostic alert read consumed the remaining 120-second bound. A thirteenth focused case reproduced that sequence red; retrying only the navigation-destroyed DOM read inside the outcome poll passed 13/13 while retaining rendered-error and downstream page assertions. Five retry-free, trace-on runs of that exact exam-mode case then passed 2/2 each in 28-33 seconds. BUG-304 remains P3 and Open pending the second follow-up's review, promotion, and production proof; the original no-navigation cause remains explicitly unproven. BUG-305 remains Open pending exact-head CI/review and owner containment. **Next Bug ID is BUG-307.**

**BUG-305 hosted correction (same update):** First-mitigation run `32916441885` passed required E2E 42/42 but still logged two unmasked values after the second bookmarks case. That run falsified the target-level idempotence guard. A new red case proves that a restored console method must be rewrapped; method-level wrapper tracking passes 4/4 in the redaction suite and awaits a new exact-head hosted receipt. The run is the third known affected public CI run, not evidence that the leak closed. Closure requires every exposed development session to be revoked or verified expired and requires separate historical public-log deletion or equivalent access containment.

**BUG-306 filing (same update):** BUG-304's follow-up gate lost a fresh Clerk session on protected navigation: the first attempt of the session-review case redirected to hosted sign-in after successful setup and Start click, an unbounded missing-locator diagnostic consumed the 180-second case budget, and the retry passed. Eleven retry-free repetitions did not reproduce the auth loss, so its cause remains explicitly unproven. A read-only TEST-instance census independently established the lifecycle defect: 47 sign-in calls across 14 spec files, no sign-out, and 70,403 sessions for the one shared test user (4,684 active / 65,719 expired). Red-first changes now create one suite session, explicitly load its ignored `storageState` only in the 14 authenticated specs, fail closed if that state is absent, sign out and remove the state in project teardown, report sign-in redirects directly, and bound the fallback locator probe. The focused result is 31/31 and the exact real-browser setup + failed case + cleanup seam passed 3/3 in 32.1 seconds. BUG-306 stays Open pending exact-head CI, review, promotion, and post-merge proof. **Next Bug ID is BUG-307.**
...
**Stable baseline (2026-07-18):** zero active must-fix bugs; every remaining active or parked register item was re-verified against the current tree on this date, and future sweeps are owner-initiated rather than self-perpetuating.

Prior update 2026-07-10 — 2026-07-10 — **Cycle D6 independently audited DEBT-453/454; both remain P4, completing the full 23-doc DDIA sweep audit campaign (B1-B4 and D1-D6).** DEBT-453 now records one write/read persistence schema plus a separate controller schema, four effective-selection computations, a truly caller-free `deleteById`, and 11 difficulty encodings across 10 production files while keeping the alleged silent difficulty-corruption cascade refuted. DEBT-454 corrects the flagged runner scope to finalize and session-backed submit only, inventories all four tx-bound lock methods, replaces an unsafe one-commit advisory-hash switch with a rolling-deploy-compatible transition, and identifies the retry delay as base-plus-uniform jitter that its zero-RNG test does not pin. Bug priorities remain two P2, eight P3, and one P4; the final active debt-sweep inventory remains eight P3 and four P4. **Cycle D5 independently audited DEBT-451/452; bug findings and priorities remain unchanged at two P2, eight P3, and one P4, while the active debt-sweep inventory remains eight P3 and four P4.** DEBT-451 now scopes its four integrity gaps to the exact application, hydration, constraint, and concurrent-race boundaries. DEBT-452 corrects a cross-register contradiction: Stripe direct-statement failures do not persist a generic failure row; they enter BUG-285, while Clerk and reconciliation retain only generic wrapper diagnostics. It also rejects a raw-stack-preserving Pino serializer because Drizzle parameters appear in both message and stack. **Cycle D4 independently audited DEBT-449/450; bug findings and priorities remain unchanged at two P2, eight P3, and one P4, while the active debt-sweep inventory remains eight P3 and four P4.** DEBT-449 no longer conflates event retention with the rate-limit horizon, no longer calls all Stripe rows 90-day-capped, and corrects failed-drain retry plus stale-success bookkeeping. DEBT-450 now uses exact source cardinalities: `4F`/800 finalize loop statements, up to 20,000 history state rows, `B` bookmark rows plus relations across three consumers, `A + C` lifetime-count candidates, and `2Aᵥ` ranked history candidates; unsupported production-plan, byte, and latency claims are labeled unverified. **Cycle D3 independently audited DEBT-447/448; bug findings and priorities remain unchanged at two P2, eight P3, and one P4, while the active debt-sweep inventory remains eight P3 and four P4.** DEBT-447 no longer treats out-of-repo Neon timeout/host/capacity settings as observed, no longer claims Vercel invocations or pool pins are unbounded despite `maxDuration`, and records that SPEC-029/BS-017 explicitly accepted postgres.js `max=10`; its vendor-doc leg now distinguishes BUG-240's missing-migration incident from `drizzle-kit push`. DEBT-448 now counts the expired-exam `getNextQuestion` finalizer as a fifth unmetered write-capable path, removes unbounded repeat-finalization/idempotency-growth claims, keeps prune mechanics/coverage under DEBT-444, and corrects the health verification plan so the existing 503 response contract survives while the new probe call is expected. **Cycle D2 audited DEBT-445/446; both remain P3 and did not change the bug or debt priority counts.** That audit separated DEBT-445's latent out-of-order migration high-water scenario from the actual early-0027 amended-after-apply incident, identified the Production merge-order variant, corrected current Neon restore semantics, and replaced a feature-preview migration skip that could serve code against an old schema. It also corrected DEBT-446's claim that all seed paths hide their target: bare migrate/seed do, but `db:seed:all` already prints a credential-free Production target and instead lacks enforced exact consent plus a per-question change plan. **Cycle D1 previously audited DEBT-443/444** and corrected unsafe idempotency replay-recovery guidance, first/replay public-error divergence, overlapping-prune contention wording, and direct real-Postgres coverage scope. Cycles B1-B4 completed BUG-283..293. The original sweep filed BUG-283..293 plus DEBT-443..454 after 12 finder lenses, per-candidate verification, and archive-wide dedup; the bug-family summaries below are unchanged. Prior update 2026-07-08 — **BUG-281 resolved and archived after production proof.** Fix shipped via PR #596 (squash `f7528935`), promoted by PR #597 (merge `603f27b3`, main/dev trees identical), production deployment `56j4BMx4o15fDtWATVzXN4xw7gom` completed, main CI passed on the merge commit, and `https://addictionboards.com/` returned HTTP/2 200. Prior update 2026-07-08 — **BUG-276 resolved and archived after production proof.** Fix shipped via PR #592 (squash `e71c7334`), promoted by PR #593 (merge `3545c83c`, main/dev trees identical), production deployment `At5uoiHGyfphqa1bWjF7GjzgWLMh` completed, CI passed on the merge commit, and `https://addictionboards.com/` returned HTTP/2 200. Prior update 2026-07-08 — **BUG-271, BUG-272, BUG-273, BUG-274, and BUG-275 resolved and archived after production proof.** Fixes shipped via PR #580 (BUG-271/272, squash `44424b2a`), PR #582 + #583 (BUG-274, squashes `a3855c81`/`224514a3`), and PR #581 (BUG-273/275, squash `cb58133a`); a 5-lens adversarial pre-promotion review of the whole wave then produced hardening PRs #584 (hidden-tab milestone baseline + refocus persistence), #585 (confirmed stale-arm BUG-274 resurrection closed: primary-button-only arming, Enter-commit restricted to the radios, focus-to-feedback on commit), #586 (literal e2e redirect assertions), and #588 (promo-review findings + 33 vacuous `toHaveTextContent('')` assertions replaced with anchored regexes across 7 spec files). Promoted by PR #587 (merge `4c380f61`, main/dev trees identical); production deployment `5355265999` succeeded 2026-07-08 and `https://addictionboards.com/` returned HTTP/2 200. BUG-274's product decision is recorded in BS-064 (instant pointer commit preserved permanently; keyboard/AT selection gets a Submit/Enter two-step). Prior update 2026-07-06 — **BUG-277, BUG-278, BUG-279, BUG-280, and BUG-282 resolved and archived after production proof.** Fixes shipped via PR #562 (BUG-278/279, squash `c9e91b4b`) and PR #563 (BUG-277/280/282, squash `a3be3330`), then promoted by PR #564 (merge `4e923359dfd391206baf6887f3ab4a1e470e3152`). Production deployment `5331520979` succeeded 2026-07-06T15:13:34Z; `https://addictionboards.com/` and the Vercel deployment URL returned HTTP/2 200, and the checked Vercel runtime logs contained no errors. DEBT-436 was resolved as a docs-only living-spec correction; DEBT-438 remains active with reduced scope. Prior update 2026-07-05 — BUG-277..282 were filed from the post-Track-A adversarial database-seam review, with companion debt DEBT-437..441. The same review re-verified the Track A core as sound: transactional dual writes, single-transaction REPEATABLE READ finalize, constraint coverage, and the `details` wire contract. Earlier historical updates remain below.
...
| Bug | Resolution | Production proof |
|-----|------------|------------------|
| [BUG-302](../_archive/bugs/bug-302-stale-report-completion-closes-newer-dialog.md) | Dialog submission-continuation ownership shipped in PR #663 (`feb7652e`) and was hardened for component removal in PR #666 (`7cc09e91`). | Promo #665 (`fd2e6fc8`); main CI 29652695750; main/dev tree `3964a0e8`; site HTTP 200. |
| [BUG-303](../_archive/bugs/bug-303-concurrent-start-abandon-retires-still-running-key.md) | Per-key claim settlement and owner-gated retirement shipped in PR #664 (`27621e37`) plus PRs #666/#667/#668/#669/#671. | Promo #665 (`fd2e6fc8`); main CI 29652695750; main/dev tree `3964a0e8`; site HTTP 200. |

## Parked (accepted-risk)

No findings are parked. Under the terminal-close rule, only a confirmed P4
finding belongs here; an empty table does not create work.

| ID | Title | Priority | Accepted risk |
|----|-------|----------|---------------|
| — | None | — | — |

**Fix-wave-4 close (2026-07-16):** PR #656 → promo #658 resolved BUG-301; PR #657 → promo #659 resolved BUG-300. Each fix PR and each promo received CodeRabbit formal APPROVED review on its exact final head after every posted rate-limit cooldown was honored in full. Final main `53ef2e2f` and dev were byte-identical at tree `b40078a8`; main CI run 29459937203 passed the complete pipeline and deploy, and a fresh close-time request to `addictionboards.com` returned HTTP/2 200. Both docs are resolved and archived.

Before archival, the combined diff `ade71553...53ef2e2f` (13 changed files; four expected commits and no unrelated lineage) was audited through independent correctness, concurrency/race, idempotency-lifecycle, cross-batch, test-hygiene, and register-integrity lenses. Eighteen raw candidates deduplicated to nine; three survived 3-verifier panels and direct production-path probes, six were refuted, none was above P4, and none was revert-grade:

| Bug | Family | Priority | Summary |
|-----|--------|----------|---------|
| [BUG-302](../_archive/bugs/bug-302-stale-report-completion-closes-newer-dialog.md) | Question feedback / dialog ownership | P4 | BUG-301 fenced the request-token slot, but the report dialog still let an obsolete submit completion close and reset a newer reopened form. **Resolved 2026-07-18** (PR #663 + hardening PR #666, promo #665; production-verified). |
| [BUG-303](../_archive/bugs/bug-303-concurrent-start-abandon-retires-still-running-key.md) | Practice / session start / recovery ownership | P4 | BUG-300's uncertainty was local to one Start call, so abandoning a refreshed session could retire the key for a different still-running request. **Resolved 2026-07-18** (PR #664 + five hardening rounds, promo #665; production-verified). |

Companion filing: [DEBT-459](../_archive/debt/debt-459-test-probe-button-rule-scope-ambiguity.md) (resolved 2026-07-17 — the live frontend guidance now states the deliberate production-scanner exception for test-only hook/state-machine probes while preserving the production Button mandate).

**Fix-wave-3 close (2026-07-14):** PR #646 → promo #648 resolved BUG-296; PR #651 → promo #649 resolved BUG-297; PR #647 plus follow-up #650 → promo #649 resolved BUG-298; and PR #652 → promo #653 resolved BUG-299. Each fix PR received CodeRabbit formal APPROVED review on its exact final head. Final main `76de5ba3` and dev were byte-identical at tree `c4fc04e0`; main CI run 29383288782 passed the complete test/build/E2E pipeline and deploy, and a fresh close-time request to `addictionboards.com` returned HTTP/2 200. All four docs are resolved and archived.

Before archival, the combined diff `ba457afd...76de5ba3` (65 files; BUG-296/297/298/299, the BUG-298 follow-up, and promos) was audited through independent concurrency/race, idempotency-lifecycle, error-classification, cross-batch, test-integrity, architecture, and security lenses. Three candidates survived source-level deduplication and were each confirmed by a 3-verifier panel; none was above P3 and none was revert-grade:

| Bug | Family | Priority | Summary |
|-----|--------|----------|---------|
| [BUG-300](../_archive/bugs/bug-300-concurrent-start-refresh-retires-pending-key.md) | Practice / session start / idempotency determinacy | P4 | BUG-299's loaded-null retirement could discard the start key after `ConcurrentRequestInProgress` while the original same-key request was still running. **Resolved 2026-07-16** (PR #657, promo #659; exact typed retirement guard; production-verified). |
| [BUG-301](../_archive/bugs/bug-301-stale-feedback-completion-clobbers-newer-request-key.md) | Question feedback / client key ownership | P3 | A stale rating/report completion could overwrite a newer possibly-committed request's preserved key and permit a duplicate append-only write. **Resolved 2026-07-16** (PR #656, promo #658; generation-CAS token slots; production-verified). |

Companion filing: [DEBT-458](../_archive/debt/debt-458-stripe-webhook-acknowledgement-error-causality.md) (P4 — resolved in FW-2: the separate acknowledgement transaction now owns its own failure causality). The previously refuted BUG-298 attempt-reset-fence claim was rechecked with no new evidence and was not refiled. BUG-300 and BUG-301 were subsequently resolved in fix wave 4; their successors BUG-302 and BUG-303 were resolved in the fix-wave-5 terminal close.
...
| Bug | Family | Priority | Summary |
|-----|--------|----------|---------|
| [BUG-296](../_archive/bugs/bug-296-post-deletion-subscription-webhooks-fail-users-fk.md) | Stripe webhook / event ledger / account deletion | P3 | After a committed account deletion, subscription webhooks carrying the deleted local UUID failed the users FK as generic `INTERNAL_ERROR`, producing incident-shaped retries despite correct terminal state. **Resolved 2026-07-14** (PR #646, promo #648; exact-FK classification + fresh-transaction acknowledgement; production-verified). |
| [BUG-297](../_archive/bugs/bug-297-checkout-success-version-cas-exhaustion-uncaught-conflict.md) | Billing / checkout success / observation-version fence | P3 | The BUG-287 CAS loop could surface an uncaught exhaustion `CONFLICT` on `/checkout/success` while concurrent webhooks had already written the correct paid row. **Resolved 2026-07-14** (PR #651, promo #649; typed-exhaustion-only local-row fallback; production-verified). |
| [BUG-298](../_archive/bugs/bug-298-preserved-keys-unbound-on-submit-mark-bookmark-surfaces.md) | Idempotency client key lifecycle | P3 | BUG-295's request-identity binding had not reached submit, mark, bookmark, or the feedback context guard. **Resolved 2026-07-14** (PR #647 + follow-up #650, promo #649; neutral fingerprint-bound key lifecycle + widened guard; production-verified). |
| [BUG-299](../_archive/bugs/bug-299-recovery-panel-external-resolution-stale-state-dead-ends.md) | Practice / session start / recovery | P3 | Externally resolved incomplete sessions left stale recovery state and an unretired start key. **Resolved 2026-07-14** (PR #652, promo #653; typed authoritative refresh + capture-then-absence retirement; production-verified). |

Companion filing from the same review: [DEBT-457](../_archive/debt/debt-457-wave2-determinacy-and-test-hygiene-residues.md) (P4 — Items 1 and 3 resolved; Item 2 closed as ACCEPTED with its revive trigger intact). The two refuted candidates (a claimed `user.updated` fallback lock-order regression and a claimed rollout-window version-fence blind spot) died with quoted-code refutations in the panel record.

**Post-wave-2 review filing (2026-07-14):** the wave-2 idempotency PR #640 (BUG-289/290/291) taught clients to preserve keys across outcome-indeterminate failures; a post-merge adversarial pass over the deployed tree confirmed the missing half of that invariant:

| Bug | Family | Priority | Summary |
|-----|--------|----------|---------|
| [BUG-295](../_archive/bugs/bug-295-preserved-idempotency-keys-replay-across-changed-intent.md) | idempotency client key lifecycle | P3 | Preserved keys are not bound to the request identity they were minted for nor retired once consumed: `withIdempotency` replays completed outcomes before `execute()`, so a changed vote/edited report under a lost-response key silently replays the committed original as "saved" (the PR #640 repository guard reaches only the fenced-claim arm), and the recovery panel's un-scoped abandon key plus never-retired start key recreate BUG-291's same-mount dead-end after a successful abandon. Fix (same PR as the filing): fingerprint-bound feedback request tokens, session-scoped abandon tokens rotated on consumed success, start-key retirement on recovery resolution, wrapper-boundary + browser-sequence regressions. **Resolved 2026-07-14** (PR #642, promo #643; CodeRabbit formal APPROVED on the exact final head; production-verified). |

**Wave-1 fix close (2026-07-11):** three parallel fix batches (PR #628 → promo #630; PR #626 → promo #631; PR #627 → promo #629) resolved and archived BUG-283/284/285/286/292/293 with per-PR CodeRabbit exact-head approval, full local gates, and per-promo production proof. A 6-lens adversarial cross-batch regression review of the combined diff (14 raw → 6 confirmed, none above P3, zero refuted-survivors dropped silently) filed the residue:
...
Companion filings from the same review: [DEBT-455](../_archive/debt/debt-455-fake-user-repository-fidelity-divergences.md) (P4, resolved) and [DEBT-456](../_archive/debt/debt-456-client-conflict-reason-discrimination-gaps.md) (P4, resolved); DEBT-443/449/452/454 received post-wave accuracy annotations.
...
**Methodology:**
- Ran 12 independent finder lenses over the full database seam — schema + migrations 0000-0028, all 15 Drizzle repositories, the idempotency/rate-limiter seams, both webhook controllers, the reconcile/drain crons, the seed pipeline, and the composition root — one lens per book concept: DDIA ch.7 transactions/weak isolation, ch.8 partial failure + webhook redelivery/ordering, the idempotency seam, ch.4 encoding/schema evolution + rollback compatibility, ch.11-12 derived/denormalized state, constraints/integrity (FK delete flows, check gaps), ch.3 storage/indexes vs actual predicates, ops-reliability (migrate-on-deploy, pooling, timeouts), Clean Architecture boundary leaks, Ousterhout module-depth/information-leakage/contract-comments, Farley test-empiricism (fake fidelity, real-Postgres coverage), and multi-tenant authz scoping.
- 59 raw candidates → cross-lens dedup to 49 → per-candidate adversarial verification, each verifier required to re-read the cited code, actively attempt refutation, and dedup against the complete archived register (444 debt + 262 bug slugs) → 41 confirmed in round 1. A completeness critic then named three uncovered areas (the DB-backed rate limiter, the `lib/db.ts` connection-pool seam, and deploy/rollback asymmetry of build-time migrations); targeted round-2 finders on those areas produced 12 more candidates → **50 confirmed total, 10 duplicates correctly matched to settled law** (including the DEBT-437 ACCEPT ruling, BUG-241's migrate-then-build constraint rationale, SPEC-017/DEBT-135 rate-limit rulings, and DEBT-432's pooler prepared-statements investigation), 1 refuted.
- The filing pass initially graded every confirmed finding P3/P4. Cycle B1's independent re-audit raised BUG-284 to P2 after rejecting the proposed identity check: a silent cross-account transfer of entitlement and practice history is an authorization/data-isolation breach even with compound reachability. Cycle B2 raised BUG-288 to P2 because its rare but directly reachable paid-checkout path creates an untracked recurring financial obligation after account deletion. Cycle B3 kept BUG-289..291 at P3 while separating rollback-certain failures from indeterminate outcomes, removing an unmeasured "typically commits" claim, and replacing blanket retry/key-rotation proposals that could duplicate append-only feedback or attempts. Cycle B4 kept BUG-292 at P3 and BUG-293 at P4 after correcting the Drizzle error wrapper, refuting BUG-293's public end-vs-discard trigger, and narrowing it to concurrent account deletion. No finding is P1.
- Every filed bug's load-bearing mechanics were then independently re-verified at source in the orchestrating session before filing (transaction bindings, lock-acquisition orders, idempotency policy wiring, client key-rotation paths, guard arms, schema indexes) — the same trust-but-verify convention as the 2026-06-30 sweep.
- Three previously un-filed follow-up candidates from past post-merge reviews (fake-vs-real error-contract divergence, transient-error caching in the idempotency layer, conflict-seam test-hygiene gaps) were all confirmed at source and are now formally filed (BUG-289/BUG-290, DEBT-443/DEBT-451).

**11 new bugs filed (BUG-283..BUG-293) + 12 grouped debt items (DEBT-443..DEBT-454, absorbing the 39 debt-class findings — see `docs/debt/index.md`):**

| Bug | Family | Priority | Summary |
|-----|--------|----------|---------|
| [BUG-283](../_archive/bugs/bug-283-clerk-webhook-email-reclaim-dead-in-transaction.md) | Clerk webhook / user provisioning | P3 | The BUG-147 email-reclaim fallback runs its recovery UPDATE on the same postgres-js transaction the failed INSERT just aborted, so `user.updated` for a recreated account 500s deterministically (25P02) through Svix retry exhaustion; the row heals only on the raw-db sign-in path, while the Clerk controller's fresh transaction correctly preserves a durable failed-event record. **Resolved 2026-07-11** (PR #628, promo #630; production-verified). |
| [BUG-284](../_archive/bugs/bug-284-user-upsert-email-reclaim-cross-identity-takeover.md) | User provisioning / identity | P2 | The fallback treats email as identity: with a stale local row, a different person re-registering the freed email can silently inherit Stripe linkage and practice history. If the acting user already owns a row, sign-in reaches an opaque `users_clerk_user_id_uq` CONFLICT; the tx-bound webhook fails earlier through BUG-283 as INTERNAL_ERROR. Safe remediation must never merge identities from email plus old-ID nonexistence alone. **Resolved 2026-07-11** (PR #628, promo #630; production-verified). |
| [BUG-285](../_archive/bugs/bug-285-stripe-webhook-markfailed-on-aborted-transaction.md) | Stripe webhook / event ledger | P3 | A direct pg error after a persistable subscription write leaves the shared transaction aborted, so the catch's in-transaction `stripeEvents.markFailed` dies with 25P02 and no durable failure state commits. The secondary query becomes the top-level error (the original may survive in serialized query params); current production reachability requires pre-existing compatible ownership drift or another direct-statement abort. **Resolved 2026-07-11** (PR #626, promo #631; production-verified). |
| [BUG-286](../_archive/bugs/bug-286-webhook-vs-reconcile-lock-order-deadlock.md) | Stripe webhook / checkout success / reconcile cron | P3 | The webhook and checkout-success eager-sync transactions acquire advisory-user-lock → `stripe_customers` row lock; reconcile Phase 4 acquires them in the opposite order — an AB-BA cycle, so either subscription-first writer for user U landing inside U's Phase-4 window can deadlock (40P01) and one victim aborts. A webhook victim then enters BUG-285's secondary-25P02/top-level-error-replacement path; a checkout-success victim loses that request's eager sync. **Resolved 2026-07-11** (PR #626, promo #631; residual fourth-writer cascade inversion filed as BUG-294; production-verified). |
| [BUG-287](../_archive/bugs/bug-287-reconcile-cron-stale-snapshot-overwrites-newer-webhook-state.md) | Billing / reconcile cron | P3 | `shouldPersistSubscriptionWrite` unconditionally persists same-identity writes — no observation/version fence — so the daily cron's Phase-4 write of its stale Phase-1 snapshot can revert a webhook-applied cancellation or renewal that landed in the window. Entitlement can be wrong in either direction until a later webhook or successful reconciliation; the daily cron is a repair opportunity, not a 24-hour upper bound. **Resolved 2026-07-12** (PR #635, promo #636; migration 0029 observation-version CAS; production-verified). |
| [BUG-288](../_archive/bugs/bug-288-checkout-completes-after-account-deletion-orphan-billing.md) | Billing / account deletion | P2 | A paid Checkout Session opened before Clerk `user.deleted` remains completable for Stripe's default 24-hour lifetime after the deletion flow performs only point-in-time subscription cancellation. Completion creates a Stripe-only subscription whose local upsert repeatedly fails the users FK (23503); no current app path sees it, so recurring charges can continue without an application-owned stop. **Resolved 2026-07-13** (PR #634, promo #638; deletion-owned Stripe customer lifecycle; deferred FK-23503 residue filed as BUG-296; production-verified). |
| [BUG-289](../_archive/bugs/bug-289-idempotency-caches-transient-errors-billing-bookmark-feedback.md) | Idempotency / billing / bookmarks / feedback | P3 | Five actions omit `shouldCacheError`, so the wrapper caches every execute failure for 24h; exhausted Stripe retries are raw errors normalized to `INTERNAL_ERROR`, and mount-scoped or success-only client keys replay the cached failure. Fixes must preserve indeterminate keys and make append-only rating/report writes request-idempotent before broad retries. **Resolved 2026-07-14** (PR #640, promo #641; migration 0030 feedback request tokens; identity binding completed by BUG-295; production-verified). |
| [BUG-290](../_archive/bugs/bug-290-state-write-idempotency-caches-transient-internal-error.md) | Idempotency / practice state writes | P3 | The state-write policy caches every error except typed `StateChangedConcurrently`; a rollback-certain non-retried failure such as `57014` is stored as `INTERNAL_ERROR`, so reused submit/mark keys replay it. Quick/filtered Practice, standalone questions, Tutor/session-review submit, and Exam mark are affected; active Exam draft navigation and finalize are not. **Resolved 2026-07-14** (PR #640, promo #641; SQLSTATE cause-chain classification at the transaction owners; production-verified). |
| [BUG-291](../_archive/bugs/bug-291-session-start-key-rotation-on-timeout-conflict-dead-end.md) | Practice / session start | P3 | `startSession` rotates its key after a 15s client timeout even though the uncanceled request can still finish within the route's 30s server budget. If it commits, the fresh-key retry re-executes into the incomplete-session CONFLICT while the mount-only resume panel remains stale; production frequency is unmeasured. **Resolved 2026-07-14** (PR #640, promo #641; determinacy-branched key lifecycle + recovery-panel refresh; key retirement completed by BUG-295; production-verified). |
| [BUG-292](../_archive/bugs/bug-292-discard-unowned-serialization-failure-internal-error.md) | Practice / discard | P3 | `discard()` self-opens a REPEATABLE READ transaction with no 40001/40P01 catch, mapping, or retry, outside `runPracticeSessionStateWriteTransaction`; racing an expired-exam finalizer can abort its blocked DELETE with a Drizzle query error carrying `40001`, which the action normalizes to generic INTERNAL_ERROR although a bounded fresh-transaction retry would converge to guarded no-op success. **Resolved 2026-07-11** (PR #627, promo #629; production-verified). |
| [BUG-293](../_archive/bugs/bug-293-end-split-prereads-phantom-corruption-error.md) | Practice / end session | P4 | Standalone `end()` reads the session and state rows in separate autocommit snapshots, so a concurrent Clerk account-deletion cascade can produce a false "missing normalized question state" INTERNAL_ERROR action result instead of NOT_FOUND. Public end-vs-discard is mode-excluded, and transaction-bound exam finalize already inherits one RR snapshot. **Resolved 2026-07-11** (PR #627, promo #629; production-verified). |
**Coverage ledger:**

| Surface | Result |
|---------|--------|
| Transactions & isolation (webhooks, practice writes, cron) | BUG-283/285/286/292/293 filed; all write-skew re-derivations correctly deduped to the DEBT-437 ACCEPT ruling |
| Webhook/cron lifecycle & causality | BUG-287/288 filed; event-ledger lifecycle filed as DEBT-449 |
| Idempotency seam | BUG-289/290/291 filed; durability/evolution + prune contention filed as DEBT-443/DEBT-444 |
| Identity / user provisioning | BUG-283/284 filed: BUG-283's integration proof covers only the raw-db path; BUG-284's unit/fake tests encode reassignment and lockout mechanics but never prove that two Clerk IDs belong to the same human |
| Schema constraints & migration pipeline | DEBT-445/DEBT-451 filed; migrate-then-build coupling and question-delete cascade candidates correctly deduped to BUG-241/BUG-028 rulings |
| Query performance, pooling & session config | DEBT-450/DEBT-447 filed; prepared-statements-vs-pooler correctly deduped to DEBT-432 |
| Rate limiting | DEBT-448 filed (retention, unmetered practice writes, health-probe ordering); webhook-IP-bucket and health-cost candidates correctly deduped to DEBT-135/SPEC-017 rulings |
| Ops / local tooling blast radius | DEBT-446 filed (bare migrate/seed `.env.local` fallback; unattended prod seeding) |
| Observability, contracts & duplication | DEBT-452/DEBT-453/DEBT-454 filed |
| Authz scoping & PII export | Clean apart from the DEBT-452 logging seam — repository reads verified user-scoped; the feedback export's flag-gated PII posture correctly deduped to the documented SPEC-041 design |

**Sweep (2026-06-30) — fresh-ground P0-P4 sweep after confirming the practice engine is fully swept, then adversarially audited before commit:**

**Methodology:**
- Read `docs/bugs/index.md` end to end (all prior audits, the false-positive registries, and every "Refuted candidates deliberately NOT filed" section) plus `docs/debt/index.md`'s Active/Resolved/Frontend-Debt tables to map exactly what had already been checked: practice/exam/tutor (timer, scoring, draft-save, finalization, abandon, resume, expiry — BUG-251..258), bookmarks (BUG-264, BUG-259), feedback/rating/export (BUG-250, BUG-260 + completion), history pagination (BUG-261), billing/Stripe state machine (Audit #21, BUG-263), Clerk lifecycle/webhooks, entitlement gating, cron/reconciliation, authz/data-isolation, DB/ORM infra, input/output/XSS/CSV, and 55 resolved frontend (`FE-NNN`) design-system/a11y items — all confirmed clean or already fixed, with the practice engine specifically exhausted across 10+ rounds.
- Deliberately pivoted to areas never named as a dedicated sweep target: auth/Clerk sign-in/sign-up + redirect/route-protection, the cross-cutting shared UI infrastructure used by multiple features (`app/(app)/app/shared/**` and equivalents), the interaction seams between the last ~10 days of shipped fixes (BUG-264/263/260/259/DEBT-424), marketing/public pages + quick-practice + the standalone question page, and an end-to-end accessibility pass over the practice/exam/tutor interactive flow (only ever touched piecemeal before via individual `FE-NNN` items).
- Ran 5 independent investigations in parallel, each required to trace a real entry point to a real sink with file:line evidence, adversarially refute its own candidates before reporting, and grade severity using this index's P0-P4 scale. One reported candidate (a `SessionBreakdownList`/`getReviewStatusLabel` blank-or-wrong status label when `isCorrect` is `null`) was traced all the way to its data source and **not filed**: the `isOmitted` half of the condition is mathematically excluded by its own definition (it requires `latestIsCorrect === false`), and the `isAnswered` half requires `latestSelectedChoiceId`/`latestIsCorrect` to diverge, but every write path (`finalizeDraftAnswer`) sets both atomically together — a latent fragility with no test pinning it, but no demonstrated production trigger.
- **Second pass, on request: 4 independent adversarial audits (one per filed doc or doc-pair), each with no authorship stake, tasked with re-verifying every citation and — the higher bar — stress-testing whether the root-cause analysis and proposed fix were architecturally correct, not just plausible.** This caught real errors the first pass missed, most importantly on the radio-choice bug now filed as BUG-274: the original filing claimed Exam mode was "correctly protected" and graded the bug P2; the audit found `selectChoice(choiceId)` runs *before* the `mode === 'exam'` guard (`use-practice-session-question-flow.ts` line 475 vs. 477 at current HEAD), so Exam mode's local `selectedChoiceId` is overwritten by arrow-key browsing too — just without the immediate grading call — and that value is what the exam's existing autosave persists as the graded draft answer at finalization, with no confirmation step. Independently re-verified against the live tree before accepting; severity raised to P1. Also corrected: the exam-timer bug's reachability was overstated (screen-reader-only, not "sighted or AT" — the announcement lives in a genuinely `sr-only` span) and its severity lowered to P4 to match this repo's own precedent for the announcement-timing category; the focus-management bug's root cause was misattributed (the real gap is a "View Summary" button click inside the already-focus-managed `PostExamReviewView`, not the exam-submit confirmation dialog); the pricing-CTA fix was missing 3 more pinned tests and overstated "Manage Billing" as always-visible; the stale-counts fix sketch had the wrong status count (3, not 5); and the not-found-metadata fix would have broken this app's page-title naming convention. Full detail is in each doc's Root Cause/Impact/Proposed Fix sections as shipped — this paragraph is the durable record that the correction happened and why.

**6 new bugs filed (BUG-271..276) + 1 new debt item (DEBT-436)** *(BUG-271/272/273/274/275 resolved and archived 2026-07-08 after the PR #587 production proof; BUG-276 resolved and archived 2026-07-08 after the PR #593 production proof — see Last Updated)*:

| Bug | Family | Priority | Summary |
|-----|--------|----------|---------|
| [BUG-276](../_archive/bugs/bug-276-quick-practice-status-counts-go-stale.md) | Practice / Quick Practice / UI state | P3 | Resolved 2026-07-08 after production proof: Quick Practice status-count badges now refresh after the user answers or bookmarks a single question |

Also filed [DEBT-436](../_archive/debt/debt-436-stale-spec-docs-claim-user-created-webhook-handling.md) (P3, resolved 2026-07-06): `master_spec.md`/`master_spec_part2.md` claimed the Clerk webhook handles `user.created`, contradicting both the live implementation and this repo's own correct `docs/vendor-docs/clerk.md` (lazy upsert-on-first-request design, verified race-free during this sweep).
...
2. Renumbered 2026-06-30 to BUG-268..273/DEBT-430 to clear `chore/legacy-audit`'s (PR #537) then-current BUG-265..267/DEBT-425..429.
3. Renumbered again 2026-07-02 — BUG-268/269/270/DEBT-430 → BUG-274/275/276/DEBT-435 — because that branch grew further while still unmerged and its own Track-A pre-merge sweep had by then independently consumed the entire BUG-265..270/DEBT-425..434 range. BUG-271/272/273 needed no change.
4. PR #537 merged 2026-07-02 (`9711ca48`); this branch rebased cleanly on top at that point, no further drift.
5. Renumbered a third time 2026-07-04 — DEBT-435 → **DEBT-436** — after `chore/legacy-audit`'s post-merge debt-resolution follow-up work (landed on `dev` through 2026-07-04) independently filed and archived its own unrelated DEBT-435. **Bugs were never touched by this third round** — `dev`'s Next Bug ID stayed BUG-271 throughout, confirmed live each time, so BUG-271..276 are unchanged since step 3.

Every one of these was confirmed against the other branch's actual live registry file content immediately before renumbering, not assumed or inferred from commit messages.

**Residual risk on BUG-271 — resolved, did not materialize:** this note previously flagged that `chore/legacy-audit`'s stated next-free bug ID (BUG-271) sat exactly on this branch's own BUG-271 (focus-management doc, filed 2026-06-30, unmoved since). PR #537 has since merged (`9711ca48`, 2026-07-02) without ever filing a BUG-271 of its own — confirmed directly against the merged registry (see the Active Bugs section below) before this branch was rebased on top. No collision occurred; kept here as the closed-out record of the check that mattered.

**Coverage ledger:**

| Surface | Result |
|---------|--------|
| Auth (Clerk) sign-in/sign-up, route protection, redirect logic | Found BUG-275 (pricing CTA redirect-context loss). Deep-link query-param preservation through `proxy.ts`'s `auth.protect()`, session-death-mid-task handling, two-tab sign-out, open-redirect protection (delegated to Clerk's own SDK), and the Clerk-webhook-vs-DB-provisioning timing race (non-issue: lazy synchronous upsert, not webhook-dependent) all confirmed clean. Also surfaced DEBT-436 (stale spec docs). |
| Cross-cutting shared UI infrastructure (`app/(app)/app/shared/**` + equivalents) | Investigated a `SessionBreakdownList`/`getReviewStatusLabel` blank-row candidate in depth and determined it is not currently reachable (see Methodology) — not filed. Bookmark double-click idempotency, rating double-submit guards, question-page reentrancy, and the `historySeq` dead-code fallback (BUG-152 precedent) all confirmed clean/already-accepted. One dead export (`getLoadStateErrorMessage`, zero call sites) noted but too trivial to file. |
| Interaction seams between the last ~10 days of shipped fixes (BUG-264 × DEBT-424, BUG-263 × Audit #21, BUG-260 × BUG-264) | **Clean.** `bookmark:setBookmark` is confirmed on the DEBT-424-hardened idempotency/rate-limit wrapper; the BUG-263 entitlement-tier comparator is confirmed strictly additive on top of BUG-242/243's preserved terminal-overwrite rejection, with all 6 Audit #21 scenarios re-verified against current guard code; bookmarks and feedback-context validation have zero coupling (refuted, not applicable). Verified via direct commit diffs plus 266 targeted tests across 9 invocations, not just static reading. |
| Marketing/public pages + quick-practice entry + standalone question page | Found BUG-276 (stale status counts) and BUG-273 (missing 404 metadata). Standalone question-page authorization (entitled + published-only, same level as every other question pool) and the "Try Again" retry/idempotency flow both confirmed clean; nonce/CSP plumbing, `historySeq`, and the BUG-264 bookmark contract all reconfirmed intact, not regressions. |
| Accessibility (practice/exam/tutor interactive flow, end-to-end) | Found BUG-274 (radio arrow-key auto-commit, all three modes — the most significant finding this round), BUG-271 (no focus management on the real session-summary transition), and BUG-272 (exam-timer milestone skip, screen-reader only). Re-verified all previously-shipped `FE-NNN` a11y items in this flow (toggle `aria-pressed`, fieldset grouping, navigator landmark/`aria-current`/`aria-controls`, `AlertDialog` cancel-path focus-trap, dark-mode contrast tokens) remain intact with no regressions. Confirmed the previously-parked "answer-verdict live-region announcement timing" item (`components/question/feedback.tsx`, noted 2026-06-25) is still genuinely open and unchanged since that note — not re-filed, still tracked as a known parked item under separate owner review. |

**Refuted candidates deliberately NOT filed (with why):**
- **`SessionBreakdownList`/`getReviewStatusLabel` blank-or-"Unanswered" label when `isCorrect` is `null`.** Type-permitted in isolation, but traced to its actual data source (`get-practice-session-review.ts`) and found unreachable: the `isOmitted` branch requires `latestIsCorrect === false` by construction, and the `isAnswered` branch's only population path (`finalizeDraftAnswer`) writes `latestSelectedChoiceId` and `latestIsCorrect` atomically together. No demonstrated trigger.
- **`getLoadStateErrorMessage` dead export.** Zero production call sites confirmed by grep; real but too trivial (unused-export cleanup, no behavioral risk) to file as debt.
- **Bookmark/rating double-click races, question-page submit reentrancy.** All mechanically ruled out — synchronous idempotency-key capture before the first `await`, `disabled` props gated on synchronous state, and/or a "latest-request-wins" staleness check absorb every theoretical race without state corruption.
- **`historySeq`-based navigator showing neutral status for answered questions.** Confirmed dead code, already intentionally accepted as a fallback per the archived BUG-152 fix — not a regression, not re-filed.
- **Custom `role="radiogroup"`/`role="radio"` (roving tabindex) as the fix for BUG-274.** Investigated directly against the live W3C WAI-ARIA Authoring Practices Guide: the Radio Group pattern specifies identical arrow-key-selects behavior to native radios outside a `role="toolbar"` context, so this would reproduce the defect via ARIA rather than fix it. Confirms the shipped fix direction (decouple select from commit, keep native radios) instead of refuting it.
...
**Latest archival (2026-06-16; post-merge regression proof added 2026-07-02) — BUG-241 resolved (deploy migration enforcement):**
- BUG-241 (P2) verified fixed and archived to `docs/_archive/bugs/`. The fix adds `"buildCommand": "pnpm db:migrate && pnpm build"` to `vercel.json`, so every git-triggered Vercel deploy applies checked-in Drizzle migrations to its environment-scoped Neon branch before serving, failing the build closed on migration error. Merged via PR #453 (squash `ff46fbda` → `dev`) and promoted to `main` via PR #454 (merge `daed8479`); `main` and `dev` trees are identical. Verified live on real Vercel builds: the Preview deploy migrated Neon `dev` and the Production deploy migrated Neon `main`, each logging `[✓] migrations applied successfully!` before `next build`. Re-verified on PR #537's schema-bearing squash `9711ca48`: Production deploy `dpl_7WFJk7P56byivGi7dynB52TXecyz` ran the same Build Command before serving, and both deployed ledgers are current through `0025_worthless_junta`. A `vercel.test.ts` guard now pins the Build Command in the required unit suite. Neon branch isolation (Production vs shared Preview/Development) was confirmed by a value-free host comparison. `pnpm db:seed` (content) remains a documented manual step. This closed the active register at that point in the timeline.
...
|----|-------|----------|--------|---------|
| [BUG-304](./bug-304-practice-session-start-no-navigation.md) | Practice session Start can click without navigation or error | P3 | Open | The original no-navigation event remains unproven; promotion runs `32903291145` and `32912478032` proved two navigation-observation orderings in the diagnostic helper, and the second red-first follow-up awaits review and production proof. |
| [BUG-305](./bug-305-clerk-dev-browser-credential-in-ci-logs.md) | Clerk dev-browser credential appears in public CI logs | P2 | Open | Three successful public CI runs logged an unmasked TEST-instance dev-browser credential from Clerk's teardown warning; the first mitigation failed hosted, and its red-first method-restoration correction awaits exact-head CI, review, completed revocation or verified expiry of exposed sessions, and separate historical-log containment. |
| [BUG-306](./bug-306-required-e2e-clerk-session-loss-and-accumulation.md) | Required E2E can lose Clerk session and accumulates sessions | P3 | Open | A fresh session redirected to hosted sign-in once and did not reproduce in eleven retry-free cases; the proven 47-sign-in/no-sign-out lifecycle and 180-second diagnostic hang have red-first branch-local fixes awaiting exact-head CI, review, promotion, and an owner disposition for the historical TEST-session backlog. |
...
PR #537 merged 2026-07-02 (`9711ca48`) with no other active bugs from that branch. BUG-276 from the 2026-06-30 sweep was resolved and archived 2026-07-08 after the PR #593 production proof; BUG-271/272/273/274/275 from that sweep were resolved and archived 2026-07-08 after the PR #587 production proof; BUG-277/278/279/280/282 were resolved and archived after PR #562/#563 plus the PR #564 production proof; BUG-281 from the 2026-07-05 post-Track-A adversarial database-seam review was resolved and archived after PR #596 plus the PR #597 production proof.

**2026-07-09 — 11 bugs filed from the DDIA-lens adversarial database-seam sweep: BUG-283..BUG-293 (BUG-284 and BUG-288 at P2, eight at P3, BUG-293 at P4).** Full methodology, per-bug table, and coverage ledger are in the "Latest sweep (2026-07-09)" section near the top of this file; each bug doc carries its verified reproduction interleaving, severity rationale, and proposed fix. Cycle B1 corrected BUG-284's identity-transfer impact; Cycle B2 raised BUG-288 for its untracked recurring financial obligation; Cycle B3 corrected the idempotency trio's client scopes, platform-timing claims, and retry designs without changing their P3 grades; Cycle B4 corrected BUG-292's Drizzle-wrapped error shape and determinacy-aware retry plan, and narrowed BUG-293 from an impossible public end-vs-discard race to the reachable Clerk account-deletion cascade without changing its P4 grade. The remaining P3/P4 items are narrow webhook failures, entitlement drift repaired by later webhooks or successful reconcile runs (not time-bounded), cached-transient-error retry wedges, and race-window misclassifications. **Next Bug ID is BUG-295.** (BUG-294 was filed 2026-07-11 by the wave-1 close review; BUG-295 followed 2026-07-14 from the post-wave-2 review; the 2026-07-14 wave-2 close resolved and archived BUG-287..291/294/295 and filed BUG-296..299; the 2026-07-14 wave-3 close resolved and archived BUG-296..299 and filed BUG-300/301, making the next free ID BUG-302.)
