...
| Bug | Family | Priority | Summary |
|-----|--------|----------|---------|
| [BUG-294](./bug-294-user-deletion-cascade-inverts-subscription-lock-order.md) | Clerk webhook / account deletion / Stripe billing | P3 | The `user.deleted` flow's `ON DELETE CASCADE` is a fourth subscription writer hard-wired to the inverse of the BUG-286 canonical lock order (`stripe_customers` before `stripe_subscriptions`, no advisory lock) — 40P01 reproduced on real Postgres against a concurrent subscription writer; the reconcile-vs-cascade pair was newly created by PR #626's Phase-4 reorder. Fix: acquire the subscription writers' advisory lock inside the deletion transaction; coordinate with BUG-288. |
