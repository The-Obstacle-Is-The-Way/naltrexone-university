...
**Last Updated:** 2026-07-05 — **BUG-277..282 filed from the 2026-07-05 post-Track-A adversarial database-seam review** (five independent DDIA-lens reviewers — isolation/locking, idempotency, schema/constraints, derived-data consistency, client-server conflict contract — every finding line-level verified against `e3853656` before filing; companion debt DEBT-437..441, see `docs/debt/index.md`). Three P2s: BUG-277 (exam-expiry finalize carrying a stale loaded-question draft flush >15s past the deadline — even with `selectedChoiceId: null` — aborts the entire finalization with a reason-less CONFLICT → false full-screen "Practice session has not ended", and drops the at-expiry answer when one was selected; one-shot client guards), BUG-278 (end/discard pass no appropriate `shouldCacheError` policy and the abandon surface's idempotency key is the raw `sessionId` — one transient failure, including a real discard-vs-finalize AB-BA deadlock, bricks session abandonment for the 24h TTL), BUG-279 (the shared idempotency wrapper conflates a failed `storeResult` attempt with a failed execution when the business effect committed but the idempotency success row was not recorded, durably caching failure-after-success). Three P3s: BUG-280 (double-finalize race loser surfaces the attempts `23505` as a reason-less "already answered" CONFLICT that the policy caches), BUG-281 (seed re-import unconditionally rewrites `choices.is_correct` under graded history → stored grade contradicts the recomputed answer key on completed-session/post-exam review), BUG-282 (tutor session ended in another tab leaves the losing tab in a reason-less CONFLICT dead-end loop). The same review re-verified the Track A core as sound — transactional dual writes, single-transaction REPEATABLE READ finalize, constraint coverage, `details` wire contract — these are seam defects around the core, not cracks in it. Prior update 2026-07-04 (**Third numbering collision, DEBT only: DEBT-435 → DEBT-436** — after PR #537 merged (`9711ca48`, 2026-07-02) and this branch rebased cleanly on top, `chore/legacy-audit`'s own follow-up debt-resolution work (2026-07-02 through 2026-07-04, landed on `dev` at `22a24ad2`) resolved+archived DEBT-425..430/433/434 *and*, in the process, independently filed and immediately archived its own unrelated **DEBT-435** ("practice session conflict and test hygiene follow-ups") — the exact number this branch's stale-spec-docs debt item already used. Confirmed directly against `dev`'s live registry (`Next Debt ID: DEBT-436`, Active table down to only DEBT-337/DEBT-414) before renumbering. **BUG-271..276 were unaffected — `dev`'s Next Bug ID is still BUG-271, confirmed live, zero bug-side drift across all three rounds.** Renumbered the one debt doc to DEBT-436 and rebased again; see the "Numbering note" below for the full three-round history. Prior update 2026-07-02 (from `chore/legacy-audit`'s own history) — **PR #537 shipped; deploy migration gate re-verified on the schema-bearing release** — BUG-241 remains resolved: Production deploy `dpl_7WFJk7P56byivGi7dynB52TXecyz` for squash `9711ca48` ran `pnpm db:migrate && pnpm build`, logged migration success before `next build`, and both deployed Neon ledgers are current through `0025_worthless_junta`. `vercel.test.ts` now pins the Build Command so removing or reordering the migration step fails the required unit suite. PR #537 branch-local blockers BUG-265..270 stayed fixed before merge; BUG-269 remains invalidated. Prior update 2026-07-02 (from this branch's own history) — PR #537 merged to `main`/`dev` as `9711ca48`; this branch rebased cleanly on top with no drift at that point. Prior update 2026-07-01 — **PR #537 Track A pre-merge blockers resolved, then a systematic post-fix sweep filed 3 more and one was invalidated on second-opinion verification** — BUG-265 remains archived as self-resolved by `aab81ca1` / migration `0024_needy_jimmy_woo.sql`; BUG-266 is fixed by extending the seed choice-deletion guard to normalized practice-session state references; BUG-267 is fixed by opening the composition-root practice-session-state write transactions at `repeatable read` and proving the real driver isolation. A follow-up DDIA-style audit of the same branch then filed BUG-268 (CAS retry loop doesn't handle `repeatable read` serialization failures), BUG-269 (stale finalize snapshot clobber) and BUG-270 (seed choice-reorder can violate the sort-order unique index and abort the rest of the seed batch). BUG-269 was invalidated against current HEAD: the stale window reaches BUG-268's raw `40001` failure mode under repeatable read, not silent data loss. BUG-268 and BUG-270 are now fixed and archived under the branch-local pre-merge convention below; none of BUG-265..270 shipped to users before resolution.)
...
**Latest sweep (2026-06-30) — fresh-ground P0-P4 sweep after confirming the practice engine is fully swept, then adversarially audited before commit:**
...
**6 new bugs filed (BUG-271..276) + 1 new debt item (DEBT-436):**

| Bug | Family | Priority | Summary |
|-----|--------|----------|---------|
| [BUG-274](bug-274-radio-choice-arrow-key-auto-commits-answer.md) | Practice / question answering / accessibility | P1 | Arrow-key navigation between native radio answer choices records (and in Quick Practice/Tutor, auto-submits and locks) an unintended choice in all three modes — Exam mode is not exempt, just silent |
| [BUG-275](bug-275-pricing-cta-drops-redirect-context-unauthenticated.md) | Billing / pricing / auth redirect | P3 | Pricing "Subscribe" CTA redirects unauthenticated clicks to a bare `/sign-up`, losing the chosen plan and return destination ("Manage Billing" has the same defect, narrower reach) |
| [BUG-276](bug-276-quick-practice-status-counts-go-stale.md) | Practice / Quick Practice / UI state | P3 | Quick Practice status-count badges (e.g. "Unanswered (12)") never refresh after the user answers or bookmarks a single question |
| [BUG-271](bug-271-no-focus-management-on-session-summary-transition.md) | Practice / session lifecycle / accessibility | P3 | No DOM focus management when reaching Session Summary (manual Tutor end, timer expiry, or the "View Summary" button after exam submission) |
| [BUG-272](bug-272-exam-timer-milestone-announcements-can-skip.md) | Practice / exam timer / accessibility | P4 | Exam-timer 5-minute/1-minute/30-second milestone announcements (screen-reader only) can be silently skipped if a backgrounded tab's next check jumps past the exact threshold |
| [BUG-273](bug-273-not-found-missing-page-metadata.md) | Marketing / routing / metadata | P3 | `app/not-found.tsx` is the only page with no per-page `metadata` export, so 404s show the generic site title |

Also filed [DEBT-436](../debt/debt-436-stale-spec-docs-claim-user-created-webhook-handling.md) (P3): `master_spec.md`/`master_spec_part2.md` still claim the Clerk webhook handles `user.created`, contradicting both the live implementation and this repo's own correct `docs/vendor-docs/clerk.md` (lazy upsert-on-first-request design, verified race-free during this sweep).
...
PR #537 merged 2026-07-02 (`9711ca48`) with no other active bugs from that branch. The first 6 rows below are unrelated, from a separate 2026-06-30 sweep (see "Latest sweep (2026-06-30)" above) that was in flight at the same time and renumbered twice to avoid colliding with BUG-265..270 above. BUG-277..282 (rows 7-12) are from the 2026-07-05 post-Track-A adversarial database-seam review (see "Last Updated" at the top of this file):

| Bug | Severity | Component | Summary |
|-----|----------|-----------|---------|
| [BUG-271](bug-271-no-focus-management-on-session-summary-transition.md) | P3 | Practice / session lifecycle / accessibility | No focus management when reaching Session Summary |
| [BUG-272](bug-272-exam-timer-milestone-announcements-can-skip.md) | P4 | Practice / exam timer / accessibility | Exam-timer milestone announcements (screen-reader only) can be silently skipped across a backgrounded tab |
| [BUG-273](bug-273-not-found-missing-page-metadata.md) | P3 | Marketing / routing / metadata | `app/not-found.tsx` missing per-page metadata |
| [BUG-274](bug-274-radio-choice-arrow-key-auto-commits-answer.md) | P1 | Practice / question answering / accessibility | Arrow-key navigation between native radio answer choices records (Quick Practice/Tutor: auto-submits and locks) an unintended choice in all three modes |
| [BUG-275](bug-275-pricing-cta-drops-redirect-context-unauthenticated.md) | P3 | Billing / pricing / auth redirect | Pricing subscribe/manage-billing CTAs drop plan + return context on unauthenticated redirect |
| [BUG-276](bug-276-quick-practice-status-counts-go-stale.md) | P3 | Practice / Quick Practice / UI state | Quick Practice status-count badges never refresh after answering or bookmarking |
| [BUG-277](bug-277-exam-expiry-finalize-stale-flush-aborts-finalization.md) | P2 | Practice / exam finalize / conflict contract | Exam-expiry finalize carrying a stale loaded-question draft flush (>15s past deadline — e.g. backgrounded tab, even `selectedChoiceId: null`) aborts the entire finalization with a reason-less CONFLICT; client shows false "Practice session has not ended", drops any selected at-expiry answer, and the one-shot timer guards never retry |
| [BUG-278](bug-278-end-discard-idempotency-caches-transient-errors-under-fixed-key.md) | P2 | Practice / session lifecycle / idempotency | `endPracticeSession`/`discardPracticeSession` cache all execution errors (no appropriate `shouldCacheError`) and the abandon surface's key is the never-rotating `sessionId` — one transient failure (incl. a real discard-vs-finalize AB-BA deadlock → raw `40P01`) bricks session abandonment for the 24h TTL |
| [BUG-279](bug-279-idempotency-wrapper-caches-error-after-committed-success.md) | P2 | Shared adapters / idempotency wrapper | `executeIdempotent` conflates a failed `storeResult` attempt with a failed execution — if the business effect committed but the idempotency success row was not recorded, the wrapper can cache INTERNAL_ERROR as the outcome of a request that succeeded (failure-after-success replayed for the TTL) |
| [BUG-280](bug-280-double-finalize-race-maps-to-reasonless-conflict.md) | P3 | Practice / exam finalize / attempts | Double-finalize race loser hits the attempts `23505` unique index and surfaces as reason-less CONFLICT "This question has already been answered in this session" (not `AlreadyEnded`), which the idempotency policy then caches under the loser's key |
| [BUG-281](bug-281-seed-reimport-rewrites-answer-key-under-graded-history.md) | P3 | Content pipeline / seed / review integrity | Seed re-import's choice upsert unconditionally rewrites `choices.is_correct` under graded history — stored grades contradict the recomputed answer key on completed-session/post-exam review (fires exactly when an editor fixes a miskeyed question); needs a product decision (disclose / regrade / block) |
| [BUG-282](bug-282-tutor-two-tab-ended-session-dead-end-loop.md) | P3 | Practice / tutor mode / conflict handling | Tutor session ended in another tab leaves the losing tab in a reason-less CONFLICT dead-end loop — `submit-answer`/`get-next-question` lack the `AlreadyEnded` annotation and those client surfaces lack the existing summary recovery; only a full reload escapes |
