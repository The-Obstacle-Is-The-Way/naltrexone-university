...
- [BUG-255](./bug-255-review-submit-screen-stops-exam-timer.md) (P4) filed: the Review & Submit screen disables the active exam timer, so expiry does not auto-finalize; harm is inflated recorded duration only — answers are locked at the deadline (server rejects post-deadline saves), so no score/integrity impact.
...
|-----|----------|---------|
| [BUG-255](./bug-255-review-submit-screen-stops-exam-timer.md) | P4 | Review & Submit screen disables the active exam timer, so a late manual submit inflates recorded duration only — answers are locked at the deadline (server rejects post-deadline saves), no score/integrity impact. |
