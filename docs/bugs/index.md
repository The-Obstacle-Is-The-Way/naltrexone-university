...
| [BUG-305](./bug-305-clerk-dev-browser-credential-in-ci-logs.md) | Clerk dev-browser credential appears in public CI logs | P2 | Open | Three successful public CI runs logged an unmasked TEST-instance dev-browser credential from Clerk's teardown warning; the first mitigation failed hosted, and its red-first method-restoration correction awaits exact-head CI, review, and an owner decision on historical-session/log containment. |
