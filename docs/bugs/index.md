...
**Last Updated:** 2026-06-25 (**BUG-260 completion follow-up in progress** — a re-audit found the shipped feedback-context helper still accepted an owned standalone attempt paired with an unrelated owned session that contained the same question. The completion fix requires a real attempt/session relationship when both IDs are present: direct `attempt.practiceSessionId` match or verified `session_review` retry provenance. No new bug ID consumed; BUG-263 remains reserved. Prior — **resolved + archived BUG-262** (P4) — the daily billing cron ran reconcile and the deleted-account Stripe-cancellation drain in one shared `try`, so a true page-0 reconcile throw (e.g. the `listLocalSubscriptions` query failing — not a caught row-level error) skipped the drain (BUG-246's safety net), leaving a deleted user's Stripe sub billable an extra ~24h. Fix decouples the drain into its own `try`/`catch` with `drainFailed = throw || drain.failed > 0`. Shipped via PR #520 (squash `5aac1d5d`) → promo #521 (merge `f40b1904`); production deploy `dpl_BGQQppuN4vaiozxrocgbuXjrDi7t` verified READY 2026-06-25 (`addictionboards.com` HTTP 200); doc moved to `docs/_archive/bugs/`, the Active Bugs register is empty again.)
...
**Latest follow-up (2026-06-25) — BUG-260 completion (no new bug ID):**
- [BUG-260](../_archive/bugs/bug-260-question-feedback-trusts-client-context-ids.md)'s original fix shipped + prod-verified + archived, but a post-archive re-audit found one residual both-ID context gap that is **active in flight** (see Active Bugs) until this completion merges + prod-verifies: a standalone owned attempt for the feedback question could still be paired with an unrelated owned session that also contained that question. Completion branch `fix/bug-260-feedback-context-completion` tightens `validateFeedbackContext` so both IDs require direct attempt membership in the supplied session or a verified session-review retry (`retryOrigin=session_review` and matching `retrySessionId`). The retry exception is required by the real question-page session-review "Try Again" flow, which creates a standalone retry attempt while feedback still carries the reviewed session id. Regression coverage rejects the incoherent standalone/session pair at the helper level and through both feedback use cases; `BUG-263` remains reserved.
...
|-----|----------|-----------|---------|
| [BUG-260 (completion)](../_archive/bugs/bug-260-question-feedback-trusts-client-context-ids.md) | P4 | Question feedback / analytics metadata | **Completion in flight** (PR #524, branch `fix/bug-260-feedback-context-completion`). The shipped + archived BUG-260 fix still accepted an owned standalone attempt paired with an unrelated owned session containing the same question; the completion tightens `validateFeedbackContext` to require direct session membership or a standalone `session_review` retry. The archived BUG-260 Resolution is updated once this merges + prod-verifies. No new bug ID (BUG-263 stays reserved). |
