...
**2026-07-09 — 11 bugs filed from the DDIA-lens adversarial database-seam sweep: BUG-283..BUG-293 (BUG-284 at P2, nine at P3, BUG-293 at P4).** Full methodology, per-bug table, and coverage ledger are in the "Latest sweep (2026-07-09)" section near the top of this file; each bug doc carries its verified reproduction interleaving, severity rationale, and proposed fix. Cycle B1 re-audit corrected the blanket "no data-corruption" claim: BUG-284 can silently transfer one account's paid entitlement and practice history to another identity. The remaining P3/P4 items are narrow webhook failures, bounded entitlement drift, an orphan-billing race, cached-transient-error retry wedges, and race-window misclassifications. **Next Bug ID is BUG-294.**
