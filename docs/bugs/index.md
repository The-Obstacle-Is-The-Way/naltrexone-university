...
**Last Updated:** 2026-06-28 (**BUG-263 fix in progress** — BUG-263 is now the active billing canonicalization bug. The fix branch adds entitlement-tier-aware subscription canonicalization shared by the reconciliation job and subscription write guard, preserving same-subscription lifecycle transitions while preventing non-entitled duplicate subscriptions from overwriting a current entitled row.)
...
|-----|----------|-----------|---------|
| [BUG-263](bug-263-billing-canonicalization-prefers-non-entitled-duplicate-subscription.md) | P2 | Billing / Stripe Reconciliation / Subscription Canonicalization | Duplicate subscription canonicalization can prefer a later non-entitled row over a current paid-access subscription, revoking app access and canceling the active Stripe subscription |
