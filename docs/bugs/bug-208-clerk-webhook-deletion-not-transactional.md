...
1. **The `user.deleted` flow is a read-then-delete sequence with no lock:** `src/adapters/controllers/clerk-webhook-controller.ts` does `findByClerkId(...)`, then `stripeCustomerRepository.findByUserId(user.id)`, then optional `cancelStripeCustomerSubscriptions(...)`, then `deleteByClerkId(...)`.
2. **Two live code paths can create Stripe linkage after that read:** `src/adapters/controllers/stripe-webhook-controller.ts` inserts `stripe_customers` and `stripe_subscriptions` inside its transaction, and `app/(marketing)/checkout/success/checkout-success-sync.tsx` does the same during checkout-success sync.
3. **The later user delete removes those new local rows atomically:** `src/adapters/repositories/drizzle-user-repository.ts` deletes the user row, and `db/schema.ts` defines `onDelete: 'cascade'` for both `stripe_customers.userId` and `stripe_subscriptions.userId`.
4. **Recovery does not exist after that interleaving:** the reconciliation entrypoint only enumerates surviving local `stripe_subscriptions` rows in `app/api/cron/reconcile-stripe-subscriptions/route.ts` and `src/adapters/jobs/reconcile-stripe-subscriptions.ts`. Once the cascade removes those rows, reconciliation has nothing left to discover or cancel.
5. **The current invalidation rationale only covers a narrower case:** cancel-before-delete ordering and idempotent retry behavior do make already-observed Stripe cancellations safe, but they do not protect against a Stripe mapping that appears after the webhook's lookup and before its delete.
