...
- 2026-07-14 (post-merge review correction): the key lifecycle above was still incomplete. The abandon key was mount-scoped rather than bound to the session it targets and was never rotated when its success outcome was consumed, and nothing retired the preserved start key when the recovery session was resolved by abandonment — so a same-config Start after a successful abandon replayed the stale cached outcome (this doc's dead-end recreated through the recovery flow itself, or navigation into the just-abandoned session). Filed and fixed as [BUG-295](./bug-295-preserved-idempotency-keys-replay-across-changed-intent.md): session-scoped abandon tokens rotated on consumed success plus start-key retirement on recovery resolution.
