...
**Amendment (2026-08-14 — [DEBT-466](../debt/debt-466-checkout-idempotency-replay-chain-exhaustion.md); extended 2026-08-15 — [DEBT-467](../debt/debt-467-trial-setup-checkout-stale-session-url-replay.md); reconciled 2026-08-17 — [DEBT-470](../debt/debt-470-checkout-replay-tail-jump.md)):** the default rule above and the Billing Portal example remain binding. BUG-245's subscription Checkout path is one deliberate, bounded exception: its caller UUID remains application-level idempotency while the Stripe adapter derives a deterministic provider key to collapse concurrent same-plan creates. DEBT-467 extends the same narrow license to the trial-payment-method setup path after adding strict live retrieval. These exceptions are licensed only while all of these conditions remain true:
