...
- subscription Checkout may select the unique newest full-renewal-metadata Session through a bounded, reverse-chronological provider-list scan before entering its named recovery ladder. An open listed candidate is live-retrieved before reuse; an ambiguous, malformed, incomplete, or failed scan falls back to the deterministic primary result and the bounded ladder. Trial setup does not use this scan and retains its own named, bounded strict-retrieval ladder;
