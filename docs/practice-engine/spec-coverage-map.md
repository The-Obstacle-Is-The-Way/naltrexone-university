...
| Core question loop (fetch → render → submit → grade → explain) | SPEC-012 | Implemented | Active exam uses draft-save/finalize rather than per-question submit; 2026-09-21 status correction: BUG-239's implicit latest-reader fallback gap was resolved in PR #290 |
| Practice sessions (start → answer → navigate → review → end/finalize → summary) | SPEC-013 | Implemented | Active exam draft/finalize flow shipped; 2026-09-21 status correction: BUG-238's draft timing validation gap was resolved in PR #287 |
...
**2026-09-21 forward note:** The preceding dated snapshot is historical. BUG-238
and BUG-239 are now resolved and archived (PRs #287 and #290); their links above
carry the closeout receipts. BS-014's archived record retains optional decisions,
not an unarchived implementation. This note does not reverify the full map.
