...
**Status note (2026-04-23).** In the currently shipped exam-flow contract, the two controls never co-occupy a surface: exam mode shows only `Mark for review`, and feedback/review surfaces such as post-exam review, tutor/quick practice after feedback, and `/app/questions/[slug]?mode=review` show only `Bookmark`. The stage itself is the disambiguator, which is why DEBT-365 Concern 2 was closed as intentional-by-design rather than with a rename. The contingency pattern above remains the rule if a future surface needs both.
