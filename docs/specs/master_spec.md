...
> Root-level configuration files exist but are not repeated here unless relevant. This tree is **exact** for the directories requested.

```text
/app
  /(marketing)
    layout.tsx
      - Public layout with <ClerkProvider>, site header/footer, Tailwind globals.
    page.tsx
      - Marketing homepage with CTA buttons (Sign in / Pricing).
    pricing/page.tsx
      - Pricing page with Monthly/Annual plan cards and Subscribe buttons.
    checkout/success/page.tsx
      - Authenticated-only success page: reads session_id, syncs Stripe -> DB, redirects to /app/dashboard.
  ...
  /(app)
    /app
      layout.tsx
        - App shell layout; enforces subscription server-side; nav links; redirects to /pricing if unsubscribed.
      dashboard/page.tsx
        - Dashboard with stats cards and recent activity.
      practice/page.tsx
        - Practice setup page (mode/count/filters) + Start button.
      practice/[sessionId]/page.tsx
        - Session runner page; uses getNextQuestion + submitAnswer + endPracticeSession.
      review/page.tsx
        - Missed questions list + reattempt flow.
      bookmarks/page.tsx
        - Bookmarked questions list + start reattempt.
      billing/page.tsx
        - Subscription status + "Manage billing" (portal) button.

      _actions
        actionResult.ts
          - Shared ActionResult<T> types.
        stripe.actions.ts
          - createCheckoutSession, createPortalSession.
        practice.actions.ts
          - startPracticeSession, endPracticeSession.
        questions.actions.ts
          - getNextQuestion, submitAnswer.
        stats.actions.ts
          - getUserStats.
        review.actions.ts
          - getMissedQuestions.
...
/components
  /ui
    - shadcn/ui components generated by CLI (Button, Card, Dialog, Badge, Tabs, etc.).
  /layout
    AppHeader.tsx
      - Top nav for app routes (Dashboard/Practice/Review/Bookmarks/Billing).
    MarketingHeader.tsx
      - Header for public routes.
  /markdown
    Markdown.tsx
      - ReactMarkdown wrapper with remark-gfm + rehype-sanitize, used for stems/explanations/choice text.
  /question
    QuestionCard.tsx
      - Displays stem + choices UI + bookmark toggle.
    ChoiceRadioGroup.tsx
      - Choice selection (Radix/shadcn).
    AnswerFeedback.tsx
      - Correct/incorrect banner; shows correct answer label.
    ExplanationPanel.tsx
      - Renders explanation markdown when allowed.
  /stats
    StatCard.tsx
      - Dashboard stat tile component.
    RecentActivityList.tsx
      - List of recent attempts.

/lib
  env.ts
    - Zod-validated environment variables (server + client).
  db.ts
    - Neon + Drizzle client factory (singleton).
  auth.ts
    - Clerk helpers: getClerkUserOrThrow(), ensureUserRow(), requireSubscriptionOrThrow().
  stripe.ts
    - Stripe SDK initialization (server-only).
  subscription.ts
    - Entitlement logic: computeActiveSubscription(userId).
  markdown.ts
    - rehype-sanitize schema for GFM tables/links/code blocks.
  content
    parseMdxQuestion.ts
      - Split content into Stem/Explanation; canonicalization helpers for hashing.
    schemas.ts
      - Zod schemas for content pipeline.
  stats
    computeStats.ts
      - Pure functions to compute accuracy/streak from attempt timestamps.
  review
    queries.ts
      - Drizzle query builders for missed questions.
  bookmarks
    queries.ts
      - Drizzle query builders for bookmarks.

/db
  schema.ts
    - Drizzle schema (Section 3).
  migrations/
    0000_init.sql
      - Creates pgcrypto extension + all tables/indexes/enums.
    meta/
      - drizzle-kit metadata (generated).

/scripts
  seed.ts
    - Seed/upsert questions from /content/questions/**/*.mdx.

/content
  /questions
    /opioids
      buprenorphine-induction-precipitated-withdrawal.mdx
      ...
    /alcohol
      ...
    /general
...
/tests
  /unit
    helpers.ts
      - Shared helpers for unit tests (seeded RNG, canonical json, etc.).
    README.md
      - Documents how unit tests are colocated next to source but share helpers here.
  /integration
    db.integration.test.ts
      - Tests migrations + basic CRUD with real Postgres.
    actions.stripe.integration.test.ts
      - Tests createCheckoutSession/createPortalSession with Stripe mocked.
    actions.questions.integration.test.ts
      - Tests getNextQuestion/submitAnswer with mocked auth + real DB.
  /e2e
    global.setup.ts
      - Clerk Playwright auth state generation using @clerk/testing.
    auth.spec.ts
      - Sign-in and access protected route.
    subscribe.spec.ts
      - Subscribe flow via Stripe test mode.
    practice.spec.ts
      - Practice session end-to-end: start -> answer -> summary.
    review.spec.ts
      - Missed questions + reattempt flow.
    bookmarks.spec.ts
      - Bookmark toggle + bookmarks page.
```
...
* `lib/stripe.ts`, `lib/subscription.ts`, `lib/auth.ts`
...
* `lib/content/schemas.test.ts` (colocated): validates frontmatter rules.
* `tests/integration/actions.questions.integration.test.ts`: submitAnswer inserts attempts and grades correctly.
...
3. Build `getNextQuestion` and `submitAnswer`.
4. Build practice runner UI for single-question flow (no sessions yet):

   * fetch next question
   * select choice
   * submit and show explanation
5. Add bookmark toggle button on question view (calls toggleBookmark).
...
* `lib/stats/computeStats.test.ts` (colocated): streak and accuracy calculations.
* `tests/e2e/practice.spec.ts`: answering questions updates dashboard stats.

**Implementation Checklist:**

1. Implement `getUserStats()`.
2. Build `/app/dashboard` page with stat cards and recent list.
...
* stats computations (accuracy, streak)
* content parsing logic (stem/explanation extraction)
...
* Drizzle migrations and schema integrity
* DB queries (missed questions, bookmarks)
* Server actions with mocked Clerk auth + real DB
