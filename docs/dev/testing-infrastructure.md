...
Before a local trial checkout mutates provider state, `assertLocalTrialCheckoutReplayCapacity()` performs a read-only, current-(user, monthly, `trial:7`) census. It ignores CI, where every job's fresh database supplies a new app-user UUID. Locally it fails early with `[E2E_CHECKOUT_CHAIN_SATURATED]` only when terminal Sessions created less than 24 hours ago exceed the production traversal cap; the message reports counts and no identifiers or key material. This is the only receipt that scopes a local `trial-start` gate exception: a generic `checkout=error` is still a defect until the focused run produces that count. The 2026-08-17 characterization found 11 current-tuple completed Sessions inside that window against cap 10. Stripe's 24-hour statement is only the earliest pruning eligibility, not an exact self-heal time. The constant-depth structural follow-up is [DEBT-470](../debt/debt-470-checkout-replay-tail-jump.md).
