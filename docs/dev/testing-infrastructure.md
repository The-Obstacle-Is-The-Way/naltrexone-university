...
Local tracing uses `retain-on-failure`, so a first-attempt product failure keeps
its diagnostic trace even with zero retries. Successful attempts discard their
traces. CI tracing stays disabled, and both workflows exclude trace archives
from public artifacts because traces can contain authentication material.
...
Normal local `pnpm test:e2e` runs reuse that per-clone Docker database; they do not tear it down or reset its volume. Only `pnpm db:test:reset` invokes `docker compose down -v`. Checkout Sessions created by the required redirect-boundary tests and by explicit hosted-smoke runs can remain deterministic recovery-chain rungs while Stripe retains their idempotency keys. [DEBT-466](../_archive/debt/debt-466-checkout-idempotency-replay-chain-exhaustion.md) and [DEBT-470](../_archive/debt/debt-470-checkout-replay-tail-jump.md) retain provider-faithful blocking contract coverage for that traversal; do not rotate the user or drop the database volume to mask a traversal failure.
...
The scheduled Stripe runner requires eleven named cases: the two trial-clock outcomes plus nine `FakeStripeCheckoutClient` scenarios for frozen idempotent replay versus live retrieval, reverse-chronological cursor pagination with `has_more`, terminal-Session visibility, same-key/different-parameter rejection, and a customer's Subscription listing by status and retrieval by id, including the default listing's exclusion of canceled Subscriptions, a single Subscription cancel whose repeat Stripe rejects as a 404 `resource_missing`, Customer Search by whole, case-insensitive metadata value, a PaymentMethod's attach and detach rules, and a Subscription default update. The real half attaches a test card, creates an active Subscription on the TEST Price plus a second one it cancels at once, and cancels any Subscription still active in cleanup. It also creates Search Customers, which cleanup deletes, and polls each Search read until Stripe has indexed them. `pnpm test:stripe-provider` is the one supported manual entry point; it loads `.env.local` without overriding exports, validates the TEST-mode key and Price through the shared provider gate, and injects both `RUN_STRIPE_*` flags only into its bounded child process. The ordinary integration lane stays credential-free and reports all eleven cases as skipped. The child receives a 20-second scheduled-only test budget because a healthy live trial-clock case was measured at 10.28 seconds, just beyond the ordinary integration lane's global 10-second budget; the existing five-minute process-tree limit remains the outer failure bound. The Search case registers its own 120-second budget. The real half waits for Stripe to index new Customers within one 60-second bound shared by all of the case's reads, so that budget covers the whole wait. The runner rejects a zero-exit skip, a missing/duplicate file or case, any non-passing case, and malformed reporter output. The 2026-08-26 local activation receipt was `PASS executed=6 passed=6 skipped=0`; hosted run `33038731445` supplied the same six-case receipt at verified `main` head `3162a7be91e57eb5c66f0575d675414c91646991` on 2026-08-27. The fifth Checkout scenario (DEBT-472 step 5, 2026-09-24 UTC) raised the requirement to seven cases; its local TEST-mode receipt was `PASS executed=7 passed=7 skipped=0` at 16:53:58Z. The sixth scenario, Subscription cancel (2026-09-25 UTC), raised the requirement to eight cases; its local TEST-mode receipt was `PASS executed=8 passed=8 skipped=0` (00:13:39Z–00:14:19Z). The seventh scenario, customer Search (2026-09-25 UTC), raised the requirement to nine cases; its local TEST-mode receipts were `PASS executed=9 passed=9 skipped=0` (07:56:32Z–07:57:33Z and 07:57:51Z–07:59:01Z, and 08:38:19Z–08:39:17Z after the shared-bound review correction). The PaymentMethod and Subscription-default scenarios (2026-09-25 UTC) raised the requirement to eleven cases; their local TEST-mode receipt was `PASS executed=11 passed=11 skipped=0` (10:21:44Z–10:23:03Z).
...
CI runs the required E2E layer only on pushes and human same-repo PRs, because it needs repository secrets; Dependabot and fork PRs skip it. Browser installation and all 398 browser tests run on every trigger because they need no provider credential. Before unit coverage, required CI runs `pnpm lint:doubles`; local `pnpm lint` invokes the same fail-closed test-double fidelity ratchet, while ordinary Vitest runs no longer pay for it through `globalSetup`. A final `Evidence summary` step reads the actual unit, integration, browser, Build, and E2E `steps.<id>.outcome` values, runs after failures with `if: ${{ !cancelled() }}`, and writes skipped evidence plus a warning instead of letting the aggregate green stand alone ([DEBT-473](../_archive/debt/debt-473-green-without-evidence.md) F5, step 3). Fork PRs run typecheck, lint, unit, integration, browser, and build; the Build step uses real public values when available and shape-valid server-only placeholders. `.github/workflows/stripe-hosted-checkout-smoke.yml` runs only on its daily schedule or explicit dispatch, uses a separate `E2E_STRIPE_OWNER`, and selects only the observational `stripe-hosted` project. Both workflows upload `playwright-report/` after every non-cancelled run and upload `test-results/` only when their E2E step failed. Every upload excludes `**/.auth/**` and `**/trace.zip`; Playwright tracing is disabled in CI but uses `retain-on-failure` locally. [BUG-307](../_archive/bugs/bug-307-public-playwright-artifacts-expose-test-session-credentials.md) records the promoted one-file artifact's zero-match scan and closure. Path-filtering the required workflow is intentionally deferred: required-check naming and skipped-workflow behavior can block merges or let a misclassified code change evade the lane, while the hosted split and bounded Chromium installer remove most of the incentive.
