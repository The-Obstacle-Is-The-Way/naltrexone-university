...
The scheduled Stripe runner requires eight named cases: the two trial-clock outcomes plus six `FakeStripeCheckoutClient` scenarios for frozen idempotent replay versus live retrieval, reverse-chronological cursor pagination with `has_more`, terminal-Session visibility, same-key/different-parameter rejection, and a customer's Subscription listing by status and retrieval by id, including the default listing's exclusion of canceled Subscriptions, and a single Subscription cancel whose repeat Stripe rejects as a 404 `resource_missing` (the real half attaches a test card, creates an active Subscription on the TEST Price plus a second one it cancels at once, and cancels any Subscription still active in cleanup). `pnpm test:stripe-provider` is the one supported manual entry point; it loads `.env.local` without overriding exports, validates the TEST-mode key and Price through the shared provider gate, and injects both `RUN_STRIPE_*` flags only into its bounded child process. The ordinary integration lane stays credential-free and reports all eight cases as skipped. The child receives a 20-second scheduled-only test budget because a healthy live trial-clock case was measured at 10.28 seconds, just beyond the ordinary integration lane's global 10-second budget; the existing five-minute process-tree limit remains the outer failure bound. The runner rejects a zero-exit skip, a missing/duplicate file or case, any non-passing case, and malformed reporter output. The 2026-08-26 local activation receipt was `PASS executed=6 passed=6 skipped=0`; hosted run `33038731445` supplied the same six-case receipt at verified `main` head `3162a7be91e57eb5c66f0575d675414c91646991` on 2026-08-27. The fifth Checkout scenario (DEBT-472 step 5, 2026-09-24 UTC) raised the requirement to seven cases; its local TEST-mode receipt was `PASS executed=7 passed=7 skipped=0` at 16:53:58Z. The sixth scenario, Subscription cancel (2026-09-25 UTC), raised the requirement to eight cases; its local TEST-mode receipt was `PASS executed=8 passed=8 skipped=0` (00:13:39Z–00:14:19Z).
