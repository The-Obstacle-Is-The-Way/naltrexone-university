...
- SHA-pin every action. Four workflows currently exist: `tests/ci-workflow.test.ts` enumerates permitted secret consumers and verifies pins in CI and both Stripe workflows; `scripts/security-txt-renewal.test.ts` separately verifies the renewal workflow's pins and least-privilege permissions. Every added workflow needs the corresponding contract. `strictDepBuilds`/`allowBuilds` constrain install scripts, not all later package execution.
