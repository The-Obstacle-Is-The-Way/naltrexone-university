...
| `FakeRateLimiter` | Dated shared-contract waiver (2026-08-23): isolation suite plus `rate-limiter.integration.test.ts` and `rate-limiter-guards.integration.test.ts`; no fake↔real parity claim. | Scripted-result fake; its default path does not enforce a real request count/window threshold and does not reproduce Postgres concurrency, atomicity, or cleanup contention. |
