...
3. Notify the New York Attorney General, Department of State, and Division of State Police as required, without delaying resident notice. The regulator submission must state the **timing, content, and distribution** of the resident notice and the **approximate number of New York residents affected**, and must include **a copy of the template resident notice**. Notify the Department of Financial Services only if the operator is a covered entity under the cited rule; current status must be verified. *(Verified 2026-08-27: the operator is not a Department of Financial Services covered entity, so no DFS notice applies. Regulator submission path verified the same day: the New York State Data Breach Notification Collaboration accepts a single submission through the Attorney General's online portal at https://formsnym.ag.ny.gov/OAGOnlineSubmissionForm/faces/OAGSBHome, which effects notice to the Attorney General, the Department of State Division of Consumer Protection, and the Division of State Police; the fallback is direct submission to all three agencies by email or fax, carrying the same fields the portal collects (entity and submitter, number of persons and New York residents affected, breach and discovery dates, description, information acquired, notice method, and the template resident notice): the Attorney General (Consumer Frauds and Protection Bureau, breach.security@ag.ny.gov, fax 212-416-6003), the Division of State Police (New York State Intelligence Center, risk@nysic.ny.gov, fax 518-786-9398), and the Department of State Division of Consumer Protection (security_breach_notification@dos.ny.gov, fax 518-473-9055), per the [Department of State business fact sheet](https://dos.ny.gov/businesses-nys-information-security-breach-and-notification-act) verified 2026-08-28. The reporting-form template hosted by the Office of Information Technology Services is the state-entity variant under State Technology Law § 208 and is not a business submission route; use its field layout only, and send to the three § 899-aa recipients above.)*
...
| 2026-08-27 | Tabletop, scenario 1: the production `CLERK_SECRET_KEY` (the Backend API key; distinct from `CLERK_WEBHOOK_SIGNING_SECRET`, which is rotated alongside it only if it was also exposed) is exposed in a public paste. Walked § 2 in order: record the discovery time; capture the key's dashboard `Last used` value and the capture time before rotating; rotate the key in the Clerk dashboard and the Vercel environment and redeploy; revoke sessions; open a case at security@clerk.dev; triage. Classified SEV-2 per § 4 as an exposed secret, with every other § 4 SEV-1 trigger (confirmed or reasonably likely access or acquisition, active credential abuse, broad compromise) remaining operative. Affected population at the exercise date: the owner's own accounts plus one inactive third-party account. § 5 is applied in order: first record whether private information was accessed or acquired, or is reasonably believed to have been, using the captured `Last used` value, Clerk's Admin Logs and Application Logs, and the provider case, documenting each source's coverage and retention; read-only Backend API requests (for example listing users) may not be observable in those logs, so an unobservable read scope is recorded as unresolved, never as proof of no access, and incomplete audit coverage escalates the notification analysis rather than letting containment plus absent events close it. If access or acquisition is established or reasonably believed and the affected person is a New York resident, provide individual notice and the Attorney General portal submission; if New York notice is not required, record that conclusion on its own and then run the § 6 state-by-state analysis for every affected resident before any global no-notice determination is written and retained. The 5,000-resident consumer-reporting-agency step is not reached in any branch. | Owner with assistant | No open exceptions |
| 2026-08-27 | Tabletop, scenario 2: a provider breach notice arrives from Sentry. Discovery time is set from the notice; Sentry's category-level inventory for this product is exceptions, stack traces, and request context with IP storage disabled since 2026-08-13 and no replay, credentials, or payment data, so the exercised outcome is: first enumerate, at the time of the notice, the retained event fields and representative event types in the affected project together with the scrubbing settings in force and any evidence limits (a qualifying value inside request context must be looked for, not assumed absent); only if that inventory shows no § 899-aa private-information element is a written no-notice determination retained for five years, followed by the § 7 review. | Owner with assistant | No open exceptions |
...
- [New York Department of State: data breach reporting form and compliance guidance for businesses](https://dos.ny.gov/data-breach-reporting-form-and-compliance-guidance-businesses) (links the Attorney General portal used by the Data Breach Notification Collaboration)
- [New York Department of State: Information Security Breach and Notification Act fact sheet for business](https://dos.ny.gov/businesses-nys-information-security-breach-and-notification-act) (agency contacts for the fallback path)
- [New York State Office of Information Technology Services: breach notification and incident reporting](https://its.ny.gov/breach-notification-and-incident-reporting) (hosts the reporting-form template; its listed recipients are for state entities under State Technology Law § 208, not the § 899-aa business path)

## 2026-09-16 delivery-channel supplement

**Non-lawyer correction, F09/F12 in the [DEBT-414 review](../debt/debt-414-public-legal-pages-privacy-terms.md#2026-09-16-non-lawyer-adversarial-pre-counsel-review).** The August 27 adoption and exercise are historical evidence; they do not establish that the following channel decision has been exercised. Repeat this part of the tabletop before relying on it.

Under [N.Y. GBL § 899-aa(5)](https://www.nysenate.gov/legislation/laws/GBS/899-AA), select and document the legally permitted method before sending:

1. Written notice is permitted under (a). Direct electronic notice under (b) requires the recipient's express consent to that form of breach notice and a log of each notification. That consent cannot be required as a condition of doing business. A verified email address, subscription-email consent, or accepted Terms is not evidence that this separate condition was met. Telephone notice under (c) requires its own notification log. The application ordinarily holds no mailing address or phone number; do not presume an available channel or collect additional information without deciding necessity and authority.
2. Substitute notice under (d) requires demonstrating to the Attorney General that ordinary notice would cost more than $250,000, the affected class exceeds 500,000, or sufficient contact information is unavailable. Record the basis and promptly obtain advice on that demonstration; email-only contact data does not automatically authorize substitution. Substitute notice comprises **all** applicable components: email where available, conspicuous website posting if a website exists, and notification to major statewide media. Preserve each distribution record. The existing § 6 explanation of (d)(1)'s compromised-email substitution still applies to that component; it does not replace website/media notice. No tested customary-IP online-notice mechanism is currently evidenced.
3. Build the affected-state channel/deadline matrix with counsel immediately on discovery. If the intended email channel is unavailable or legally insufficient, escalate that day; do not wait for the 30-day outer limit. Record the chosen method, consent or substitute-notice basis, notices, timestamps and logs. This supplement does not assert that a specific future incident qualifies for substitute notice.

**Incident stores:** Apple Notes/iCloud and the Bitwarden backup designated in § 1 are conditional personal-data stores. Before first use for affected-person data, document provider terms, access/MFA and recovery, encryption scope, backup/deletion behavior and the minimum necessary fields. Keep credentials out of incident narratives. Review incident-record retention annually and preserve the five-year statutory inadvertent-disclosure decision record where applicable. The corresponding inventory correction is in the Information Security Program and Privacy publication copy; no incident or customer-data transfer is asserted to have occurred.

**2026-09-16 recording-sequence clarification (F12):** start § 2's record immediately with non-identifying discovery metadata and known facts, using the reporter's role rather than personal identifiers while the store reviews remain open. Preserve original evidence in its existing restricted source and continue containment and deadline tracking without waiting for those reviews. Add affected-person identifiers and the remaining affected-person triage details to the designated evidence log and backup only after the Apple Notes/iCloud and Bitwarden reviews above are documented.
