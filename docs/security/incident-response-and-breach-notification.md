...
| 2026-08-27 | Tabletop, scenario 1: the production `CLERK_SECRET_KEY` (the Backend API key; distinct from `CLERK_WEBHOOK_SIGNING_SECRET`, which is rotated alongside it only if it was also exposed) is exposed in a public paste. Walked § 2 in order: record the discovery time; rotate the key in the Clerk dashboard and the Vercel environment and redeploy; revoke sessions; open a case at security@clerk.dev; triage. Classified SEV-2 per § 4 (exposed secret; SEV-1 only on evidence of use). Affected population at the exercise date: the owner's own accounts plus one inactive third-party account. § 5 is applied in order: first record whether private information was actually accessed or acquired (the exposed key's use is checked against Clerk's Admin Logs and Application Logs in the dashboard, which record Backend API and instance activity, and against the provider case; exposure alone is not a breach); only if it was, or is reasonably believed to have been, and the affected person is a New York resident, provide individual notice and the Attorney General portal submission; otherwise retain a written no-notice determination. The 5,000-resident consumer-reporting-agency step is not reached in any branch. | Owner with assistant | No open exceptions |
