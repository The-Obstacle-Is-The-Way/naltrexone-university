...
**Out-of-band contact:** the owner's personal mobile number, recorded in the private incident record rather than in this public repository (verified absent from the repository and its history on 2026-08-27) and confirmed at the 2026-08-27 tabletop; it is the same device that carries the owner's authenticator and sign-in prompts, so it is exercised daily. It is the channel for use when the support mailbox, its forwarding chain, or the email provider is itself compromised or unavailable; an email-provider incident must not sever the incident channel.
...
| 2026-08-27 | Verified during the tabletop: owner account access under MFA at every provider with recovery codes vaulted; provider security contacts (live-checked 2026-08-13); the regulator-form location and single-portal path (§ 5); 30-day clock arithmetic from the earliest documented discovery time; resident-notice content elements (§ 6); the out-of-band contact and evidence-log stores (§ 1). | Owner with assistant | Repeat at least annually (program § 8) |
