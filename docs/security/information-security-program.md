...
| Resend | Transactional account, billing, renewal, and subscription notices; configured 2026-09-16 (verified domain, sensitive production-only key); the implemented durable queue makes no provider call while `RESEND_API_KEY` is absent | Recipient address, message body, delivery status, provider event identifiers |
