...
> **STATUS: ADOPTED 2026-08-27 (owner sign-off in § 9).** This document converts the New York SHIELD Act safeguard categories into an operational program for the current sole proprietorship. The former `OPEN` items below were closed on the dates recorded with each item, with evidence of the type each item required (the one control that was still pending at adoption, the Resend account, was created and verified on 2026-09-16, resolving open exception 4 in § 9): the provider and account rows were verified against the live provider accounts (the support-mail delivery check on 2026-08-05 and 2026-08-06; the measured CLI/API audit on 2026-08-13; owner dashboard verification and hardening through 2026-08-27), the operator-device row by host inspection, the physical-safeguard row by owner attestation, and the training and drill rows by the recorded read-through, the executed restore drill, and the tabletop; each checked item labels its evidence source, and plan-gated limitations are recorded as open exceptions in § 9 rather than silently passed. It is not legal advice.

## 1. Scope and coordinator

This program covers computerized personal and private information owned, licensed, or maintained by Addiction Boards, including information processed through Clerk, Stripe, Neon, Vercel, Sentry, ImprovMX, Google Workspace, Resend, the conditional incident-evidence stores designated in the incident procedure § 1 (Apple Notes/iCloud and its Bitwarden backup), operator devices, and local development environments.
...
| Resend | Subscription acknowledgments and renewal notices (the only implemented producers: the Stripe-webhook acknowledgment and the daily renewal cron); configured 2026-09-16 (verified domain, sensitive production-only key); the implemented durable queue makes no provider call while `RESEND_API_KEY` is absent | Recipient address, message body, delivery status, provider event identifiers |
| Apple Notes/iCloud (conditional incident store) | Locked incident evidence log designated in the incident procedure § 1; the note is created at first use, and provider terms, access/recovery, encryption, retention, and disposal are reviewed before affected-person data is stored | Limited incident facts, affected identifiers, notification records, and decision evidence, only if an incident occurs |
| Bitwarden (conditional incident store) | Secure-note backup of the incident log, designated in the incident procedure § 1; same first-use review as the Apple Note | The same limited incident evidence as the Apple Note, only if an incident occurs |
...
- [x] Confirm MFA/passkeys and recovery methods for GitHub, Clerk, Stripe, Neon, Vercel, Sentry, ImprovMX, Google Workspace, Resend, and DNS. **DONE for every existing account 2026-08-20 to 2026-08-24 (owner-verified in each dashboard); Resend DONE 2026-09-16 (team created by GitHub sign-in with no separate Resend password, so its second factor and recovery codes are the verified GitHub ones):** GitHub 2FA with authenticator app and GitHub Mobile, no SMS, recovery codes regenerated and vaulted; Vercel two-factor active (passkey plus authenticator, recovery codes vaulted; two active tokens, both the owner's); Stripe passwordless sign-in (Google plus passkey) with authenticator two-step and a regenerated backup code; Google Workspace 2SV with authenticator and prompt, SMS removed as a factor, backup codes vaulted, the account password rotated 2026-08-22, and the personal recovery mailbox hardened the same way; Clerk dashboard passkey, TOTP, and backup codes (added 2026-08-22); Sentry authenticator, passkey, and recovery codes with organization-wide 2FA required (2026-08-24); ImprovMX 2FA plus passkey and no API keys; Neon sign-in is OAuth only (Vercel and GitHub identities, no password set); DNS and the registrar are managed solely through the Vercel account (the domain was purchased via Vercel; auto-renew verified true, expiry 2027-02-04); Resend was created 2026-09-16 with GitHub sign-in only (no separate password), which resolves open exception 4 in § 9; its API key is sending-only and domain-restricted. Recovery codes live in the owner's password manager, never in this repository.
...
- [x] Confirm production and preview environment-variable scope and rotate any stale credentials. **DONE 2026-08-13 (API-verified by name and target only):** the Vercel variable set matches the application's expected keys across production, preview, and development; `RESEND_API_KEY` was absent everywhere at the audit (fail-closed queue) and was added on 2026-09-16 as a sensitive production-only variable after Resend activation, still absent from preview and development; the E2E-only Stripe webhook owner variable is preview-scoped; no stale variables were found and none required rotation; the E2E preflight now fails closed on live-mode Stripe keys (PR #810).
...
- [x] Confirm Stripe restricted-key scope, webhook endpoints/secrets, portal cancellation, fraud controls, and administrator access. **DONE 2026-08-13 to 2026-08-22:** the live API-key inventory was reviewed and matches the expected set with no stray keys (key types and rotation dates are held in the private adoption ledger); exactly one live webhook endpoint (`/api/stripe/webhook`, seven events, current API version); portal cancellation is enabled at period end; Radar runs provider defaults (pre-revenue); the dashboard has one member (owner, Super Administrator) with passwordless sign-in and two-step verification.
...
| 2026-08-28 | Promotion-PR review correction (#857): exception (4) in the adoption line reworded from "Resend has no account yet, so its MFA row closes at activation" to the closure condition it carries today (account exists, MFA or a passkey configured, recovery method verified) | Owner-directed session | The 2026-08-27 wording is preserved in git history at the adoption commit; the exception list is unchanged |
| 2026-09-16 | Adoption exception (4) closed: the Resend account was created with GitHub sign-in only (its second factor and recovery codes are the verified GitHub ones), the sending domain was verified, and one real delivery passed DKIM, SPF, and DMARC. The conditional incident-evidence stores designated at the 2026-08-27 tabletop (Apple Notes/iCloud, Bitwarden backup) were added to § 1 scope and the § 2 inventory; the supplement below qualifies the analytics and retention statements | Owner-directed session ([DEBT-414 review](../debt/debt-414-public-legal-pages-privacy-terms.md#2026-09-16-non-lawyer-adversarial-pre-counsel-review)) | The adoption line below is kept as last corrected on 2026-08-28 and is not rewritten with post-adoption facts; exceptions (1), (2), (3), (5), and (6) remain open |
...
- **Resend:** the September 16 activation record (§ 9 revision row dated 2026-09-16) closes adoption exception (4); the adoption line keeps the exception as last corrected on 2026-08-28 (§ 9) rather than being rewritten with the closure. The committed application sends subscription acknowledgments and renewal notices. A direct delivered test is not proof that the deployed application's queue and daily cron sent a notice within its statutory window. Record that separate evidence before relying on automated delivery.
...
- **Conditional incident-data providers:** Apple (Notes/iCloud) and Bitwarden (secure-note backup), as designated by the adopted incident procedure § 1, are now listed in § 1 scope and the § 2 inventory as conditional stores and are in scope for the access, secret-rotation, retention, and incident reviews above. Potential data comprises limited incident facts, affected identifiers, notification records and decision evidence. The incident note is created at first use. Provider terms, access/recovery, encryption, retention and disposal review for this use are still to be recorded before affected-person data is stored. This entry is not a claim that the existing eight production providers were the complete possible data-flow inventory.
