...
**Every open-ended or non-guaranteed retention policy above requires its own annual review with a recorded justification** — not just a subset. The current open-ended set is: (1) rate-limit cleanup with no hard maximum row age; (2) best-effort idempotency pruning; (3) unresolved Stripe events held until replay/resolution; (4) handled Clerk events with no terminal period; (5) Clerk deletion tombstones with no terminal period; (6) pending Stripe-customer cleanup held until success. For each, the annual review records why continued retention remains necessary or converts it to a bounded policy. *(Corrected 2026-08-03 per promo #724 review — an earlier revision said "the two indefinite policies," undercounting the list above.)* Provider, support-mail, export, backup, and local-copy retention must be added to the inventory once verified.
