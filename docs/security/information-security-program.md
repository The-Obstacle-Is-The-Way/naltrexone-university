...
| Resend | Transactional account, billing, renewal, and subscription notices (planned — not yet active; no messages are sent through Resend until its adapter and credentials are configured) | Recipient address, message body, delivery status, provider event identifiers |
...
- [ ] Record the Google Workspace destination inbox's retention and access settings.
